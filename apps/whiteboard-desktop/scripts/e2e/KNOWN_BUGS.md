...
- A review whose repository directory moves or is deleted renders `WhiteboardApiError: Review operation failed.` — fix-pr #355
...
- **Actual:** exits 1 with
  `{"name":"WhiteboardApiError","message":"Not found."}` thrown from
  `WhiteboardApiClient.response`. `review info` with no `--review` fails the same way,
  so the verb is unusable.
- **Notes:** `whiteboard-info.ts:33` calls `client.read("/")`, and
  `whiteboard-api-client.ts:75` builds `${serverUrl}/reviews-api${route}`, so the
...
- **Actual:** exits 1 with
  `{"name":"WhiteboardApiError","message":"Review or version not found."}` thrown from
  `WhiteboardApiClient.post`. Reproduced for both importable fixtures.
- **Notes:** `whiteboard-app.ts:63` does
  `review = await client.read("/<uuid>")` expecting
  `Pick<WhiteboardApiSummary, "reviewId" | "title">`, but `GET /reviews-api/:id`
  returns `inspectSnapshot(...)` — an array of blocks — unless `full=true`
  (`session-api/http.ts:620-637`). So `review.reviewId` is `undefined` and
  `whiteboard-app.ts:93` posts to `/reviews-api/undefined/open`, which 404s.
...
- **Notes:** `whiteboardTelemetry.contribution.ts:36-41` stores the shown flag
  before it calls `notificationService.prompt`, so the notice is spent whether
  or not anyone saw it, and
  `whiteboardCuratedExtensions.contribution.ts:664` runs
...
- **Notes:** `whiteboardCommunity.contribution.ts:35-38` stores the flag from the
  dialog's promise callback, and
  `whiteboardCuratedExtensions.contribution.ts:664` reloads the window on a fresh
...
- **Actual:** neither click reaches its button. Playwright reports
  `<header class="whiteboard-topbar">…</header> intercepts pointer events` for the
  pill and `<div class="whiteboard-topbar-actions">…</div> … subtree intercepts
  pointer events` for the toggles, and `document.elementFromPoint` at the
  centre of each control returns an element inside `.whiteboard-topbar`.
- **Notes:** measured in the workbench renderer (viewport 1200x800): the review
  canvas starts at y=74, `.whiteboard-topbar` is `position: sticky` from 74 to 109
  with `z-index: var(--whiteboard-debug-layer)` (`styles.css:2318`), and the whole
  ...
  above the canvas — so they land inside that band: `.whiteboard-toc-toggle`
  (`position: fixed; top: calc(32px + var(--whiteboard-page-top))`,
  `styles.css:4641-4646`) measures 92–124, and
  `.whiteboard-find-widget` (`position: absolute; top: 48px; z-index: 120`,
  ...
  the only way into the contents below a 1360 px shell (`whiteboard-toc.tsx:25`,
...
## A review whose repository directory moves or is deleted renders `WhiteboardApiError: Review operation failed.`
...
- **Actual:** the canvas renders only
  `<p role="status">WhiteboardApiError: Review operation failed.</p>` inside
  `.whiteboard-canvas-root [data-whiteboard-api]`, with no title, no document and no
...
- **Notes:** the stored `repositoryPath` is absolute and is never re-resolved —
  ...
  becomes the generic 500 at `session-api/http.ts:50`, which
  ...
  when `data.sourcePins(snapshot)` throws a 404 `WhiteboardInputError`
  ...
  today: the only `{ kind: "source" }` render (`whiteboardCanvasPart.ts:308`) never
...
- **Actual:** the CLI prints none of them. It goes to the launcher, which runs
  ...
  touched.
- **Notes:** `whiteboard-app.ts:49-50` calls `runtime.launch()` before it reads the
  pointer, and `whiteboard-app-launcher.ts:281-289` catches every discovery error on
  ...
  rethrowing anything but `null` from `runWhiteboardAppPick` would fix it. Two
  ...
  `review app launch` and retry `review app pick`." throw at `whiteboard-app.ts:52-55`
...
- **Expected:** the host starts, the reviews it can read are available, and the
  ...
  (`whiteboard-home.ts:364-366`, `:706-710`).
...
- **Notes:** `json-cutover.ts:92-109` collects a parse failure into
  ...
  (`whiteboardServerSupervisor.ts:290-313`), leaving the schema dump in the log.
  ...
  `report.errors`, and `migrateJsonWhiteboards` leaves every original untouched —
...
- **Expected (by the plan):** Home lists the review as needing migration and
  names the command to run, from the `MIGRATION_REQUIRED` `WhiteboardHomeError`
...
- **Notes:** `ensureJsonCutover` (`json-cutover.ts:238-270`) returns on its
  ...
  store (`session-api/store.ts:485`). The `MIGRATION_REQUIRED` error has no
  Desktop consumer at all: it reaches only `ListWhiteboardsResult.errors`
  (`whiteboard-home.ts:139-143`), `listWhiteboards` has one non-test caller,
  `publish-preparation.ts:24` — the only place that sets
  `reportUnreadableWhiteboards` — and `WhiteboardHomeError` appears nowhere in
  `packages/whiteboard/app/src`. What is worth fixing is the path where such a
...
- **Expected:** Review asks before it downloads and builds a language server,
  ...
  (`whiteboardCuratedExtensions.contribution.ts:244-527`), or at least the way the
...
- **Notes:** `golang.go@0.56.0` `dist/goMain.js:32837` `maybeInstallImportantTools`
  ...
  the pinned one.
  `whiteboardConfigurationDefaults.ts:104-107` already turns off that extension's
...
- **Notes:** `whiteboardLocalLanguageFeatures.acquire` (`:152-166`) adds the
