...
import { WhiteboardDesktopHost } from '../../whiteboard/electron-main/whiteboardDesktopHost.js';
import { WhiteboardMenubarMainService } from '../../whiteboard/electron-main/whiteboardMenubar.js';
import { WhiteboardUpdateDialog } from '../../whiteboard/electron-main/whiteboardUpdateDialog.js';
import { WHITEBOARD_DESKTOP_CHANNEL, WhiteboardDesktopChannel } from '../../whiteboard/electron-main/whiteboardDesktopChannel.js';
...
export class CodeApplication extends Disposable {
	...
	private whiteboardDesktopHost: WhiteboardDesktopHost | undefined;
	...
	async startup(): Promise<void> {
		...
		this.whiteboardDesktopHost = this._register(appInstantiationService.createInstance(WhiteboardDesktopHost));
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// Menubar: Review owns the native menu in the main process, because its
		// workbench never sends the menu data the stock service builds from.
		services.set(IMenubarMainService, new SyncDescriptor(WhiteboardMenubarMainService));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		if (this.whiteboardDesktopHost) {
			mainProcessElectronServer.registerChannel(WHITEBOARD_DESKTOP_CHANNEL, new WhiteboardDesktopChannel(this.whiteboardDesktopHost));
		}
		...
		this._register(accessor.get(IInstantiationService).createInstance(WhiteboardUpdateDialog));
