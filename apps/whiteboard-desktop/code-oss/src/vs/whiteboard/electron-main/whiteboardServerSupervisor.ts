...
import {
  ...
  resolveWhiteboardServerEntry,
} from "../common/whiteboardDesktopBootstrap.js";
import { WHITEBOARD_SERVER_RESTART_DELAYS } from "../common/whiteboardReconnect.js";
 ...
 */
export interface IWhiteboardServerProcess extends IDisposable {
...
export interface WhiteboardServerSupervisorOptions {
  ...
  readonly createProcess: () => IWhiteboardServerProcess;
  readonly telemetryEnabled?: boolean;
  readonly userExtensionsPath?: string;
}

export function createWhiteboardServerEnvironment(options: {
 ...
 */
export class WhiteboardServerSupervisor extends Disposable {
  private serverProcess: IWhiteboardServerProcess | undefined;
  ...
  private readonly connected = new DeferredPromise<WhiteboardDesktopConnection>();
  private readonly readyTimeout: number;

  constructor(private readonly options: WhiteboardServerSupervisorOptions) {
    super();
  ...
  private startWithEnvironment(
    ...
    if (this.stopping) return;
    const serverEntry = resolveWhiteboardServerEntry({
    ...
    const reader = new WhiteboardReadyEventReader({
      token: this.token,
    ...
    const onTerminated = (detail: string) => {
      ...
      const delay = WHITEBOARD_SERVER_RESTART_DELAYS[this.restartCount++];
      if (delay === undefined) {
        this.failStartup(
          new Error(
    ...
    const environment = createWhiteboardServerEnvironment({
    ...
    const started = serverProcess.start({
      ...
      entryPoint: "vs/whiteboard/electron-utility/whiteboardDesktopHostMain",
