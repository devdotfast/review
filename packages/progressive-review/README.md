...
The host owns review metadata, pinned source evidence, document versions, maps, checkpoints and conversations in one shared database. Clients never author `review.mdx`, `data.ts`, SQL, Git notes or bundles.
...
Publish an immutable checkpoint with `review.publish`. The reader can inspect Live or historical checkpoints, leave private drafts, submit feedback, or ask a fresh local agent. Questions and completed answers are saved in the host; Ask does not require the original author session. Trusted local execution is not filesystem/tool isolation.
...
Optional trace capture/search, desktop installation, settings and telemetry remain separate features. They are not prerequisites for JSON authoring or fresh-context Ask.
