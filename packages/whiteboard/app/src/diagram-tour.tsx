...
import { GuidedTourPanel } from "./whiteboard-components";
...
export function DiagramTourOverlay({
  ...
  // The overlay portals into .whiteboard-canvas-root, OUTSIDE .whiteboard-app — the
  ...
  return (
    <div
      ref={overlayRef}
      className={`diagram-tour-overlay whiteboard-app--theme-${theme} ${className ?? ""}`}
      role="dialog"
      aria-modal="true"
...
export function useDiagramTourShell(open: boolean, onClose: () => void) {
  ...
  // The desktop build wraps every canvas rule in @scope (.whiteboard-canvas-root),
  // so the overlay must portal INSIDE the canvas root or it renders unstyled.
  const portalTarget = useWhiteboardContainer();
