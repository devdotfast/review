...
import { WhiteboardBackgroundLaunch } from "./whiteboardBackgroundLaunch.js";
...
test("a normal launch leaves window creation and focus alone", () => {
	const launch = new WhiteboardBackgroundLaunch({});
...
test("a background launch shows the first window inactive and swallows focus until the user clicks", () => {
	const launch = new WhiteboardBackgroundLaunch(background);
	const options = { show: true };
...
test("only the value 1 marks a background launch", () => {
	assert.equal(new WhiteboardBackgroundLaunch({ DEV_FAST_WHITEBOARD_DESKTOP_BACKGROUND: "0" }).suppressesFocus, false);
	assert.equal(new WhiteboardBackgroundLaunch({ DEV_FAST_WHITEBOARD_DESKTOP_BACKGROUND: "" }).suppressesFocus, false);
});
