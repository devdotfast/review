...
1. `session_capabilities({})`. Draw only when `desktopAvailable` and `scratchpadEnabled` are true and `authoringMode` is `interactive`. Otherwise answer in chat; if the pad is off, say once that it can be turned on in Whiteboard Settings.
