...
import { AnchorLink, WhiteboardSection } from "./whiteboard-components";
...
function CalloutBlock({ node, children }: BlockProps<"callout">) {
  return (
    <blockquote data-tone={node.tone}>
      {node.title && <strong data-whiteboard-copy-prose>{node.title}</strong>}
      {children(node.children)}
    </blockquote>
  );
}
...
function ImageBlock({ node, data }: BlockProps<"image">) {
  return (
    <figure className="whiteboard-image">
