...
export interface IProductConfiguration {
	...
	/**
	 * Review's own release number, the one the update feed publishes and the one
	 * a user downloads. `version` above is the Code OSS base version, which the
	 * curated extensions match their `engines.vscode` range against, so the two
	 * cannot be the same field. Kept in sync with `apps/whiteboard-desktop/package.json`.
	 */
