...
import { runLegacyReviewCli, serveLegacyReviewMcp } from "./legacy-rename.js";
...
it("answers cached MCP mutations with a protocol error and reconnect instructions", async () => {
  ...
  const server = await serveLegacyReviewMcp(stdin, stdout);
  ...
  try {
    ...
    const reply = await request(2, "tools/call", {
      name: "review_delete",
      arguments: { reviewId: "saved-session" },
    });
