...
const packagedRoot = path.resolve(process.argv[2] ?? "apps/whiteboard-desktop/VSCode-linux-x64");

const { applicationName } = JSON.parse(
  await readFile(path.join(packagedRoot, "resources", "app", "product.json"), "utf8"),
);

const output = path.resolve(process.argv[3] ?? "apps/whiteboard-desktop/dist/linux-ui");
...
for (const [controls, theme, scale] of [["native", "Whiteboard Dark", 1], ["custom", "Whiteboard Light", 1.25]]) {
  ...
  const env = { ...process.env, DEV_WHITEBOARD_HOME: path.join(profile, "whiteboard-home"), DEV_WHITEBOARD_IMPORT_FROM: "none" };
  ...
  try {
    app = await electron.launch({
      ...
      chromiumSandbox: process.env.WHITEBOARD_TEST_DISABLE_SANDBOX !== "1",
      timeout: 90_000,
    });
    ...
    await page.locator(".whiteboard-onboarding-headline").waitFor({ state: "visible", timeout: 90_000 });
    ...
    await page.waitForFunction(() => document.querySelector(".whiteboard-application-menu")?.getAttribute("aria-expanded") === "false");
    await menu.click();
    ...
    await page.waitForFunction(() => {
      const titlebar = document.querySelector(".whiteboard-titlebar-container");
      ...
      const bounds = await page.locator(".whiteboard-titlebar-container").evaluate(element => ({
...
async function dismissStartupInvitations(page, menu) {
  ...
  await page.getByRole("button", { name: "Not now", exact: true })
    .or(page.locator(".whiteboard-onboarding-headline")).first()
