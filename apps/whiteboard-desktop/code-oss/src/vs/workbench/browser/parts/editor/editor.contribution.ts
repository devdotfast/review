...
import {
	...
	IsSessionsWindowContext, IsWhiteboardWindowContext, ActiveCustomEditorDiffCanToggleLayoutContext, ActiveCustomEditorTextDiffContext, EditorPartModalContext
...
MenuRegistry.appendMenuItem(MenuId.EditorTitleContext, { command: { id: UNPIN_EDITOR_COMMAND_ID, title: localize('unpin', "Unpin") }, group: '3_preview', order: 20, when: ContextKeyExpr.and(ActiveEditorStickyContext, ActiveEditorCannotCloseContext.toNegated()) });
...
MenuRegistry.appendMenuItem(MenuId.EditorTitle, {
	command: {
		...
		toggled: {
			condition: ContextKeyExpr.equals('config.diffEditor.renderSideBySide', false),
...
// menu (see below) rather than being shown as a primary toolbar icon. Review has
// no split editor workflow at all, so it drops the icon without an overflow
// replacement.
appendEditorToolItem(
	...
	ContextKeyExpr.and(SplitEditorsVertically.negate(), IsSessionsWindowContext.toNegated(), IsWhiteboardWindowContext.toNegated()),
	SPLIT_ORDER,
	{
...
appendEditorToolItem(
	...
	ContextKeyExpr.and(SplitEditorsVertically, IsSessionsWindowContext.toNegated(), IsWhiteboardWindowContext.toNegated()),
	SPLIT_ORDER,
	{
