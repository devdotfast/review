...
Use this example only when it fits the change. A step takes exactly one of `explanation`, illustrative `code:{language,text}`, or a verified `source:{side,file,fromLine,toLine}`. A `source` on a step, frame or operation is shown as a code peek, so it must cover visible code; a whitespace-only range is rejected. Read the tool schema for the other components.
