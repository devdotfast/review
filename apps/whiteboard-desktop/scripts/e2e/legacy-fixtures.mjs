...
import { pickWhiteboard, sourcePackage, workspace } from "./harness.mjs";
...
export const openLegacyWhiteboard = (ctx, fixture) =>
  pickWhiteboard(ctx, fixture.metadata.sourceUuid, fixture.worktreePath);
...
export const waitForImport = (ctx, sessionId) =>
  ctx.until(async () => {
    ...
    const snapshot = await ctx.api(`/sessions-api/${sessionId}?full=true`);
