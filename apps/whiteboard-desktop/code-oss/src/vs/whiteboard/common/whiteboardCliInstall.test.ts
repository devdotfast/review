...
import { whiteboardCliInstallResyncRequest } from './whiteboardCliInstall.js';

const baseStatus: WhiteboardCliInstallStatus = {
	agents: [],
	fingerprint: 'current',
	stamp: null,
	stale: true,
...
test('skips resync when neither skills nor the CLI are managed', () => {
	assert.equal(whiteboardCliInstallResyncRequest(baseStatus), undefined);
});
