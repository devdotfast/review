...
export async function materializePublishRevision(
  ...
  const isComplete = async () => {
    try {
      ...
      const record = await readPresentedWhiteboardRecord(destinationPath);
  ...
  try {
    await (dependencies.materialize ?? materializeWhiteboardRevision)(
...
export async function readPresentedWhiteboardRecord(documentBuildDir: string) {
  return parseAnyStoredWhiteboardRecord(
    parseJsonText(
...
export async function whiteboardWithPresentedDocumentPins(
  stored: StoredWhiteboard,
  documentBuildDir: string,
  presentedRecord?: Awaited<ReturnType<typeof readPresentedWhiteboardRecord>>,
): Promise<StoredWhiteboard> {
  const presented =
    presentedRecord ?? (await readPresentedWhiteboardRecord(documentBuildDir));

  return {
    ...stored,
    review: {
      ...stored.review,
      ...whiteboardSourcePins(presented),
    },
  };
}
