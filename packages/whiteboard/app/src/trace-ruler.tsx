...
export function TraceRuler({
  ...
  events: readonly WhiteboardAgentTraceEvent[];
}) {
  ...
  const tickFromPointer = useCallback((clientY: number) => {
    ...
    const rects = [
      ...rail.querySelectorAll<HTMLElement>(".whiteboard-trace-ruler-tick"),
  ...
  for (let tick = 0; tick < tickCount; tick += 1) {
    ...
    const className = [
      "whiteboard-trace-ruler-tick",
      // While the comb is active only the hover treatment shows; the
      // viewport run returns when the pointer leaves.
      isVisible && hoverTick === null
        ? "whiteboard-trace-ruler-tick--visible"
        : "",
      tick === hoverTick ? "whiteboard-trace-ruler-tick--hovered" : "",
    ]
      .filter(Boolean)
      .join(" ");
  ...
  return (
    <div ref={anchorRef} className="whiteboard-trace-ruler" aria-hidden="true">
      <div
        ref={railRef}
        className="whiteboard-trace-ruler-rail"
        style={{
          height: railHeight,
          top: rect?.top ?? 0,
          left: (rect?.left ?? 0) + 6,
      ...
      >
        ...
        {preview && hoverTick !== null && (
          <div className="whiteboard-trace-ruler-card" style={{ top: cardTop }}>
            <span className="whiteboard-trace-ruler-card-title">
              {preview.title}
            </span>
            {preview.snippet && (
              <span className="whiteboard-trace-ruler-card-snippet">
                {preview.snippet}
              </span>
            )}
          </div>
