...
import { stageDiffrBinary } from "../../../apps/whiteboard-desktop/scripts/stage-whiteboard-runtime.mjs";
import {
  cleanupTempDirs,
  gitRepository,
  tempDir,
} from "../src/whiteboard-test-utils.ts";
...
describe("Relocated runtime diffr integrates with Review streams and settings", () => {
  ...
  beforeAll(async () => {
    ...
    delete process.env.WHITEBOARD_DIFFR_BINARY;
  });
  ...
  test("an explicit executable override takes precedence over the bundle", async () => {
    ...
    process.env.WHITEBOARD_DIFFR_BINARY = override;
...
`,
    ...
    test("an explicit host override runs real diffs and settings", async () => {
      process.env.WHITEBOARD_DIFFR_BINARY = launcher;
      successfulFiles(await collect(repository, base, head), 3);
