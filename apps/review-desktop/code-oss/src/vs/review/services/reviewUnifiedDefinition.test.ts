...
test("unified definitions delegate base and head rows to their pinned models", async () => {
	...
	const { provideReviewUnifiedDefinition } = await import("./reviewUnifiedDefinition.js");
	...
	for (const sample of [
		...
		const expectedResource =
			sample.side === "base"
				? URI.file("/tmp/review-base/src/example.ts")
				: URI.file("/tmp/review-head/src/example.ts");
		...
		const resources = {
			unifiedResource: (resource: URI) =>
				resource.toString() === unifiedResource.toString()
					? {
							original: expectedResource,
							modified: expectedResource,
					...
					return [
						{
							...
							originSelectionRange: new Range(sample.sourceLine, 7, sample.sourceLine, 15),
						},
					];
				}
		...
		assert.equal(definitions[0]?.uri.toString(), URI.file("/tmp/definition.ts").toString());
		assert.deepEqual(definitions[0]?.originSelectionRange, new Range(sample.unifiedLine, 7, sample.unifiedLine, 15));
...
test("unified hovers delegate base and head rows to their pinned models", async () => {
	...
	const { provideReviewUnifiedHover } = await import("./reviewUnifiedDefinition.js");
	const unifiedResource = URI.from({
		scheme: "devfast-review-unified",
		path: "/src/example.ts",
	});
	...
	for (const sample of [
		...
		const hover = await provideReviewUnifiedHover(
			{
				unifiedResource: () => ({
					original: sample.resource,
					modified: sample.resource,
					targetForRange: () => ({
						path: "src/example.ts",
						side: sample.side,
						startLine: sample.sourceLine,
		...
		assert.deepEqual(hover?.range, new Range(sample.unifiedLine, 7, sample.unifiedLine, 15));
		assert.equal(disposed, true);
	}
});
...
test("unified definition model references are disposed when delegation fails", async () => {
	...
	await assert.rejects(
		provideReviewUnifiedDefinition(
			{
				unifiedResource: () => ({
					original: URI.file("/tmp/base.ts"),
					modified: URI.file("/tmp/head.ts"),
					targetForRange: () => ({
						path: "src/example.ts",
						side: "base",
						startLine: 17,
