...
export class ReviewCanvasEditorPane extends EditorPane {
  ...
  constructor(
    ...
    this.refreshProgress = this._register(
    ...
    this._register(
      desktopConnection.onDidFail((error) => void this.renderFailure(error)),
    );
    this._register(
    ...
    this.desktopConnection.attachControl(async (value) => {
      ...
      if (request.name === "openApiReview") {
        ...
        return response.ok
          ? {
              ok: true,
              result: { softwareMapEnabled: this.currentSoftwareMapEnabled() },
            }
          : response;
      }
  ...
  private async setReviewInput(
    ...
    if (input.target.kind === "api") {
      try {
        ...
        const [connection, assets] = await Promise.all([
          this.desktopConnection.getConnection(),
          this.loadAssets(),
        ]);
        if (generation !== this.loadGeneration || token.isCancellationRequested)
          return;
        ...
        void this.apiCatalog
          ...
          .catch((error) =>
            this.logService.warn(
              "[Review] Could not mark review viewed:",
              error,
            ),
          );
        let sourceSelection: ReviewSourceSelection = {
          reviewId,
          kind: "current",
        };
        let sourceView: ReviewSourceView = resolveReviewSourceView({
          reviewId,
          version: 0,
          pins: {},
        });
        const source = this.apiSource.canvas(
          () => sourceView,
          this.inlineEditors,
          this.diffViews,
        );
            ...
            bridge: {
              ...
              post: async (request) => {
                if (request.name === "openSourceTree") {
                  await this.tabsService.openApiSource(
                    sourceSelection,
                    input.getName(),
                  );
    ...
    if (input.target.kind === "home") {
      ...
      const renderHome = async () => {
        ...
        const openReview = (uuid: string) => {
          ...
          const api = this.apiCatalog.reviews.find(
            (review) => review.reviewId === uuid,
          );
          return api
            ? this.tabsService.openApiReview(uuid, api.title)
            : Promise.resolve();
        };
        return this.render(
          {
            ...
            openSourceTree: (uuid) => {
              ...
              if (api) {
                void this.tabsService
                  .openApiSource(
                    { reviewId: api.reviewId, kind: "current" },
                    api.title,
                  )
      ...
      subscriptions.add(
        this.desktopConnection.onDidChangeLists(() => void renderHome()),
      );
    ...
    if (input.target.kind === "welcome") {
      ...
      this.modelSubscription.value = this.desktopConnection.onDidChangeLists(
        () => void renderWelcome(),
      );
      await renderWelcome();
      return;
  ...
  getSelection(): IEditorPaneSelection | undefined {
    ...
    return new ReviewEmbeddedEditorSelection(editor, {
      ...
      side:
        new URLSearchParams(model.uri.query).get("side") === "base"
          ? "base"
          : "head",
...
export const IReviewCanvasPartsService =
  createDecorator<IReviewCanvasPartsService>("reviewCanvasPartsService");

export interface IReviewCanvasPartsService {
  readonly _serviceBrand: undefined;
}

export class ReviewCanvasParts
  extends Disposable
  implements IReviewCanvasPartsService
{
