...
import { isCallableValue, isObjectValue } from "@dev.fast/whiteboard-protocol";
import { z } from "zod";
...
const ELEMENT_MARKER = "__whiteboardPublishElement";
...
const authoringComponentNameSchema = z.keyof(
  z.object(whiteboardAuthoringPropsSchemas),
);
...
export interface WhiteboardDocumentPublishAudit {
...
export function auditWhiteboardDocumentComponent(input: {
...
}): WhiteboardDocumentPublishAudit | null {
...
function auditElement(
  ...
  if (name === "DatabaseLens") {
    ...
    const parsed = whiteboardAuthoringPropsSchemas[name].safeParse(
      element.props,
    );
