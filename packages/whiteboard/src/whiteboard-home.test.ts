...
import { resolvePublishWhiteboard } from "./publish-preparation";
import {
...
import {
  ...
  WhiteboardHomeScanError,
  computeSync,
  createWhiteboardDir,
  findWhiteboard,
  findWhiteboardForRepair,
  listWhiteboards,
  ...
  touchWhiteboardAgentSession,
  whiteboardDescriptor,
...
describe("review home", () => {
  it("accepts every stored schema version and rejects unknown or extended records", async () => {
    ...
    ).toMatchObject({ schemaVersion: WHITEBOARD_SCHEMA_VERSION });
    expect(
      parseAnyStoredWhiteboardRecord({
        ...base,
        schemaVersion: WHITEBOARD_SCHEMA_VERSION,
  ...
  it("upserts agent roles and timestamps without changing the legacy field", async () => {
    ...
    const first = await touchWhiteboardAgentSession(
      created,
      "codex:creator",
      "publisher",
    ...
    const second = await touchWhiteboardAgentSession(
      first,
      "codex:creator",
      "publisher",
    ...
    expect(parseStoredWhiteboardRecord(legacy)).not.toHaveProperty(
      "agentSessions",
    );
  });
  ...
  it("ignores a legacy softwareMap key in review.json", async () => {
    ...
    const loaded = await findWhiteboard(created.review.uuid);
  ...
  it("describes pull request and diff metadata for the home view", async () => {
    ...
    await expect(whiteboardDescriptor(created)).resolves.toMatchObject({
      baseRef: baseCommit,
  ...
  it("describes the pinned diff even when the checkout moved elsewhere", async () => {
    ...
    const whiteboardHead = await git(root, ["rev-parse", "HEAD"]);
    ...
    const samePin = await createWhiteboardDir({
      worktreePath: root,
      baseRef: baseCommit,
      baseCommit,
    ...
    await expect(whiteboardDescriptor(samePin)).resolves.toMatchObject({
      diffStats: null,
      commits: [],
    });
  });
  ...
  it("ignores active and stale locks and unrelated directories during discovery", async () => {
    ...
    const staleWhiteboardDir = path.join(
      reviewsHomeDir(),
      "11111111-1111-4111-8111-111111111111",
    );

    const staleLock = `${staleWhiteboardDir}.mutation-lock`;
    ...
    await expect(
      withWhiteboardMutationLock(staleWhiteboardDir, async () => "reclaimed"),
  ...
  it("distinguishes invalid UUIDs, missing reviews, and malformed records", async () => {
    ...
    await expect(findWhiteboard(malformedUuid)).rejects.toBeInstanceOf(
      WhiteboardHomeScanError,
    );
  });
  ...
  it("filters scanned reviews and computes source-head sync", async () => {
    ...
    await expect(
      listWhiteboards({ worktreePath: root }),
  ...
  it("keeps system Whiteboards addressable but out of ordinary lists", async () => {
    const root = await gitRepository();
    await whiteboardHome();
    ...
    await expect(listWhiteboards()).resolves.toMatchObject({
      reviews: [],
      errors: [],
    });
    await expect(
      listWhiteboards({ includeSystem: true }),
    ).resolves.toMatchObject({
      reviews: [
        {
          dir: review.dir,
    ...
    await expect(findWhiteboard(review.review.uuid)).resolves.toMatchObject({
      dir: review.dir,
...
describe("legacy records on read", () => {
  it("reads repair metadata without migration and preserves lookup validation", async () => {
    ...
    const loaded = await findWhiteboardForRepair(created.review.uuid);
    ...
    for (const value of [
      ...
      await expect(
        findWhiteboardForRepair(created.review.uuid),
      ).rejects.toBeInstanceOf(WhiteboardHomeScanError);
  ...
  it.each([2, 3, 4, 6] as const)(
    ...
    async (schemaVersion) => {
      ...
      const recordPath = path.join(created.dir, "review.json");
      await sealWhiteboardCandidate(created.dir, "Initial document");
      ...
      await expect(findWhiteboard(created.review.uuid)).rejects.toBeInstanceOf(
        WhiteboardHomeScanError,
      );
      expect(await readFile(recordPath, "utf8")).toBe(bytes);
  ...
  it.each([2, 3, 4] as const)(
    ...
    async (schemaVersion) => {
      const root = await gitRepository();
      const home = await whiteboardHome();
      ...
      const stored = await findWhiteboard(created.review.uuid);
      ...
      expect(bundle && whiteboardDocumentBundleData(bundle).title).toBe(
        "Sealed",
      );
      expect(await whiteboardDescriptor(stored!)).toMatchObject({
        available: true,
        status: "accepted",
      });

      const bytes = await readFile(recordPath, "utf8");
      await listWhiteboards();
      expect(await readFile(recordPath, "utf8")).toBe(bytes);
    },
  );
  ...
  it("migrates once when two readers race", async () => {
    ...
    const [first, second] = await Promise.all([
      findWhiteboard(created.review.uuid),
      findWhiteboard(created.review.uuid),
    ]);
