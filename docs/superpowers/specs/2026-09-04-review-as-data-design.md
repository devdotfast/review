...
| D2 | No dual loader. Reviews sealed as JS bundles are regenerated by `review migrate apply`, the existing store-migration command; a review the migration cannot regenerate shows a republish state. | The migration already rebuilds document bundles from sealed sources, and the home screen already shows a copyable migrate prompt for out-of-date stores. |
...
| D9 | Publish validates every code peek against the pinned worktree, then strips resolutions from the JSON; the app resolves lazily on render. | Peek references stay validatable at publish time. The `CodePeek` component already resolves a ref whose resolution is null, and the server produces resolutions on demand. |
| D10 | Attribute expressions run through `new Function`. | One line. `data.ts` runs arbitrary code in the same process anyway, and the plain-JSON authoring format removes both. The call site carries a comment saying so. |
