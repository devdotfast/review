...
export function whiteboardAppTelemetryHeaders(session: WhiteboardSession) {
  return { [WHITEBOARD_APP_SESSION_ID_HEADER]: session.appSessionId };
}

export function captureAppOpened(session: WhiteboardSession): void {
...
export function captureUiEvent(
  ...
  try {
    void whiteboardFetch("/telemetry/event", {
      method: "POST",
