...
export interface WhiteboardBranchLinks {
  baseUrl: string | null;
  headUrl: string | null;
}
 ...
 */
export async function resolveWhiteboardBranchLinks(
  input: {
    rootPath: string;
    baseRef: string;
    headRef: string;
  ...
  const usedRemotes = new Set(
    [candidates.baseUrl, candidates.headUrl].flatMap((value) =>
      value ? [value.remote] : [],
    ),
  );
...
function emptyBranchLinks(): WhiteboardBranchLinks {
