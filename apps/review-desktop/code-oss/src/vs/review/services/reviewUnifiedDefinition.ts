...
import type { DefinitionProvider, Hover, HoverProvider, LocationLink } from "../../editor/common/languages.js";
...
type UnifiedLanguageFeatureResources = Pick<IReviewCodeResourceService, "unifiedResource">;
type UnifiedLanguageFeatureModelService = Pick<ITextModelService, "createModelReference">;
type UnifiedLanguageFeatureExtensionService = Pick<IExtensionService, "activateByEvent">;
...
export async function provideReviewUnifiedDefinition(
	...
	return withReviewUnifiedSourcePosition(
		...
		async ({ sourceModel, sourcePosition, unifiedLine }) => {
			const definitions = await resolveDefinitions(definitionProviders, sourceModel, sourcePosition, false, token);
...
export async function provideReviewUnifiedHover(
	...
	return withReviewUnifiedSourcePosition(
		...
		undefined,
		async ({ sourceModel, sourcePosition, unifiedLine }) => {
			const hovers = await resolveHovers(hoverProviders, sourceModel, sourcePosition, token, false);
			...
			return {
				...
				range: first.range
					? model.validateRange(new Range(unifiedLine, first.range.startColumn, unifiedLine, first.range.endColumn))
					: undefined,
			};
		},
	);
...
async function withReviewUnifiedSourcePosition<T>(
	...
	const sourceReference = await textModelService.createModelReference(
		mapped.side === "base" ? unified!.original : unified!.modified,
	);
	try {
		const sourceModel = sourceReference.object.textEditorModel;
		if (!sourceModel) return fallback;
		const sourcePosition = sourceModel.validatePosition(new Position(mapped.startLine, position.column));
		await extensionService.activateByEvent(`onLanguage:${sourceModel.getLanguageId()}`);
		return provide({
			sourceModel,
			sourcePosition,
...
async function followSameFileDefinition(
	...
	const targetRange = Range.lift(definition.targetSelectionRange ?? definition.range);
	...
	const forwarded = definitions.filter(
		(candidate) =>
			...
			!Range.equalsRange(candidate.targetSelectionRange ?? candidate.range, targetRange),
	);
