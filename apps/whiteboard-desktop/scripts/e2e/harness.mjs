...
import { assertRuntimeContents } from "../stage-whiteboard-runtime.mjs";
...
export const sourcePackage = path.join(workspace, "packages/whiteboard");
  ...
  const apiCanvasFor = (title) =>
    until(async () => {
      for (const candidate of browser
        ...
        if (
          (await candidate
            .locator(".whiteboard-canvas-root [data-whiteboard-api]")
            .count()
            .catch(() => 0)) > 0 &&
          (await candidate
...
export async function createWhiteboard(ctx, spec) {
  ...
  return {
    ...
    repositoryId: repository.value.id,
    title: spec.title,
    canvas: page.locator(".whiteboard-canvas-root [data-whiteboard-api]"),
  };
}

/** The standard two-block order review used by several journeys. */
export const orderWhiteboardBlocks = [
  {
    type: "section",
    title: "Overview",
    children: [
...
export async function pickWhiteboard(ctx, sessionId, cwd = ctx.repo) {
...
export async function openSettings(ctx) {
  const settings = ctx.page.locator(
    ".whiteboard-home-content.whiteboard-settings-page",
  );
...
export async function installExtensionGroup(
  ...
  await settings
    .locator(".whiteboard-settings-row")
...
export async function openHome(ctx) {
  ...
  await ctx.page.locator("main.whiteboard-home").waitFor({ timeout: 60000 });
}
