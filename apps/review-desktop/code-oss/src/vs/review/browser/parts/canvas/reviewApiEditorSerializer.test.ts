...
test("native group restoration preserves both reviews, order and pinned source versions without duplicate tabs", async (t) => {
  ...
  const instantiation = {
    createInstance(ctor: any, ...args: any[]) {
      ...
      const input = new ReviewCanvasEditorInput(args[0], {} as never, {} as never, {} as never);
      inputs.push(input);
      return input;
    },
    invokeFunction(fn: any) { return fn({ get: () => tabs }); },
  };
  ...
  const createTabs = () => new ReviewCanvasEditorTabsService(instantiation as never, editors as never, groupService as never, {} as never);
  tabs = createTabs();
  ...
  const group = (state?: ReturnType<EditorGroupModel["serialize"]>) => new EditorGroupModel(state, instantiation as never, config as never);
  const left = group(), right = group();
  t.after(() => {
    registration.dispose(); tabs.dispose(); left.dispose(); right.dispose();
    groups.forEach(group => group.dispose()); inputs.forEach(input => input.dispose());
  });
  ...
  left.openEditor(tabs.inputFor({ kind: "api-source", reviewId: "a", title: "Review A", version: 7 }), { pinned: true, active: false });
  ...
  tabs.dispose(); tabs = createTabs();
  groups.push(...saved.map(state => group(state)));
  groupService.mainPart.activeGroup = groups[1];
  assert.deepEqual(groups.map(group => group.getEditors(EditorsOrder.SEQUENTIAL).map(editor => editor.getName())), [
...
test("invalid saved tabs are ignored instead of preventing the window from restoring", () => {
  ...
  for (const value of ["invalid JSON", "null", '{"kind":"api"}', '{"kind":"api-source","reviewId":"a","title":"A","version":-1}']) {
