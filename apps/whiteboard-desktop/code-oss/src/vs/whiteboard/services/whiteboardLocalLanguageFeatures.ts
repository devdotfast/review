...
import { whiteboardSourceQuery, type WhiteboardLanguageEnvironment } from "../common/whiteboardProtocol.js";
import { sourceLocation } from "../common/whiteboardSourceView.js";
import { WHITEBOARD_LANGUAGE_SOURCE_SCHEME } from "../common/whiteboardReadonlySource.js";
import { WHITEBOARD_API_SOURCE_SCHEME } from "./whiteboardApiSourceService.js";
...
import { acquireWhiteboardLanguageRoot } from "./whiteboardLocalWorkspace.js";
import { WhiteboardLanguageEnvironmentRequests } from "./whiteboardLanguageEnvironmentRequests.js";
...
export class WhiteboardLocalLanguageFeatures extends Disposable {
	...
	private readonly environments = new WhiteboardLanguageEnvironmentRequests();
	private readonly roots = new Map<string, number>();
	private readonly uncertainRoots = new Set<string>();
	private generation = 0;

	constructor(
		@IWhiteboardDesktopConnectionService private readonly connection: IWhiteboardDesktopConnectionService,
		...
		const warm = (model: ITextModel) => {
			if ([WHITEBOARD_API_SOURCE_SCHEME, WHITEBOARD_LANGUAGE_SOURCE_SCHEME].includes(model.uri.scheme)) void this.localSource(model);
		};
		this._register(modelService.onModelAdded(warm));
		modelService.getModels().forEach(warm);
		for (const scheme of [WHITEBOARD_API_SOURCE_SCHEME, WHITEBOARD_LANGUAGE_SOURCE_SCHEME]) {
		...
		for (const scheme of [WHITEBOARD_API_SOURCE_SCHEME, WHITEBOARD_LANGUAGE_SOURCE_SCHEME]) {
			...
			this._register(languages.referenceProvider.register(target, {
				provideReferences: (model, position, context, token) => this.withSource(model, position, token, async (local, at, pinned) => {
					...
					return this.whiteboardLocations(pinned, results.flatMap(result => result ?? []), token);
				}),
			}));
		}
	}

	private async environment(model: ITextModel, validate = false): Promise<WhiteboardLanguageEnvironment | undefined> {
	...
	private async acquire(model: ITextModel, context: { rootPath: string; identity: string }): Promise<LocalSource | undefined> {
		...
		const resource = model.uri.scheme === WHITEBOARD_LANGUAGE_SOURCE_SCHEME ? URI.file(model.uri.path) : URI.joinPath(root, relative);
		if (!await this.files.exists(resource) || model.isDisposed()) return undefined;
		const owned = new DisposableStore();
		try {
			owned.add(await acquireWhiteboardLanguageRoot(this.workspace, root));
	...
	private async locations(model: ITextModel, position: Position, token: CancellationToken, kind: "definition" | "type" | "implementation"): Promise<LocationLink[] | undefined> {
		return this.withSource(model, position, token, async (local, at, pinned) => {
			...
			return this.whiteboardLocations(pinned, results, token);
		});
	}

	/** Keep navigation in the same saved version/side only when destination contents match. */
	private async whiteboardLocations<T extends LocationLink>(pinned: ITextModel, locations: T[], token: CancellationToken): Promise<T[]> {
		if (![WHITEBOARD_API_SOURCE_SCHEME, WHITEBOARD_LANGUAGE_SOURCE_SCHEME].includes(pinned.uri.scheme)) return locations;
		...
		await Promise.all([...groups.values()].map(async group => {
			...
			try {
				...
				if (target.authority === source.root.authority && target.path.startsWith(prefix)) {
					const candidate = pinned.uri.with({ scheme: WHITEBOARD_API_SOURCE_SCHEME, path: "/" + target.path.slice(prefix.length) });
				...
				if (!original?.equalsTextBuffer(local.getTextBuffer())) {
					const candidate = pinned.uri.with({ scheme: WHITEBOARD_LANGUAGE_SOURCE_SCHEME, path: target.path });
...
registerWorkbenchContribution2(WhiteboardLocalLanguageFeatures.ID, WhiteboardLocalLanguageFeatures, WorkbenchPhase.BlockRestore);
