...
// `whiteboardConfiguration.js`: this runs in the main process before bootstrapESM(),
// so reaching the configuration registry behind it would throw
// `!!! NLS MISSING: <n> !!!` in a packaged build. See whiteboardConfigurationDefaults.ts.
import { curatedExtensionConfigurationDefaults, whiteboardConfigurationDefaults, type WhiteboardKeymap } from '../common/whiteboardConfigurationDefaults.js';

export const WHITEBOARD_USER_CONFIG_IMPORT_VERSION = 1;

export type { WhiteboardKeymap };
export type WhiteboardUserConfigImportMode = 'startup' | 'preview' | 'apply';

export interface WhiteboardUserConfigImportOptions {
	readonly userDataPath: string;
	readonly mode?: WhiteboardUserConfigImportMode;
...
export interface WhiteboardUserConfigImportResult {
	readonly status: 'disabled' | 'not-found' | 'skipped' | 'ready' | 'imported';
	readonly source?: string;
	readonly keymap: WhiteboardKeymap;
...
const BLOCKED_SETTING_KEYS = new Set<string>([
	...Object.keys(whiteboardConfigurationDefaults),
...
const KEYMAP_EXTENSIONS: Readonly<Record<Exclude<WhiteboardKeymap, 'none'>, string>> = {
	vim: 'vscodevim.vim',
...
function discoverSource(options: WhiteboardUserConfigImportOptions): SourceCandidate | 'disabled' | undefined {
...
function detectKeymap(extensionsDir: string | undefined): WhiteboardKeymap {
	if (!extensionsDir || !existsSync(extensionsDir)) {
		return 'none';
	}

	const metadataIds = extensionIdsFromMetadata(extensionsDir);
	const candidates: { keymap: Exclude<WhiteboardKeymap, 'none'>; modifiedAt: number }[] = [];
	for (const entry of readdirSync(extensionsDir, { withFileTypes: true })) {
		if (!entry.isDirectory()) {
			continue;
		}
		const normalizedName = entry.name.toLowerCase();
		for (const [keymap, extensionId] of Object.entries(KEYMAP_EXTENSIONS) as [Exclude<WhiteboardKeymap, 'none'>, string][]) {
...
function filterWhiteboardUserSettings(settings: Record<string, unknown>): Record<string, unknown> {
...
function readSettings(settingsPath: string): Record<string, unknown> | undefined {
	...
	return filterWhiteboardUserSettings(value as Record<string, unknown>);
}
...
function writeImport(prepared: PreparedImport, userDir: string, now: () => Date): string[] {
	...
	writeFileSync(path.join(userDir, '.review-import.json'), `${JSON.stringify({
		version: WHITEBOARD_USER_CONFIG_IMPORT_VERSION,
...
export function importWhiteboardUserConfig(options: WhiteboardUserConfigImportOptions): WhiteboardUserConfigImportResult {
