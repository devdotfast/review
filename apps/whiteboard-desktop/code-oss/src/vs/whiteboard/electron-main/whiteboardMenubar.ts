 ...
 */
export class WhiteboardMenubarMainService
  extends Disposable
  implements IMenubarMainService
{
  ...
  private install(): void {
    ...
    this.logService.info(
      `whiteboardMenubar#install - menus [${labelsOf(menubar)}], application menu [${labelsOf(applicationMenu)}]`,
    );
  }
  ...
  private runActionInFocusedWindow(commandId: string): void {
    ...
    if (!window) {
      this.logService.info(
        `whiteboardMenubar#runActionInFocusedWindow - no window for ${commandId}`,
      );
      return;
    }
              ...
              setTimeout(() => {
                this.updateService.checkForUpdates(true).catch((error) => {
                  this.logService.error(
                    "whiteboardMenubar#checkForUpdates failed",
                    error,
                  );
                });
              }, 0),
          ...
          new MenuItem({
            ...
            click: () => {
              this.updateService.quitAndInstall().catch((error) => {
                this.logService.error(
                  "whiteboardMenubar#quitAndInstall failed",
                  error,
                );
              });
            },
