import type { WhiteboardCommitSummary } from "@dev.fast/whiteboard-protocol";
...
import type { LocalSessionData } from "../../src/session-api/local-data";
import type { Snapshot } from "../../src/session-api/store";
import type { NormalizedSoftwareModel } from "../../src/software-map-model";
import type { DocumentPeekableAnchor } from "../../src/whiteboard-document-data";
...
import { WhiteboardDocumentTitle } from "./whiteboard-document-surface";
import {
  cssIdentifier,
        ...
        commits:
          snapshot.sourceUnavailable || !snapshot.pins
            ...
            : await once(`commits:${JSON.stringify(snapshot.pins)}`, () =>
                client.read<WhiteboardCommitSummary[]>(
...
export function ApiDocument({
  ...
  return (
    <>
      {!hasTitle && !scratchpad && (
        <WhiteboardDocumentTitle>{data.snapshot.title}</WhiteboardDocumentTitle>
      )}
      {(data.snapshot.target?.kind === "worktree" ||
        data.snapshot.sourceUnavailable) && (
        <p className="whiteboard-source-context">
          {data.snapshot.sourceUnavailable
...
function useHeadingFragments(): void {
  ...
  useEffect(() => {
    ...
    const onClick = (event: MouseEvent) => {
      ...
      scrollToWhiteboardHeading(id, article, roots.scrollRegionRef.current);
    };
...
export const DocumentNode = memo(function DocumentNode({
  ...
  return (
    <div
      ...
      data-whiteboard-node-id={node.id}
      data-motion={motion}
      data-whiteboard-copy-prose={
        node.type === "markdown" || node.type === "trace_quote" || undefined
      }
    >
      <BlockErrorBoundary
        type={block.type}
        onError={(error) => reportWhiteboardDocumentRenderError(session, error)}
      >
        {stale ? (
          <p role="status">
