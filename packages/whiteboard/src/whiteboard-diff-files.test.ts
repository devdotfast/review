...
import {
  ...
  resolveWhiteboardFileContent,
} from "./whiteboard-diff-files";

describe("resolveWhiteboardDiffFiles", () => {
  test("falls back to jj for unexported jj commits", async () => {
    ...
    try {
      ...
      const result = await resolveWhiteboardDiffFiles({
        rootPath,
        baseRef,
        headRef,
      });
  ...
  test("can request enough context for declaration-shaped CodePeek patches", async () => {
    ...
    try {
      ...
      const narrow = await resolveWhiteboardDiffFiles({
        rootPath,
        baseRef,
        headRef,
      ...
      const wide = await resolveWhiteboardDiffFiles({
        rootPath,
        baseRef,
        headRef,
        paths: ["src/example.ts"],
  ...
  test("reuses a precomputed comparison when reading file content", async () => {
    ...
    try {
      const comparison = await resolveWhiteboardDiffFiles({
