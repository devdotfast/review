...
import type { WhiteboardStructuralDiffEvent } from "@dev.fast/whiteboard-protocol";
...
import { resolveWhiteboardBranchLinks } from "../whiteboard-branch-links.js";
import { resolveWhiteboardStackLayers } from "../whiteboard-stack.js";
...
export function createSessionApi(
  ...
  const sharedGuard: MiddlewareHandler = async (context, next) => {
    ...
    readWhiteboard(id, query.version);
  ...
  const readWhiteboard = (id: string, version?: number): Snapshot => {
  ...
  app.get("/:id/progress", async (context) => {
    ...
    const snapshot = readWhiteboard(context.req.param("id"), query.version);
  ...
  app.post("/:id/progress", async (context) => {
    ...
    const progress = await whiteboardProgress(
      store,
      data,
      snapshot,
      context.req.raw.signal,
  ...
  app.get("/:id/lenses", async (context) => {
    ...
    const snapshot = readWhiteboard(context.req.param("id"));

    return context.json({
      version: snapshot.version,
      ...lensReport(
        snapshot.lenses ?? [],
        await whiteboardProgress(store, data, snapshot, context.req.raw.signal),
      ),
    });
  });
  ...
  const openWhiteboard = async (whiteboard: Snapshot) => {
    if (!open)
    ...
    try {
      if (whiteboard.target?.kind === "commits" && whiteboard.pins)
        void data?.workspaces
          .open(whiteboard.sessionId, whiteboard.pins)
          .catch(() => {});
      environmentIssues = data?.currentEnvironmentIssues(whiteboard);
    } catch (error) {
      environmentIssues = [
        {
  ...
  const openCreated = async (sessionId: string) => {
    try {
      ...
      return { opened: true, ...(await openWhiteboard(store.read(sessionId))) };
    } catch (error) {
      return {
        opened: false,
  ...
  app.post("/:id/open", async (context) => {
    ...
    return context.json({
      ok: true,
      ...(await openWhiteboard(readWhiteboard(id))),
    });
  });
  ...
  if (data) {
    ...
    const tracePins = (id: string, version?: number) => {
      const { pins } = readWhiteboard(id, version);
    ...
    app.get("/:id/tree", async (context) => {
      ...
      const { pins } = await data.resolveSource(
        readWhiteboard(context.req.param("id"), input.version),
        input.commit,
        queryAnchor(input),
      );
    ...
    app.get("/:id/maps/:resourceId", async (context) => {
      ...
      return context.json(
        await data.map(
          await data.sourcePins(readWhiteboard(id, query.version)),
          z.string().parse(context.req.param("resourceId")),
        ),
      );
    });
    ...
    app.get("/:id/resources/:resourceId", async (context) => {
      const id = context.req.param("id");
      const snapshot = readWhiteboard(id);
    ...
    app.get("/:id/language-context", async (context) => {
      ...
      const snapshot = readWhiteboard(context.req.param("id"), input.version);
    ...
    app.post("/:id/environment", async (context) => {
      ...
      return context.json({
        issues: await data.environmentIssues(
          readWhiteboard(context.req.param("id")),
          input.retry,
        ),
      });
    });
    ...
    app.get("/:id/file", async (context) => {
      ...
      const { snapshot, pins } = await data.resolveSource(
        readWhiteboard(id, input.version),
        input.commit,
        anchor,
      );
    ...
    app.get("/:id/structural-diff", async (context) => {
      ...
      const stream = new ReadableStream<Uint8Array>({
        async start(controller) {
          const send = (event: WhiteboardStructuralDiffEvent) => {
  ...
  app.get("/:id/branch-links", async (context) => {
    ...
    const id = context.req.param("id");
    const snapshot = readWhiteboard(id, query.version);
    ...
    const links = await resolveWhiteboardBranchLinks({
  ...
  app.get("/:id/stack", async (context) => {
    ...
    const repoKey = (whiteboard: Pick<Snapshot, "origin" | "pins">) =>
      whiteboard.origin?.pullRequestUrl?.replace(/\/pull\/\d+.*$/, "") ??
      whiteboard.pins?.repositoryId ??
      "";

    const layers = await resolveWhiteboardStackLayers(
      {
        pullRequestUrl: snapshot.origin?.pullRequestUrl,
      },
      store.list().map((whiteboard) => ({
        uuid: whiteboard.sessionId,
        title: whiteboard.title,
        repoKey: repoKey(whiteboard),
        pullRequestNumber: whiteboard.origin?.pullRequestNumber,
        presentedDocumentRevision: String(whiteboard.version),
      })),
    );
  ...
  app.get("/:id", async (context) => {
    ...
    const snapshot = {
      ...readWhiteboard(context.req.param("id"), query.version),
    };
