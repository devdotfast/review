    ...
    if ("display" in source) {
      ...
      for (const [key, side] of [
        ...
        const file = result.file[key],
          content = result.sources.same ?? result.sources[key];
  ...
  async validateSourceTolerant(
    ...
    try {
      ...
      if (error instanceof ReviewInputError)
        return `${"display" in source ? "Diffr evidence" : `${source.side}/${source.file}#L${source.fromLine}-L${source.toLine}`}: ${error.message}`;
      throw error;
    }
  }
