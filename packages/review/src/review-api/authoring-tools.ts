...
export function authoringTools(mode: AuthoringMode = "interactive") {
  ...
  const descriptions = {
    ...
    edit: "Insert, update, move, remove or replace a component. The host assigns short durable IDs. Create an outline of section headings and short descriptions with status:pending first. Patch each section to status:in_progress before filling it, then status:complete after checking its content. Use returned IDs to fill sections in place. Section status persists independently of activity; absent status is unspecified. Accepted edits are saved immediately. Omitted placement appends; null removes an optional field in a patch. While a reader may be watching, write small and often: one paragraph, one sequence step, one flow_node or one flow_edge per edit (parentId names the diagram), so the document draws itself as you go. Wire as you go: after each new flow_node, insert the flow_edge that joins it to the board before the next node. Removing a flow_node removes its edges.",
