...
import { ReviewWorkspaceContextService } from '../services/workspace/browser/workspaceContextService.js';
import { getWorkspaceIdentifier } from '../../platform/workspaces/common/workspaceIdentifier.js';
import { ReviewDefaultAccountService } from '../services/reviewDefaultAccountService.js';
import { reviewAgentsWindowDefaultOverrides } from '../common/reviewConfigurationDefaults.js';

export class ReviewMain extends Disposable {
	...
	private registerListeners(workbench: ReviewWorkbench, storageService: NativeWorkbenchStorageService): void {
	...
	private async initServices(): Promise<{ serviceCollection: ServiceCollection; logService: ILogService; storageService: NativeWorkbenchStorageService; configurationService: ReviewConfigurationService }> {
		...
		// Default Account
		const defaultAccountService = this._register(new ReviewDefaultAccountService());
		...
		const workspaceContextService = new ReviewWorkspaceContextService(workspaceIdentifier, uriIdentityService);
	...
	private async createConfigurationService(
		workspaceContextService: ReviewWorkspaceContextService,
	...
	): Promise<ReviewConfigurationService> {
		const configurationCache = new ConfigurationCache([Schemas.file, Schemas.vscodeUserData], environmentService, fileService);
		const configurationService = new ReviewConfigurationService(userDataProfileService, workspaceContextService, uriIdentityService, fileService, policyService, logService, configurationCache, environmentService, { defaultOverrides: reviewAgentsWindowDefaultOverrides });
		try {
...
export function main(configuration: INativeWindowConfiguration): Promise<void> {
	const workbench = new ReviewMain(configuration);
