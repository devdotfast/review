...
class WhiteboardDefaultConfiguration extends DefaultConfiguration {
	constructor(
		cacheScope: string,
		configurationCache: IConfigurationCache,
		environmentService: IBrowserWorkbenchEnvironmentService,
		logService: ILogService,
		private readonly defaultOverrides: Readonly<Record<string, unknown>>,
	) {
		super(cacheScope, configurationCache, environmentService, logService);
	}

	protected override getDefaultValue(key: string, propertySchema: IRegisteredConfigurationPropertySchema): unknown {
		if (Object.hasOwn(this.defaultOverrides, key)) {
			return deepClone(this.defaultOverrides[key]);
		}
		if (propertySchema.agentsWindow) {
			return deepClone(propertySchema.agentsWindow.default);
		}
		return super.getDefaultValue(key, propertySchema);
	}

}

export class WhiteboardConfigurationService extends Disposable implements IWorkbenchConfigurationService {
	...
	constructor(
		...
		configurationCache: IConfigurationCache,
		environmentService: IBrowserWorkbenchEnvironmentService,
		options: { readonly defaultOverrides?: Readonly<Record<string, unknown>> } = {},
	) {
		super();

		this.settingsResource = userDataProfileService.currentProfile.settingsResource;
		this.defaultConfiguration = this._register(new WhiteboardDefaultConfiguration(userDataProfileService.currentProfile.id, configurationCache, environmentService, logService, options.defaultOverrides ?? {}));
...
class ConfigurationEditing {
	...
	constructor(
		private readonly fileService: IFileService,
		private readonly configurationService: WhiteboardConfigurationService,
	) { }
