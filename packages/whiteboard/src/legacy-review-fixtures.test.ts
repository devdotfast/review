...
import {
  ...
  readLegacyWhiteboardGolden,
  snapshotWhiteboardTree,
...
const fixtures = listLegacyWhiteboardFixtures();
...
async function extract(name: string) {
  const extracted = await extractLegacyWhiteboardFixture(name);
...
it("snapshots authored locks, databases, and managed metadata", async () => {
  ...
  const snapshot = await snapshotWhiteboardTree(dir);
...
describe.each(fixtures)("legacy fixture $name", (fixture) => {
  it("migrates to golden JSON while preserving metadata, stale files and old history", async () => {
    ...
    const loaded = await readStoredWhiteboard(dir);
    ...
    expect(normalizeMigratedRecord(record)).toEqual(
      await readLegacyWhiteboardGolden(fixture.name, "record"),
    );
    ...
    await materializeWhiteboardRevision(dir, documentRevision, documentDir);
    let actualMap = null;
    ...
    if (fixture.hasMap) {
      ...
      actualMap = await readWhiteboardSoftwareMapBundle(mapDir);
      expectedMap = await readLegacyWhiteboardGolden(fixture.name, "map");
    }
    ...
    for (const revision of oldCommits)
      expect(await whiteboardVcs.resolve(dir, revision)).toBe(revision);
    ...
    for (const entry of oldRefs.filter(
      ...
      const [ref, revision] = entry.split(" ");
      expect(await whiteboardVcs.resolve(dir, ref!)).toBe(revision);
    }
    ...
    const snapshot = await snapshotWhiteboardTree(dir);
    expect(await readStoredWhiteboard(dir)).toEqual(loaded);
    expect(await listWhiteboards()).toMatchObject({ errors: [] });
    expect((await findWhiteboard(uuid))?.review.schemaVersion).toBe(5);
  ...
  it("reports repair without mutations for a corrupt sealed document", async () => {
    ...
    const brokenRevision = await sealWhiteboardCandidate(
      dir,
...
it("lists healthy reviews alongside a corrupt sealed presentation", async () => {
  ...
  const snapshot = await snapshotWhiteboardTree(broken.dir);
  ...
  expect(await snapshotWhiteboardTree(broken.dir)).toEqual(snapshot);
});
