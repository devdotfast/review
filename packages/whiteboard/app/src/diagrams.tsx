...
import type { Step } from "../../src/session-api/document";
...
import type { GuidedTour, PeekAnchor } from "./whiteboard-panel-model";
import { useTourPersist, useTourRestore } from "./whiteboard-view-state";
...
function SequenceDiagramFigure({
  ...
  theme: ReturnType<typeof useWhiteboardDebugSettings>["theme"];
  stopCount: number;
  openTour: (anchor?: string) => void;
    ...
    <>
      ...
      <EdgeLabelRenderer>
        <button
          ...
          data-whiteboard-anchor-id={data.message.id}
          data-motion={stepMotion}
          onClick={(event) => {
            event.stopPropagation();
            data.openTour(data.message.id);
