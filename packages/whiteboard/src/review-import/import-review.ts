...
import { errorMessage, loadWhiteboardAgentTrace } from "@dev.fast/trace-core";
...
import { decodeImage } from "../session-api/image-decode";
import type { LocalSessionData } from "../session-api/local-data";
import type {
  ImportedVersionInput,
  SessionStore,
  SnapshotOrigin,
} from "../session-api/store";
import { readWhiteboardSoftwareMapBundle } from "../software-map-bundle";
import { legacySoftwareMapBundle } from "../stored-review-migration";
import { WHITEBOARD_DOCUMENT_BUNDLE_DIR } from "../whiteboard-bundle";
import {
...
import { type WhiteboardVcsLogEntry, whiteboardVcs } from "../whiteboard-vcs";
import {
  type ImageRequest,
  type TraceRequest,
...
export interface ImportLegacyWhiteboardInput {
  ...
  log?: (dir: string) => Promise<WhiteboardVcsLogEntry[]>;
  loadTrace?: typeof loadWhiteboardAgentTrace;
  now?: () => Date;
...
type StoredRecord = StoredWhiteboard["review"];
 ...
 */
export async function importLegacyWhiteboard(
  input: ImportLegacyWhiteboardInput,
  ...
  const traces = new TraceResolver({
    ...
    repositoryId,
    worktreePath,
    loadTrace: input.loadTrace ?? loadWhiteboardAgentTrace,
  });
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    const sealedRecord = parseAnyStoredWhiteboardRecord(
    ...
    if (preserved?.origin?.revision === entry.oid) {
      const sealed = whiteboardDocumentDataSchema.parse(
        upgradeWhiteboardDocumentJson(parseJsonText(raw)),
      );
...
async function importMapSection(
  review: StoredWhiteboard,
  revision: string,
  materialize: ImportLegacyWhiteboardInput["materialize"],
  store: SessionStore,
  repositoryId: string,
  warnings: string[],
  pins?: Pins,
  ...
  try {
    ...
    const bundle =
      (await readWhiteboardSoftwareMapBundle(dir)) ??
...
async function pendingRevisions(
  ...
  log: (dir: string) => Promise<WhiteboardVcsLogEntry[]>,
): Promise<WhiteboardVcsLogEntry[] | null> {
...
async function readSealedDocument(dir: string): Promise<string | null> {
  try {
    return await readFile(
      path.join(dir, WHITEBOARD_DOCUMENT_BUNDLE_DIR, SEALED_DOCUMENT_FILE),
      "utf8",
    );
  } catch (error) {
    ...
    for (const bundleDir of [WHITEBOARD_DOCUMENT_BUNDLE_DIR, ".bundle"]) {
      try {
      ...
      const evaluated = await evaluateSealedWhiteboardDocument(dir);
...
async function revisionPins(
  ...
  try {
    const record = parseAnyStoredWhiteboardRecord(
      parseJsonText(await readFile(path.join(dir, "review.json"), "utf8")),
    );
