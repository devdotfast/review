	...
	public static fromSerialized(data: ISerializedMultiDiffEditorInput, instantiationService: IInstantiationService): MultiDiffEditorInput {
		return instantiationService.createInstance(
			...
			data.resources?.map(resource => new MultiDiffEditorItem(
				...
				undefined,
				resource.labelUris ? {
					original: resource.labelUris.original ? URI.parse(resource.labelUris.original) : undefined,
					modified: resource.labelUris.modified ? URI.parse(resource.labelUris.modified) : undefined,
				} : undefined,
			)),
			false
		);
	...
	public serialize(): ISerializedMultiDiffEditorInput {
		return {
			...
			resources: this.initialResources?.map(resource => ({
				...
				labelUris: resource.labelUris ? {
					original: resource.labelUris.original?.toString(),
					modified: resource.labelUris.modified?.toString(),
				} : undefined,
			})),
		};
	}
			...
			const result: IDocumentDiffItemWithMultiDiffEditorItem = {
				...
				get options() {
					return {
						...
						...computeMultiDiffEditorOptions(textResourceConfigurationService.getValue(uri)),
						...r.options,
...
export function computeMultiDiffEditorOptions(configuration: IEditorConfiguration): IDiffEditorOptions {
