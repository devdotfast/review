...
import {
	WHITEBOARD_CLIENT_RECONNECT_DELAYS,
	reconnectUntilAborted,
} from "./whiteboardReconnect.js";
...
test("walks the delay table between failures and returns once connect resolves", async () => {
	...
	assert.deepEqual(waits, WHITEBOARD_CLIENT_RECONNECT_DELAYS.slice(0, 3));
...
test("keeps retrying at the last delay when no exhaustion handler is given", async () => {
	const waits: number[] = [];
	const total = WHITEBOARD_CLIENT_RECONNECT_DELAYS.length + 2;
	await reconnectUntilAborted(new AbortController().signal, failing(total), {
		wait: async (ms) => {
			waits.push(ms);
		},
	});
	const last = WHITEBOARD_CLIENT_RECONNECT_DELAYS[WHITEBOARD_CLIENT_RECONNECT_DELAYS.length - 1];
	assert.deepEqual(waits, [...WHITEBOARD_CLIENT_RECONNECT_DELAYS, last, last]);
});
...
test("calls onExhausted with the last error once the table runs out", async () => {
	...
	await assert.rejects(
		reconnectUntilAborted(
			...
			failing(WHITEBOARD_CLIENT_RECONNECT_DELAYS.length + 1),
			{
				onExhausted: (error) => {
		...
		(error: Error) =>
			...
			(error.cause as Error).message ===
				`failure ${WHITEBOARD_CLIENT_RECONNECT_DELAYS.length + 1}`,
	);
	assert.deepEqual(waits, [...WHITEBOARD_CLIENT_RECONNECT_DELAYS]);
});
...
test("a successful connection restarts the delay table", async () => {
	...
	const first = WHITEBOARD_CLIENT_RECONNECT_DELAYS[0];
