...
export class NativePolicyService extends AbstractPolicyService implements IPolicyService {
	...
	protected async _updatePolicyDefinitions(policyDefinitions: IStringDictionary<PolicyDefinition>): Promise<void> {
		...
		// Review does not ship @vscode/policy-watcher; native OS policies are
		// unsupported and the service degrades to reporting no policies.
		let createWatcher;
		try {
			({ createWatcher } = await import('@vscode/policy-watcher'));
		} catch {
			this.logService.trace(`NativePolicyService#_updatePolicyDefinitions - @vscode/policy-watcher is not installed; native policies are disabled`);
			return;
		}
