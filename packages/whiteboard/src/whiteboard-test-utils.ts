...
const DEFAULT_STORED_WHITEBOARD_UUID = "11111111-1111-4111-8111-111111111111";

const DEFAULT_LEGACY_DOCUMENT_CODE = `import { createActiveWhiteboardDocument, jsx } from "review-doc-runtime";
export default createActiveWhiteboardDocument({ title: "Sealed", routePath: "/", filePath: "review.mdx", modelNames: [], models: {}, Component: () => jsx("h1", { children: "Exact sealed title" }), isDefault: true });`;
...
export async function whiteboardHome(): Promise<string> {
...
export async function storedWhiteboardFixture(
  options: { schemaVersion?: 4 | 5; uuid?: string } = {},
): Promise<{ whiteboardDir: string; record: JsonObject }> {
  const root = await tempDir("review-promotion-");
  const whiteboardDir = path.join(root, "review");
  await mkdir(path.join(whiteboardDir, ".git"), { recursive: true });
  await mkdir(path.join(whiteboardDir, ".bundle"));

  const record: JsonObject = {
    schemaVersion: options.schemaVersion ?? 5,
    uuid: options.uuid ?? DEFAULT_STORED_WHITEBOARD_UUID,
    repoKey: "repo",
    worktreePath: "/source",
    baseRef: "main",
  ...
  await writeFile(path.join(whiteboardDir, ".git", "HEAD"), "old-head");
  await writeFile(
    path.join(whiteboardDir, ".bundle", "document"),
    "old-document",
  );

  return { whiteboardDir, record };
}
...
export async function writeLegacyDocument(
  ...
  const target = path.join(whiteboardDir, ".bundle/document");
