...
import {
  MAX_WHITEBOARD_TAB_DWELL_MS,
  type WhiteboardTabDwellPayload,
  createWhiteboardTabDwellTracker,
  createWhiteboardTabTelemetryTransport,
  sendWhiteboardTabTelemetryPayload,
} from "./tab-dwell-telemetry";

describe("createWhiteboardTabDwellTracker", () => {
  ...
  it("flushes and pauses when the page becomes hidden", () => {
    let now = 0;
    let visible = true;
    const sent: WhiteboardTabDwellPayload[] = [];
  ...
  it("uses sendBeacon for pagehide delivery when available", () => {
    let now = 0;
    const beacons: Array<{ url: string; body: WhiteboardTabDwellPayload }> = [];
    ...
    const tracker = createWhiteboardTabDwellTracker({
      ...
      now: () => now,
      isVisible: () => true,
      send: createWhiteboardTabTelemetryTransport({
        endpoint: "/telemetry/tab",
        navigator: {
          sendBeacon(url, data) {
            beacons.push({
              url: String(url),
              body: JSON.parse(String(data)) as WhiteboardTabDwellPayload,
            });
  ...
  it("falls back to keepalive fetch when sendBeacon rejects the document origin", () => {
    ...
    const delivery = sendWhiteboardTabTelemetryPayload(
      {
        tab: "review",
        duration_ms: 500,
        reason: "pagehide",
  ...
  it("captures the session endpoint before the bridge is torn down", () => {
    ...
    const send = createWhiteboardTabTelemetryTransport({
  ...
  it("caps stale visible segments at four hours", () => {
    ...
    now = MAX_WHITEBOARD_TAB_DWELL_MS + 1_000;
    tracker.unmount();

    expect(sent[0]?.duration_ms).toBe(MAX_WHITEBOARD_TAB_DWELL_MS);
