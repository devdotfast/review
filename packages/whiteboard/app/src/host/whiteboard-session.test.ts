...
import { createWhiteboardSession } from "./whiteboard-session";

it("sends JSON requests to the displayed version with authentication, including beacon delivery", async () => {
  const request = vi.fn<ReturnType<typeof testWhiteboardBridge>["request"]>(
  ...
  const session = createWhiteboardSession(
    testWhiteboardBridge({}, { request }),
    {
      jsonWhiteboard: { id: "review/id", version: () => version },
    },
  );
