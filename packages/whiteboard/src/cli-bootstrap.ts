...
export async function runCliBootstrap(entryUrl: string): Promise<number> {
  ...
  const ownCliPath = fileURLToPath(entryUrl);

  const delegatedExitCode = await maybeDelegateToDesktopCli(argv);

  return delegatedExitCode ?? (await runCli(ownCliPath));
    ...
    if (!supportedNodeRuntime()) {
      process.stderr.write(
        `Whiteboard needs Node.js 24 or newer; found ${process.versions.node}. ` +
          `Update Node, or use the whiteboard command installed by Desktop.\n`,
      );
  ...
  async function maybeDelegateToDesktopCli(
    ...
    if (!/[\\/]dist[\\/](?:whiteboard-)?cli\.js$/.test(ownPath)) return null;

    // This bootstrap runs before the Node floor check, so it cannot import
    // devWhiteboardHome() from @dev.fast/trace-core: that module graph needs a modern
    // Node. Keep this copy in step with devWhiteboardHome().
    ...
    try {
      ...
      cliPath = path.join(path.dirname(discoveredCliPath), "whiteboard-cli.js");
