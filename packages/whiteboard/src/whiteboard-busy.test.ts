...
import { createWhiteboardDir, readStoredWhiteboard } from "./whiteboard-home";
...
it("reports loader and open contention as busy and allows migration after release", async () => {
  ...
  const holding = withWhiteboardMutationLock(review.dir, async () => {
    entered.resolve();
  ...
  try {
    const loaded = await readStoredWhiteboard(review.dir);
