...
Cette fonctionnalité désactive [les protections de sécurité critiques](https://code.visualstudio.com/docs/editor/artificial-intelligence) et facilite considérablement la compromission de la machine par un attaquant.`,
...
suite('Policy E2E conversion', () => {
	...
	test('should render macOS manifest from policies list', async () => {
		...
		const normalizeTimestamp = (content: string) =>
			content.replace(/<date>.*?<\/date>/, '<date>TIMESTAMP</date>').trimEnd();
		assert.strictEqual(
