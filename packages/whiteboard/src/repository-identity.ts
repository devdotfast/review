...
import type { WhiteboardRepositoryIdentity } from "@dev.fast/whiteboard-protocol";

export async function resolveWhiteboardRepositoryIdentity(
  rootPath: string,
): Promise<WhiteboardRepositoryIdentity> {
...
function identity(
  kind: WhiteboardRepositoryIdentity["kind"],
  repositoryPath: string,
  worktreeRoot: string,
): WhiteboardRepositoryIdentity {
  return {
    kind,
    repositoryId: crypto
      .createHash("sha256")
