...
import type { Pins } from "../session-api/document.js";
import { createSessionApi } from "../session-api/http.js";
import { serveWhiteboardMcp } from "../session-api/mcp.js";
import { openWhiteboardProfile } from "../session-api/profile.js";
import type { Result, Snapshot } from "../session-api/store.js";
...
async function start(
  ...
  const ready = Promise.withResolvers<WhiteboardServerDiscovery>();
...
it("shares review identity, resources, sessions and live changes with Desktop in one profile", async () => {
  ...
  const local = await openWhiteboardProfile(root, { manageWorkspaces: true });
  const opened: string[] = [];
  ...
  let whiteboardStream: ReturnType<SessionApiClient["watch"]> | undefined;
  ...
  try {
    ...
    whiteboardStream = desktop.watch(created.sessionId, abort.signal);
    expect((await whiteboardStream.next()).value).toMatchObject({
    ...
    for (;;) {
      const value = (await whiteboardStream.next()).value;
    ...
    await expect(async () => {
      for await (const _value of whiteboardStream!) {
      }
...
it("authors through CLI and MCP without Desktop and retains source, unfinished sections and resources across restart", async () => {
  ...
  await server.stop();
  expect(await readWhiteboardServerDiscovery(server.stateDir)).toBeNull();
...
it("authenticates clients, reports capabilities and readiness without exposing the token, and diagnoses unavailable commits", async () => {
  ...
  expect(await whiteboardServerIsHealthy(server.discovery)).toBe(true);
...
it("rejects a second owner and keeps separate CI job stores independent", async () => {
  ...
  await first.stop();
  expect(await whiteboardServerIsHealthy(second.discovery)).toBe(true);
});
...
it("releases ownership after a port bind failure so startup can be retried", async () => {
  ...
  expect(await whiteboardServerIsHealthy(retried.discovery)).toBe(true);
});

it("does not connect to another instance through stale discovery", async () => {
  const server = await start();
  const discoveryPath = whiteboardServerDiscoveryPath(server.stateDir);
