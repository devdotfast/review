...
import type { WhiteboardStackLayer } from "@dev.fast/whiteboard-protocol";
import { z } from "zod";
...
export interface WhiteboardStackSubject {
  pullRequestUrl?: string | null;
}

export interface WhiteboardStackCandidate {
  uuid: string;
  title: string;
  repoKey: string;
...
export async function resolveWhiteboardStackLayers(
  subject: WhiteboardStackSubject,
  reviews: readonly WhiteboardStackCandidate[],
  runGitHubApi: RunGitHubApi = defaultRunGitHubApi,
): Promise<WhiteboardStackLayer[]> {
  ...
  return stack.pull_requests.map((pr, index) => {
    ...
    return {
      ...
      whiteboardTitle: review?.title ?? null,
      relation:
        index < currentIndex
          ? "earlier"
          : index === currentIndex
