...
- Check `review trace status`; `--session <id>` narrows uploads. Offline receipts are historical. If authorized, run `review trace onboard` when needed, then `review trace allow .`. Check status again.
