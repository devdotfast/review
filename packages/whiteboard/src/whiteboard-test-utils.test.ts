...
it("removes every tracked directory and unstubs the review home", async () => {
  ...
  expect(devWhiteboardHome()).toBe(home);

  await cleanupTempDirs();

  await expect(stat(dir)).rejects.toMatchObject({ code: "ENOENT" });
  await expect(stat(home)).rejects.toMatchObject({ code: "ENOENT" });
  expect(devWhiteboardHome()).not.toBe(home);
});
...
it("stages a promotable review directory at either stored schema", async () => {
  const current = await storedWhiteboardFixture();
  expect(parseStoredWhiteboardRecord(current.record).schemaVersion).toBe(5);
  expect(
    await readFile(path.join(current.whiteboardDir, ".git", "HEAD"), "utf8"),
  ).toBe("old-head");
  expect((await readdir(path.dirname(current.whiteboardDir))).sort()).toEqual([
    "review",
  ]);

  const legacy = await storedWhiteboardFixture({
    schemaVersion: 4,
...
it("writes a v1 document bundle with an overridable module body", async () => {
  const { whiteboardDir } = await storedWhiteboardFixture();
  await writeLegacyDocument(whiteboardDir, {
    code: "throw new Error('legacy');",
  });

  const bundle = path.join(whiteboardDir, ".bundle", "document");
  expect(
