...
import {
  WHITEBOARD_INTERACTION_EVENT,
  whiteboardInteractionDetail,
} from "./whiteboard-interaction-event";
...
export function TutorialExperienceProvider({
  ...
  useLayoutEffect(() => {
    ...
    const canvasRoot =
      root?.closest<HTMLElement>(".whiteboard-canvas-root") ??
      root?.parentElement;
  ...
  useEffect(() => {
    ...
    const onWhiteboardInteraction = (event: Event) => {
      const detail = whiteboardInteractionDetail(event);
  ...
  useLayoutEffect(() => {
    ...
    [...root.querySelectorAll<HTMLElement>("[data-whiteboard-section]")]
      .find((section) => section.dataset.whiteboardSection === activeTitle)
  ...
  useLayoutEffect(() => {
    ...
    const apply = () => {
      ...
      if (first && !revealed) {
        ...
        const view = (
          root.querySelector(".whiteboard-view-region") ?? root
  ...
  useLayoutEffect(() => {
    ...
    if (region) {
      for (const child of region.querySelectorAll(
        ":scope > *, :scope > .whiteboard-document-view > *",
      )) {
