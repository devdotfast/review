...
import { documentSchema, resourceReferences } from "../session-api/document";
import type { LocalSessionData } from "../session-api/local-data";
import type { SessionStore, Snapshot } from "../session-api/store";
...
export function createTutorialService(input: {
  ...
  const tutorialRoot = path.join(devWhiteboardHome(), "tutorial");
  ...
  return {
    ...
    async referencesWhiteboard(id: string) {
      return (
