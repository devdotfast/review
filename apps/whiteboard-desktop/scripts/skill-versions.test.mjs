...
import { stampWhiteboardSkills } from "./stage-whiteboard-runtime.mjs";
...
const sourceSkills = path.resolve(appRoot, "../../packages/whiteboard/skills");
...
test("stamps all packaged skills with the Desktop release, preserving source hardlinks", async () => {
  ...
  try {
    ...
    await stampWhiteboardSkills(runtime);
    ...
    await stampWhiteboardSkills(runtime, "2.0.0-preview.1");
    assert.ok(
...
test("refuses invalid release versions and skills without generated metadata", async () => {
  ...
  try {
    await assert.rejects(
      stampWhiteboardSkills(root, "not-a-version"),
      /release version/,
    );
    ...
    await assert.rejects(
      stampWhiteboardSkills(root, "1.0.0"),
