...
import type { Frame } from "../../src/session-api/document";
import { DocumentCallTree } from "./call-tree-view";
import {
  WhiteboardPanelProvider,
  useWhiteboardPanel,
} from "./whiteboard-panel";
import type { WhiteboardPanelStoreState } from "./whiteboard-panel-store";
import {
...
describe("DocumentCallTree", () => {
  it("opens a removed frame on the base side from the shared tree", async () => {
    ...
    const opened =
      vi.fn<(active: WhiteboardPanelStoreState["active"]) => void>();

    function PanelSpy() {
      const active = useWhiteboardPanel((state) => state.active);
    ...
    await act(async () => {
      root!.render(
        whiteboardSessionElement(
          testWhiteboardSession(),
          <WhiteboardPanelProvider detailRevision={0}>
            <PanelSpy />
            <DocumentCallTree
              block={{
    ...
    const removed = container.querySelector<HTMLButtonElement>(
      '[data-whiteboard-anchor-id="auth"]',
    )!;
