...
export function DiagramActions({ id }: { id: string }) {
  const lenses = useReviewLenses();
  const item = lenses?.progress?.diagrams.find((item) => item.id === id);

  if (!lenses || !item) return null;
  const progress = lenses.stats(item.sources);

  return (
    <div className="review-diagram-actions">
      <button
        disabled={!!item.unavailable}
        title={item.unavailable}
        onClick={() => lenses.select(id)}
      >
        View in diff
      </button>
      <ViewedButton
        ...
        onClick={() =>
          void lenses.mark(item.sources, progress.state !== "viewed")
        }
      />
    </div>
  );
