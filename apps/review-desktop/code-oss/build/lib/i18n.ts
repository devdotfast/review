...
// The Review fork does not install @vscode/l10n-dev (the localization export
// pipeline is unused). Types are declared locally and the module is loaded
// through a non-literal specifier so this file still parses and typechecks;
// only the XLF import/export tasks fail if actually invoked.
type l10nJsonFormat = Record<string, string | { message: string; comment: string[] }>;
interface l10nJsonDetails {
	messages: l10nJsonFormat;
	name: string;
	language: string;
}
interface L10nDevModule {
	getL10nXlf(map: Map<string, l10nJsonFormat>): string;
	getL10nFilesFromXlf(content: string): Promise<l10nJsonDetails[]>;
	getL10nJson(files: { contents: string; extension: string }[]): Promise<l10nJsonFormat>;
}
const l10nDevSpecifier = '@vscode/l10n-dev';
let l10nDevModule: Promise<L10nDevModule> | undefined;
function loadL10nDev(): Promise<L10nDevModule> {
	return l10nDevModule ??= import(l10nDevSpecifier);
}
...
function createL10nBundleForExtension(extensionFolderName: string, prefixWithBuildFolder: boolean): NodeJS.ReadWriteStream {
	...
	return gulp
		...
		.pipe(eventStream.map(function (data, callback) {
			...
			if (extension !== '.json') {
				const contents = file.contents.toString('utf8');
				loadL10nDev().then(m => m.getL10nJson([{ contents, extension }]))
					.then((json) => {
...
export function createXlfFilesForExtensions(): eventStream.ThroughStream {
	...
	return eventStream.through(function (this: eventStream.ThroughStream, extensionFolder: File) {
		...
		eventStream.merge(
			...
			if (_l10nMap?.size > 0) {
				loadL10nDev().then(({ getL10nXlf }) => {
					folderStream.queue(new File({
...
export function prepareI18nPackFiles(resultingTranslationPaths: TranslationPath[]): NodeJS.ReadWriteStream {
	...
	return eventStream.through(function (this: eventStream.ThroughStream, xlf: File) {
		...
		const parsePromise = loadL10nDev().then(m => m.getL10nFilesFromXlf(contents));
