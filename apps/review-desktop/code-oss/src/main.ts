...
registerReviewOptionalExtensionInstaller(validatedIpcMain, {
	userDataPath,
...
try {
	...
	writeReviewBootstrapBreadcrumb(userDataPath, error);
}
validatedIpcMain.handle('vscode:reviewImportUserConfig', async (_event, request: unknown) => {
	...
	return importReviewUserConfig({
		userDataPath,
		mode: mode satisfies ReviewUserConfigImportMode,
