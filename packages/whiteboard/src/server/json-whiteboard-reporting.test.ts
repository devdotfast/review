...
import { SessionStore } from "../session-api/store";
import type { WhiteboardTelemetry } from "../telemetry";
import { type BugReportPayload, submitWhiteboardBugReport } from "./bug-report";
...
it("routes sanitized telemetry and uploads only opted-in JSON context from the displayed version", async () => {
  ...
  const telemetry = {
    captureUiEvent: vi.fn<WhiteboardTelemetry["captureUiEvent"]>(),
    captureTabViewed: vi.fn<WhiteboardTelemetry["captureTabViewed"]>(),
  };
  ...
  const app = createJsonWhiteboardReporting(store, telemetry, {
    submit: (input) => submitWhiteboardBugReport({ ...input, fetchImpl }),
  });
...
it("rejects shared telemetry when the shared store is unavailable", async () => {
  ...
  const captureUiEvent = vi.fn<WhiteboardTelemetry["captureUiEvent"]>();

  const app = createJsonWhiteboardReporting(store, {
    captureUiEvent,
    captureTabViewed: vi.fn<WhiteboardTelemetry["captureTabViewed"]>(),
  });
