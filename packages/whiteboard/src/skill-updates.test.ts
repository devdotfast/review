...
import { whiteboardCliInstallResyncRequest } from "@dev.fast/whiteboard-protocol";
...
async function fixture() {
  ...
  await cp(
    path.join(workspace, "packages/whiteboard/skills"),
    path.join(packageRoot, "skills"),
    { recursive: true },
  );
  await cp(
    path.join(workspace, "packages/whiteboard/plugins"),
  ...
  const stampVersion = async (version: string) => {
    await execFileAsync(
      ...
      [
        "--input-type=module",
        "-e",
        'import { stampWhiteboardSkills } from "./apps/whiteboard-desktop/scripts/stage-whiteboard-runtime.mjs"; await stampWhiteboardSkills(process.argv[1], process.argv[2]);',
        packageRoot,
        version,
      ],
      { cwd: workspace },
  ...
  const launch = async () => {
    ...
    const request = status.stale
      ? whiteboardCliInstallResyncRequest(status)
      : undefined;
...
describe("packaged skill updates", () => {
  ...
  it("serializes independent installer processes", async () => {
    ...
    const code = `import { applyCliInstall } from "./packages/whiteboard/src/cli-install.ts";
