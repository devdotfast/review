    ...
    await withPublishedFixture(async (review, revision) => {
      await expect(
        materializePublishRevision(
          ...
          {
            materialize: async (dir, ref, destination) => {
              await materializeWhiteboardRevision(dir, ref, destination);
...
async function withPublishedFixture(
  run: (
    review: Awaited<ReturnType<typeof createWhiteboardDir>>,
    revision: string,
  ) => Promise<void>,
) {
  ...
  try {
    ...
    const revision = await sealWhiteboardCandidate(review.dir, "Complete");
