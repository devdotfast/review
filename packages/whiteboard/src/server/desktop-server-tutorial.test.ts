...
import { type JsonObject, isJsonObject } from "@dev.fast/whiteboard-protocol";
import { afterEach, describe, expect, it, vi } from "vitest";

import { elements } from "../session-api/document";
...
type GlobalServerInput = Parameters<typeof createGlobalWhiteboardServer>[0];

type TutorialServerOverrides = Pick<
  GlobalServerInput,
  "whiteboardStore" | "whiteboardData"
>;
...
function tutorialServer(home: string, overrides: TutorialServerOverrides) {
  return createGlobalWhiteboardServer({
    appPid: process.pid,
    packageRoot,
