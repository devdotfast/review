...
const [whiteboardDir, name, sourceRepository] = process.argv.slice(2);

const approved = approvedFixtures.find(
  (fixture) => fixture.name === name && fixture.repository === sourceRepository,
);

if (!whiteboardDir || !approved) {
  throw new Error(
    "usage: capture-legacy-review-fixture <approved whiteboardDir> <approved name> <devdotfast/review|tutorial-sample>",
  );
}

const sourceDir = await realpath(whiteboardDir);
