...
import {
  type WhiteboardBugReportMetaV2,
  type WhiteboardBugReportRequest,
  parseWhiteboardBugReportResponse,
} from "@dev.fast/whiteboard-protocol";

import { readWhiteboardPackageVersion } from "../package-paths";
import { type WhiteboardDiffFilesResult } from "../whiteboard-diff-files";
import {
...
export interface BugReportPayload {
  ...
  map?: string;
  diff?: WhiteboardDiffFilesResult;
...
export interface BugReportSource {
  ...
  diff(): Promise<WhiteboardDiffFilesResult>;
  trace(): Promise<AuthoringTraceAttachment | null>;
}

export async function submitWhiteboardBugReport(input: {
  report: WhiteboardBugReportRequest;
  clientErrorNames: string[];
  fetchImpl?: typeof fetch;
  source: BugReportSource;
}) {
  const cliVersion = readWhiteboardPackageVersion();
  ...
  let whiteboardSource: Record<string, string> | undefined;
  let omittedWhiteboardFiles: string[] | undefined;
  let mapSource: string | undefined;
  let changedFileDiffs: WhiteboardDiffFilesResult | undefined;
  ...
  if (input.report.include_review) {
    tasks.push(
      input.source.review().then(
        (result) => {
          whiteboardSource = result.files;

          if (result.omitted.length > 0)
            omittedWhiteboardFiles = result.omitted;
        },
        () => {
  ...
  if (whiteboardSource !== undefined) payload.review = whiteboardSource;

  if (omittedWhiteboardFiles !== undefined) {
    payload.diagnostics.review_omitted_files = omittedWhiteboardFiles;
  }
  ...
  try {
    ...
    const result = parseWhiteboardBugReportResponse(responseBody);
...
export async function buildBugReportRequest(
  ...
  const meta: WhiteboardBugReportMetaV2 = {
    schema_version: 2,
