...
import {
  ...
  parseWhiteboardDiffFilesResponse,
  parseWhiteboardFileContentRequest,
  parseWhiteboardFileContentResponse,
...
describe("review protocol parsers", () => {
  it("parses global desktop discovery and session-prefixed runtime URLs", () => {
    ...
    expect(() =>
      parseWhiteboardDesktopDiscovery({
        version: WHITEBOARD_DESKTOP_DISCOVERY_VERSION + 1,
        instanceId: "desktop-1",
  ...
  it("parses file lists and file content variants", () => {
    expect(
      parseWhiteboardFileContentRequest({ path: "src/new.ts", side: "base" }),
    ...
    expect(
      parseWhiteboardFileContentResponse({ ok: true, absent: true }),
    ).toEqual({
      ok: true,
      absent: true,
    });
    expect(
      parseWhiteboardFileContentResponse({ ok: true, binary: true }),
    ).toEqual({
      ok: true,
      binary: true,
    });
