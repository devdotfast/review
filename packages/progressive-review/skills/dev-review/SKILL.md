...
Author through the running Desktop's Review Host. JSON nodes, definitions, retained evidence, maps, saved review versions and conversations belong to that host. Never read or write Review document files, databases, Git notes or generated bundles.
...
Read [Document authoring](references/document-authoring.md) and [Component API](references/component-api.md). Read [Lifecycle and storage](references/lifecycle-and-storage.md) for code versions, history and feedback; [Trace quoting](references/trace-quoting.md) only when using supplied trace evidence.
...
3. Call `review.create({repositoryId,change,title,description?,labels?})`. For an exact range, use `change:{kind:"range",baseRef,headRef}` with resolved commit IDs. For architecture, use `{kind:"snapshot",ref}`. Record the returned review ID, `reviewVersion`, `stateVersion` and binding.
...
5. Read source through `source.read`, `source.tree`, `source.diff` and `source.commits` with the observed `reviewVersion`. A code peek references an anchor; the host resolves and retains its evidence. Plain code is illustrative, not proof of source.
6. When maps materially help, delegate a bounded worker using `dev-review-map` if available. Give it the review ID, observed review version and exact base/head commits. The worker owns map commands only and returns map-version IDs. Do not block useful content on optional map work.
7. Every accepted material edit is already saved as a review version. There is no publish/checkpoint step. Select comparison maps explicitly with `review.update({reviewId,expectedReviewVersion,mapVersions:{base,head}})`; null clears a side.
8. Confirm saved content through `review.get`/`document.get`. Invalid nodes are rejected on submission, before saving. There is no render-report API or viewer acknowledgment requirement. Inspect the real UI when visual verification is requested; do not claim to have seen it from a successful command alone.
...
Read submitted work with `feedback.list/get`, `threads.list` and `thread.get`. Private human drafts are not author-visible. Reply with `thread.reply`, update the document normally, and resolve addressed threads with `thread.set_status` when appropriate. Do not claim request changes automatically launches or resumes the author; submission persists independently of agent execution.
