...
1. `review_capabilities({})`. Draw only when `desktopAvailable` is true and `authoringMode` is `interactive`. Otherwise answer in chat.
...
4. Begin `review_activity({reviewId: "scratchpad", action: "begin", leaseId})` with a fresh UUID, then append blocks with `review_edit({reviewId: "scratchpad", edit: {type: "insert", content}})`. Omitted placement appends at the end; use `replace` on a block you drew earlier when the picture was wrong. The block shapes are the same as a review's; follow the edit tool's description for them, and read the [authoring guidance](../dev-review/references/document-authoring.md) for choosing components. End the lease when finished.
...
Keep it a log, not a document: new thoughts go below old ones. Do not build sections, outlines or status markers unless the user asks for structure. When the user asks to clear the pad, `replace` or `remove` its blocks; history keeps what was there.
