...
import { GroupIdentifier, IEditorPaneSelection } from '../../../common/editor.js';
...
export interface IHistoryService {
	...
	/**
	 * Add the active editor and its current selection as a navigation location.
	 */
	addActiveEditorNavigation(): void;

	/**
	 * Record the destination reached by the last explicit navigation.
	 */
	completeActiveEditorNavigation(groupId: GroupIdentifier, editor: EditorInput, selection: IEditorPaneSelection | undefined): void;

	/**
	 * Go to the active editor location added by the last explicit navigation.
	 */
	goBackToActiveEditorNavigation(): Promise<'none' | 'source' | 'destination'>;

	/**
	 * Go to the target saved when the explicit navigation returned to its source.
	 */
	goForwardToActiveEditorNavigation(): Promise<boolean>;
