...
**Tech Stack:** TypeScript 6, React 19 (JSX runtime), zod, vitest + jsdom (`packages/progressive-review`), node:test via tsx (`apps/review-desktop`), Code OSS fork (Monaco editor widgets), pnpm workspaces, oxlint/oxfmt.
...
- `apps/review-desktop/code-oss/src/vs/review/common/reviewProtocol.ts` is generated. Never edit it. Edit `packages/review-protocol/src/contracts.ts`, then run `pnpm --filter @dev.fast/review-desktop run protocol:sync`.
...
async function openReview(review) {
  ...
  await page.waitForFunction(() => document.body.dataset.reviewCanvasReady === "true", null, { timeout: 60000 });
  // Force every lazy peek to mount by scrolling the document to the bottom.
  await page.evaluate(async () => {
    const root = document.querySelector(".review-canvas-root");
...
async function snapshotReview() {
  return page.evaluate(() => {
    ...
    const editors = [...document.querySelectorAll(".review-inline-editor")].map((host) => ({
      ...
      additions: text(host.querySelector(".review-multidiff-additions")) || null,
      deletions: text(host.querySelector(".review-multidiff-deletions")) || null,
      hasOpenFile: !!host.querySelector(".review-multidiff-open"),
      height: Math.round(host.getBoundingClientRect().height),
      lines: [...host.querySelectorAll(".view-line")].map((l) => text(l)),
      error: text(host.nextElementSibling?.classList.contains("review-inline-editor-error") ? host.nextElementSibling : null) || null,
    }));
    return {
      title: text(document.querySelector(".review-canvas-root h1")),
      toc: [...document.querySelectorAll(".review-toc-link")].map((a) => ({ href: a.getAttribute("href"), text: text(a) })),
      headingIds: [...document.querySelectorAll(".review-canvas-root h2, .review-canvas-root h3")].map((h) => ({ id: h.id || null, text: text(h) })),
      sections: [...document.querySelectorAll(".review-section")].map((s) => ({
        title: s.dataset.reviewSection,
        collapsed: s.classList.contains("review-section--collapsed"),
        meta: text(s.querySelector(".review-section-meta")) || null,
      })),
      anchors: document.querySelectorAll("[data-review-anchor-id]").length,
...
async function collapseAllSections() {
  await page.evaluate(() => {
    for (const b of document.querySelectorAll(".review-section-toggle[aria-expanded='true']")) b.click();
  });
  await page.waitForTimeout(300);
  const meta = await page.evaluate(() =>
    [...document.querySelectorAll(".review-section")].map((s) => ({ title: s.dataset.reviewSection, meta: (s.querySelector(".review-section-meta")?.textContent ?? "").trim() || null })));
  await page.evaluate(() => {
    for (const b of document.querySelectorAll(".review-section-toggle[aria-expanded='false']")) b.click();
  });
  return meta;
}
...
async function findMatchCount(query) {
  ...
  const label = await page.evaluate(() => (document.querySelector(".review-find-count, [data-review-find-count]")?.textContent ?? "").trim());
...
```
Three selectors in this script are guesses you must confirm once against the running app and correct in place: the Home card locator in `openReview`, the scroll region selector, and the find-count element in `findMatchCount` (open `review-find.tsx` and use whatever class the count label has). Do that on the baseline run; after that the script is frozen for all phases.
...
Note: `apps/review-desktop` dev builds wipe `out/vs/review/canvas`; `run.sh` re-copies the canvas bundle, so always launch through `pnpm app:run`, never the raw binary.
...
| File | Responsibility |
|---|---|
| `packages/progressive-review/app/src/review-document-headings.ts` | Assign stable heading ids to hydrated `h2`/`h3` nodes and derive table-of-contents entries from them. Pure functions over `HydratedReviewNode[]`. |
| `packages/progressive-review/app/src/review-document-headings.test.ts` | Tests for the above. |
| `packages/progressive-review/app/src/review-section-summary.ts` | Count diagrams, code refs, and paragraphs in a section's hydrated children. Pure function. |
| `packages/progressive-review/app/src/review-section-summary.test.ts` | Tests for the above. |
| `apps/review-desktop/code-oss/src/vs/review/services/reviewUnifiedEditor.ts` | The one unified-diff `CodeEditorWidget` factory shared by inline peeks and the Files diff view. |

Files this plan deletes:

| File | Why |
|---|---|
| `apps/review-desktop/code-oss/src/vs/review/common/reviewSelection.ts` | Only caller was the dead `editorSelectionChanged` emitter. |
...
| Gate | Pass condition |
|---|---|
| 1 | Full suite green except the baseline failure list. Test count drops only by the tests this phase names for deletion (`review-doc-meta` provider cases, `reviewFindText`, `rulerTickForEvent`, `createC4MapFlow`, fingerprint assertions). |
...
| 3 | (a) Select text inside an inline peek and inside a Files diff editor: no console error, no stuck selection. (b) Home: dismiss a review; the baton chip on the document title bar shows the removed-line colour, not the default text colour. (c) `git grep -n "activeEditorChanged\|editorSelectionChanged\|reviewLastSurfaceEvent" apps packages ':!*/out/*' ':!*/dist/*' ':!apps/review-desktop/code-oss/src/vs/workbench'` prints nothing. |

### Task A1: Remove the unsubscribed editor surface events

**Files:**
- Modify: `packages/review-protocol/src/contracts.ts:1430-1447`
- Modify: `apps/review-desktop/code-oss/src/vs/review/contrib/verbs/reviewVerbs.ts:82, 109-116, 256, 298, 385, 393-431`
- Delete: `apps/review-desktop/code-oss/src/vs/review/common/reviewSelection.ts`
- Regenerate: `apps/review-desktop/code-oss/src/vs/review/common/reviewProtocol.ts`

**Interfaces:**
- Produces: `ReviewSurfaceEvent` narrows to `{event:"themeChanged"} | {event:"showReviewView"}`. Nothing else in this plan depends on it.

- [ ] **Step 1: Confirm there are no subscribers**

Run:
```bash
git grep -n "activeEditorChanged\|editorSelectionChanged" -- packages apps ':!*/dist/*' ':!*/out/*' ':!apps/review-desktop/code-oss/src/vs/workbench' ':!apps/review-desktop/code-oss/src/vs/review/common/reviewProtocol.ts'
```
Expected: exactly three hits, all in `reviewVerbs.ts` and `contracts.ts`. If an app file appears, stop and reassess.

- [ ] **Step 2: Remove the two schema members**

In `packages/review-protocol/src/contracts.ts`, `ReviewSurfaceEventSchema` becomes:
```ts
...
```
If `ReviewRangeSchema` is now unused in the file, leave it; other verbs use it (`git grep -n ReviewRangeSchema packages/review-protocol/src` to confirm).

- [ ] **Step 3: Remove the emitters and per-editor tracking in reviewVerbs.ts**
...
- in `browser/parts/canvas/reviewCanvasPart.ts:278-283`, the `dataset["reviewLastSurfaceEvent"] = event.event` assignment (a test hook with no reader; keep the `this.surfaceEvents.fire(event)` line)
...
```bash
git rm apps/review-desktop/code-oss/src/vs/review/common/reviewSelection.ts
pnpm --filter @dev.fast/review-desktop run protocol:sync
```

- [ ] **Step 5: Typecheck and test the desktop**

```bash
cd apps/review-desktop && pnpm typecheck && pnpm test
```
...
```bash
git add -A packages/review-protocol/src/contracts.ts apps/review-desktop/code-oss/src/vs/review
...
- Modify: `packages/progressive-review/app/src/review-doc-meta.tsx:21, 37-47, 61-83`
...
- Modify: `packages/progressive-review/app/src/review-doc-meta.test.tsx:63, 143`
...
`review-doc-meta.tsx`: remove the import at line 21 and the `initialData`/`initialDocumentMeta`/`initialDiffStats` locals. The two `useState` initializers become `useState<ReviewDocumentMetaState | null>(null)` and delete the `initialDiff` state entirely (grep the file for `initialDiff` and remove each use; where it was a fallback alongside `diffFiles`, keep only the `diffFiles` branch). The `/document-meta` effect loses its `if (!initialDocumentMeta)` guard and its dependency:
```tsx
useEffect(() => {
...
- Modify: `packages/progressive-review/app/src/App.tsx:609`, `packages/progressive-review/app/src/review-components.tsx:143`
- Modify: fixtures `packages/progressive-review/src/fixtures/legacy-reviews/*.expected-document.json`
- Modify: tests that assert `data-review-block-index`: `review-components.test.tsx`, `review-document-renderer.test.tsx`, `tutorial-document.test.tsx`, `src/review-document-examples.test.ts`, `src/review-document-materialize.test.ts`
...
```
Expected: only the producer lines in `rehype-review-targets.ts`, `App.tsx:609`, `review-components.tsx:143`. `data-review-block-tag` is NOT in this grep on purpose; it is still read at `review-components.tsx:283`.
...
Delete the `data-review-scroll-owner={...}` JSX attribute at `App.tsx:609` and `data-review-scroll-owner="panel"` at `review-components.tsx:143`.
...
```bash
cd packages/progressive-review && pnpm vitest run --config vitest.config.ts app/src/review-components.test.tsx app/src/review-document-renderer.test.tsx app/src/tutorial-document.test.tsx src/review-document-examples.test.ts src/review-document-materialize.test.ts src/legacy-review-fixtures.test.ts
```
Expected: pass.
...
**Files:**
- Modify: `packages/progressive-review/app/src/review-components.tsx:312-321, 421-435, 664-690, 1107-1143`

- [ ] **Step 1: Make the anchor required in ReviewPeekContentView**
...
`ProsePeekAnchorProps` (line 312), `keepAnchorLinkVisible` (line 421), `ReviewPeekPanel` (line 664), `ReviewPeekContentView` (done above). Confirm with:
```bash
git grep -n "ProsePeekAnchorProps\|keepAnchorLinkVisible\|ReviewPeekPanel\b\|ReviewPeekContentView" -- packages/progressive-review/app/src ':!packages/progressive-review/app/src/review-components.tsx'
```
Expected: no output.
...
- Modify: `packages/progressive-review/app/src/review-find-text.ts:61` (`reviewFindText`), `app/src/review-find.test.ts` (its tests)
...
`App.tsx:861` emits `review-baton-chip--${outcome}` with `outcome` in `approved | changes-requested | dismissed`. In `styles.css` rename `.review-baton-chip--rejected` (line 3093) and `.review-baton-chip--rejected .review-baton-glyph` (line 3112) to `--dismissed`, and delete the `.review-baton-chip--awaiting` rule (lines 3081-3083).
...
**Files:**
- Modify: `packages/review-protocol/src/contracts.ts:1351, 1397, 1109` (the `openFile` and `state` verb members; `codexThreadId`)
- Modify: `apps/review-desktop/code-oss/src/vs/review/contrib/verbs/reviewVerbs.ts:130, 181` (their `case` arms and any helper only they call)
...
git grep -nw "ReviewSession" -- packages/progressive-review/src ':!*/dist/*' | grep -v "types.ts:53"   # expect: none matching the bare name
```
...
- `reviewVerbs.ts`: remove `case "openFile"` and `case "state"` arms. Keep `openFileEditor`; `revealCode` still calls it. If `case "state"` was the only caller of the open-editors listing at lines 205-220 (the code that uses `editorIdentity`), delete that too, and then `editorIdentity` if it has no other caller.
- `desktop-server.ts:2526`: remove the `codexThreadId:` property from the session wire object. `types.ts`: remove `codexThreadId?: string` and the unreferenced `export interface ReviewSession` block.
- `database-lens.tsx:1138`: delete `storesForUseCase`.
- Regenerate the protocol: `pnpm --filter @dev.fast/review-desktop run protocol:sync`.

- [ ] **Step 3: Verify**

```bash
pnpm typecheck && cd apps/review-desktop && pnpm test && cd ../../packages/progressive-review && pnpm vitest run --config vitest.config.ts src/server app/src/database-lens.test.ts
```
...
| Gate | Pass condition |
|---|---|
| 1 | Full suite green except baseline. New tests `review-document-headings.test.ts` and `review-section-summary.test.ts` pass. `review-toc.test.tsx` passes with the mutation-collection cases removed. |
...
**Files:**
- Create: `packages/progressive-review/app/src/review-document-headings.ts`
...
- Produces:
  ...
  export interface ReviewTocEntry { id: string; text: string; level: ReviewTocLevel }
  ...
  ```
  `assignReviewHeadingIds` mutates `props.id` on every `h2`/`h3` element node that lacks one, using the same slug and uniqueness rules `review-toc.tsx` uses today (`slugifyHeading`, `uniqueHeadingId`, `normalizeHeadingText`, moved here verbatim). `reviewTocEntries` reads those ids back; it never assigns.
...
import { assignReviewHeadingIds, reviewTocEntries } from "./review-document-headings";
...
const section = (children: HydratedReviewNode[]): HydratedReviewNode =>
  ({ type: "component", name: "ReviewSection", props: { title: "s" }, children }) as HydratedReviewNode;
...
```bash
git add packages/progressive-review/app/src/review-document-headings.ts packages/progressive-review/app/src/review-document-headings.test.ts packages/progressive-review/app/src/review-document-hydrate.ts
...
**Files:**
- Modify: `packages/progressive-review/app/src/review-toc.tsx` (delete lines 114-176 collection effect, lines 397-435 `collectHeadingEntries`/`uniqueHeadingId`/`slugifyHeading`/`normalizeHeadingText`, and `tocEntriesEqual` if unused after)
- Modify: `packages/progressive-review/app/src/App.tsx:619`
- Modify: `packages/progressive-review/app/src/review-toc.test.tsx`

**Interfaces:**
- Consumes: `reviewTocEntries`, `ReviewTocEntry` from Task B1.
- Produces: `export function ReviewToc({ entries }: { entries: readonly ReviewTocEntry[] }): ReactElement | null`.

- [ ] **Step 1: Rewrite the TOC tests to pass entries**

`review-toc.test.tsx` builds an article via `innerHTML` and waits for collection. Change `renderArticle` to also stamp ids (`<h2 id="${slug}">`) and render `<ReviewToc entries={[...]} />` with matching entries. Delete the `settle()` waits that exist only for the rAF and 80/300 ms re-collects; keep one `act` for the initial render. Remove any test whose subject is "collects after mutation"; that behavior no longer exists.

- [ ] **Step 2: Run it to see it fail**

```bash
pnpm vitest run --config vitest.config.ts app/src/review-toc.test.tsx
```
...
In `review-toc.tsx`:
- `export function ReviewToc({ entries }: { entries: readonly ReviewTocEntry[] })`; delete `const [entries, setEntries] = useState(...)`.
...
- Delete `collectHeadingEntries`, `uniqueHeadingId`, `slugifyHeading`, `normalizeHeadingText`, and `tocEntriesEqual`. Import `ReviewTocEntry` from `./review-document-headings` and delete the local `ReviewTocLevel`/`ReviewTocEntry` declarations.
...
```bash
pnpm typecheck && pnpm vitest run --config vitest.config.ts app/src/review-toc.test.tsx app/src/App.test.ts app/src/tutorial-experience.test.tsx
```
...
**Files:**
- Create: `packages/progressive-review/app/src/review-section-summary.ts` (+ test)
- Modify: `packages/progressive-review/app/src/review-document-hydrate.ts:95-115` (`componentHydrators`)
- Modify: `packages/progressive-review/app/src/review-components.tsx:153-205`

**Interfaces:**
- Produces:
  ```ts
  export interface ReviewSectionSummary { diagrams: number; codeRefs: number; paragraphs: number }
  export function reviewSectionSummary(children: HydratedReviewNode[]): ReviewSectionSummary;
  ```
  Counts: `diagrams` = component nodes named `SequenceDiagram` or `DatabaseLens`; `codeRefs` = component nodes named `AnchorLink`, `CodePeek`, or `TraceQuote`; `paragraphs` = element nodes with tag `p`. Descend through every node. This mirrors today's DOM selectors (`.sequence-diagram, .database-lens`, `a[data-review-anchor-id], .code-peek`, `p`); the `.software-map` selector had no in-document producer and is dropped.
...
function hydrateComponentNode<K extends ReviewAuthoringComponentName>(node, anchors) {
...
```
`ReviewSectionSummary` is three numbers, so it satisfies `HydratedReviewPropValue` without a new union member.

- [ ] **Step 5: Read the summary in ReviewSection**

In `review-components.tsx`:
- Delete the local `interface ReviewSectionSummary` (line 153) and import it from `./review-section-summary`.
- Add next to the component: `const reviewSectionRenderPropsSchema = reviewSectionPropsSchema.extend({ summary: z.object({ diagrams: z.number(), codeRefs: z.number(), paragraphs: z.number() }).optional() });` and parse with it instead of `reviewSectionPropsSchema` (that schema is a `strictObject` in `authoring.ts:923`, so an unknown key would fail; `.extend` keeps strictness for the authoring shape while admitting the hydrated key).
...
**Files:**
- Create: `apps/review-desktop/code-oss/src/vs/review/services/reviewUnifiedEditor.ts`
- Modify: `apps/review-desktop/code-oss/src/vs/review/services/reviewInlineEditorService.ts:712-782`
- Modify: `apps/review-desktop/code-oss/src/vs/review/services/reviewUnifiedFilesEditor.ts:40-116`
...
```bash
git add apps/review-desktop/code-oss/src/vs/review/services
...
**Files:**
- Modify: `apps/review-desktop/code-oss/src/vs/review/services/reviewInlineEditorService.ts:712-745, 1130-1155`
...
**Files:**
- Modify: `apps/review-desktop/code-oss/src/vs/review/services/reviewInlineEditorService.ts`
- Modify: `apps/review-desktop/code-oss/src/vs/review/services/reviewMultiDiff.ts:39-50`
- Modify: `apps/review-desktop/code-oss/src/vs/review/services/reviewFilesDiffView.ts:198-224`
- Modify: `apps/review-desktop/code-oss/src/vs/review/common/reviewPeek.ts` (multi-diff height helpers, lines ~106-175)
- Modify: `packages/review-protocol/src/contracts.ts:181-184, 210` (remove `unifiedDiff` from `ReviewInlineEditorSpec` and `ReviewInlineFindSpec`)
- Modify: `packages/progressive-review/app/src/InlineCodeEditor.tsx`, `CodePeek.tsx`, `review-components.tsx` (drop the `unifiedDiff` prop end to end)
...
`reviewMultiDiff.ts:39-50`: remove the `scrollRange` constructor parameter. `reviewFilesDiffView.ts:198-224` and the remaining constructor call in `reviewInlineEditorService.ts:499-507` drop the corresponding positional argument. (Converting the six positional parameters into an options object is worthwhile here, since you are touching all three call sites; do it if it stays within this task.)

`reviewPeek.ts`: delete every export whose only importer was a deleted function (`git grep -n <name> apps/review-desktop/code-oss/src/vs/review` for each export in the file). `reviewPeekDiffWindows` and `reviewPeekLineMappings` stay: `acquireUnifiedDiff` still uses them through `resolveDiff`. Make `resolveDiff` private on `ReviewCodeResourceService` if `git grep` shows no other caller, and drop it from the `IReviewCodeResourceService` interface at lines 145-150.

- [ ] **Step 3: Remove `unifiedDiff` from the protocol and the app**

`contracts.ts`: delete the `unifiedDiff?: boolean` member and its doc comment from `ReviewInlineEditorSpec` (183-184) and `ReviewInlineFindSpec` (210). Run `pnpm --filter @dev.fast/review-desktop run protocol:sync`. Then in the app delete the prop from `InlineCodeEditor` (destructuring, `find` spec, `create` spec, effect deps), from `CodePeekCard`/`CodePeekView`/`ReviewCodePeek`/`CodePeekGroup` in `CodePeek.tsx`, and from `ReviewPeekContentView` in `review-components.tsx`. `InlineCodeEditor.test.tsx` and `CodePeek.test.tsx` will have `unifiedDiff` in a few expectations; delete those keys.
...
```bash
git add -A apps/review-desktop/code-oss/src/vs/review packages/review-protocol/src packages/progressive-review/app/src
...
- Modify: `packages/progressive-review/app/src/review-panel-model.ts:10` (type only, unchanged shape)
...
export function CodePeekCard({ input, active = false, heightMode = "capped", onNativeFocus }: {
  ...
  heightMode?: ReviewInlineEditorHeightMode; onNativeFocus?: () => void;
}) {
...
- Modify: `packages/progressive-review/app/src/host/review-session.tsx:19-23, 35, 71`
...
- Modify: `apps/review-desktop/scripts/native-authoring-e2e.mjs:584, 612` and the assertion between them

- [ ] **Step 1: Hydrate directly in desktop-entry**

Replace the `documentState` loader body:
```tsx
const documentState = useSettledLoad(documentBundle, (load): ReviewDocumentAppState => {
...
```
Delete the `purpose === "validation"` loop that threw when `anchor.peek.resolution` was null; publish already fails a document whose peek does not resolve (`review-publication-audit.ts:128-186`), so the mount-time check was redundant. Delete the diagnostics effect at 171-178 and the reset at 540. If `useSettledLoad` requires an async callback, keep `async` and return the same value.

`review-session.tsx`: `documents: Map<string, HydratedReviewDocument>`; delete `ReviewDocumentCacheEntry`.
...
- **createElement in the document renderer.** Tag and component identity come from data; JSX gains nothing. The four gratuitous `createElement` sites (`agent-markdown.tsx:146,160,261`, `review-view-state.ts:103`) are cosmetic and can be converted whenever those files are next edited.
...
- **`ReviewStateContext` / `ReviewActionsContext` split.** Load-bearing for re-render isolation of open peeks; merging would regress.
...
- **Scroll-restore `ResizeObserver` over every descendant** (`review-view-state.ts:369-371`). Deliberate; do not narrow without a repro.
...
- **Type consistency:** `ReviewTocEntry` is defined once (B1) and consumed by B2; `ReviewSectionSummary` defined once (B3); `ValidatedCodePeekInput` narrows in D1 and every later reference uses the narrowed shape; `setHeader`'s `counts` parameter (C2) matches `reviewUnifiedWindowCounts`'s return.
