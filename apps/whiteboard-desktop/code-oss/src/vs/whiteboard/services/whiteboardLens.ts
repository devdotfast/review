...
import { alignmentRows, lensContextGaps, viewedContextGaps } from '../common/whiteboardLens.js';
import type { WhiteboardDiffLens, WhiteboardDiffProgress } from '../common/whiteboardProtocol.js';
import type { WhiteboardFilesEditorEntry } from './whiteboardFilesDiffView.js';

export function lensRanges(lens: WhiteboardDiffLens, entry: WhiteboardFilesEditorEntry): WhiteboardDiffLens['ranges'] {
	return lens.ranges.filter(range => range.file === (range.side === 'base' ? entry.file.previousPath ?? entry.file.path : entry.file.path));
}

export function withLens(instantiation: IInstantiationService, entries: readonly WhiteboardFilesEditorEntry[], lens: WhiteboardDiffLens | undefined, lifetime: DisposableStore, progress: () => WhiteboardDiffProgress | undefined, onProgress: Event<void>): IInstantiationService {
	...
	const factory: IDiffProviderFactoryService = {
		...
		createDiffProvider(options) {
			...
			let activeEntry: WhiteboardFilesEditorEntry | undefined;
