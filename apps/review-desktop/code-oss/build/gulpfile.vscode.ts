...
import { getMxcExcludeFilter, getRipgrepExcludeFilter } from './lib/runtimePlatform.ts';
...
task.task(task.define('core-ci', task.series(
	...
	task.define('esbuild-vscode-min', () => runEsbuildBundle('out-vscode-min', true, true, `${sourceMappingURLBase}/core`))
)));
...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		if (platform === 'win32') {
			...
			// AppX packaging is an optional Windows integration. Products without
			// context-menu metadata do not build its package or explorer DLL.
			if ((quality === 'stable' || quality === 'insider') && 'win32ContextMenu' in product) {
...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const [vscode, vscodeMin] = ['', 'min'].map(minified => {
		...
		if (useEsbuildTranspile) {
			...
			const vscodeTaskPrepare = task.define(`vscode${dashed(platform)}${dashed(arch)}${dashed(minified)}-prepare`, task.series(
				copyCodiconsTask,
