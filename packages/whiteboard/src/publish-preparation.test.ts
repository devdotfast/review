...
describe("resolvePublishWhiteboard", () => {
  ...
  it("reports unreadable records during implicit selection while allowing a healthy explicit UUID", async () => {
    ...
    const healthy = await createWhiteboardDir({
      worktreePath: repo.rootPath,
      baseRef: "main",
      baseCommit: repo.baseCommit,
    });

    const corrupt = await createWhiteboardDir({
      worktreePath: repo.rootPath,
      baseRef: "main",
      baseCommit: repo.baseCommit,
    });
    ...
    await expect(
      resolvePublishWhiteboard(repo.rootPath, undefined),
    ).rejects.toThrow(corrupt.dir);
    await expect(
      resolvePublishWhiteboard(repo.rootPath, healthy.review.uuid),
