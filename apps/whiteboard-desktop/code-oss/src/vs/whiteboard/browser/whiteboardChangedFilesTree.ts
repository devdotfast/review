...
import { buildTree, type ChangedFileElement, type ChangedTreeElement } from "../common/whiteboardChangedFilesModel.js";
    ...
    >
{
  ...
  readonly templateId = ChangedFilesTreeRenderer.TEMPLATE_ID;
	...
	constructor(private readonly counts: Map<string, StructuralLineCounts>, private readonly progress: Map<string, WhiteboardDiffProgressFile>, private readonly states: Map<string, { status: "loading" | "error"; message?: string }>) { }

  renderTemplate(container: HTMLElement): ChangedFilesTreeTemplate {
    const row = append(container, $(".whiteboard-changed-files-row"));
    const icon = append(row, $("span.whiteboard-changed-files-icon"));
    icon.setAttribute("aria-hidden", "true");
    const label = append(row, $("span.whiteboard-changed-files-label"));
		const counts = append(row, $("span.whiteboard-tree-counts"));
  ...
  private renderElements(
    ...
    template.row.classList.toggle("whiteboard-changed-files-folder", !isFile);
		...
		template.row.classList.toggle("whiteboard-file-viewed", done === "viewed");
		template.row.classList.toggle("whiteboard-file-folded", done === "folded");
      ...
      : "whiteboard-changed-files-icon";
		...
		if (isFile) {
			...
			if (element.file.status === 'unchanged') template.counts.textContent = 'Unchanged';
				...
				const added = append(template.counts, $('span.whiteboard-tree-added')); added.textContent = `+${compact(additions)}`;
				const removed = append(template.counts, $('span.whiteboard-tree-removed')); removed.textContent = `−${compact(deletions)}`;
			}
		...
		if (state) template.icon.className = `whiteboard-changed-files-icon codicon codicon-${state.status === "loading" ? "loading codicon-modifier-spin" : "error"}`;
...
export class WhiteboardChangedFilesTree extends Disposable {
  private readonly _onDidOpenFile = this._register(
    new Emitter<WhiteboardDiffFileWire>(),
  );
  readonly onDidOpenFile: Event<WhiteboardDiffFileWire> =
  ...
  private files: readonly WhiteboardDiffFileWire[] = [];
  ...
  private activePath: string | undefined;
	...
	private readonly progress = new Map<string, WhiteboardDiffProgressFile>();
	setProgressFiles(files: readonly WhiteboardDiffProgressFile[]): void {
  ...
  constructor(
    ...
    super();
    container.classList.add("whiteboard-changed-files");
    const treeContainer = append(
      container,
      $(".whiteboard-changed-files-tree"),
    );
  ...
  setFiles(files: readonly WhiteboardDiffFileWire[]): void {
...
function fileStatusIcon(status: WhiteboardDiffFileWire["status"]): ThemeIcon {
