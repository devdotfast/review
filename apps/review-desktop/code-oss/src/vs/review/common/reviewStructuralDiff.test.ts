...
test("head-only search evidence retains native pseudocode bands and UTF-16 highlight positions", () => {
  ...
  const diff = searchStructuralDiff({ kind: "rhs", scope: { repo: "/unused", baseWorktree: { commitId: "base", path: "/unused" }, headWorktree: { commitId: "head", path: "/unused" } }, file: { rhs: { path: "x.ts", oid: "a".repeat(40), mode: "100644" } }, sources: { rhs: source } });
