...
export const WHITEBOARD_SERVER_RESTART_DELAYS = [250, 1_000, 2_000] as const;
...
export const WHITEBOARD_CLIENT_RECONNECT_DELAYS = [
  ...WHITEBOARD_SERVER_RESTART_DELAYS,
  4_000,
] as const;

export interface WhiteboardReconnectOptions {
 ...
 * `WHITEBOARD_CLIENT_RECONNECT_DELAYS` between failures. `connect` receives an
...
export async function reconnectUntilAborted(
  ...
  options: WhiteboardReconnectOptions = {},
): Promise<void> {
  const wait =
    options.wait ??
  ...
  while (!signal.aborted) {
    try {
      ...
      let delay = WHITEBOARD_CLIENT_RECONNECT_DELAYS[attempt++];
      if (delay === undefined) {
        options.onExhausted?.(error);
        delay =
          WHITEBOARD_CLIENT_RECONNECT_DELAYS[
            WHITEBOARD_CLIENT_RECONNECT_DELAYS.length - 1
          ];
      }
      await wait(delay);
    }
