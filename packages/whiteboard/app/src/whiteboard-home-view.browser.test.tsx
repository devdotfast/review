...
import { WhiteboardHome, formatRelativeTime } from "./whiteboard-home-view";
...
describe("WhiteboardHome", () => {
  ...
  it("groups chronologically across repositories and shows origins", async () => {
    ...
    await act(async () =>
      root.render(
        <WhiteboardHome whiteboards={whiteboards} onOpen={() => {}} />,
      ),
    );
    expect(
      [...container.querySelectorAll(".whiteboard-home-whiteboard-title")].map(
        (el) => el.textContent,
      ),
  ...
  it("filters repositories and changes sort order without losing review actions", async () => {
    ...
    const onOpen = vi.fn<(whiteboard: SessionSummary) => void>();

    const onDismiss = vi.fn<(whiteboard: SessionSummary) => Promise<void>>(
      async () => undefined,
    );
    ...
    const titles = () =>
      [
        ...container.querySelectorAll(
          ".whiteboard-home-table-open .whiteboard-home-whiteboard-title",
        ),
    ...
    expect(onDismiss).toHaveBeenCalledWith(whiteboards[0]);
    expect(onOpen).not.toHaveBeenCalled();
    await act(async () =>
      container
        .querySelector<HTMLButtonElement>(".whiteboard-home-table-open")!
        .click(),
    );
    expect(onOpen).toHaveBeenCalledWith(whiteboards[0]);
  });
  ...
  it("puts the scratchpad first, above the reviews and out of their workspaces", async () => {
    ...
    const whiteboard = summary({ sessionId: uuid(1), title: "A review" });
    const onOpen = vi.fn<(whiteboard: SessionSummary) => void>();
    await act(async () =>
      root.render(
        <WhiteboardHome whiteboards={[whiteboard, pad]} onOpen={onOpen} />,
      ),
    );
    ...
    expect(container.querySelectorAll(".whiteboard-home-table")).toHaveLength(
      1,
    );
  ...
  it("opens API reviews grouped by repository without needing a checkout path", async () => {
    ...
    await act(async () =>
      root.render(<WhiteboardHome whiteboards={[item]} onOpen={onOpen} />),
    );
  ...
  it("shows reviews from different repositories in one table", async () => {
    ...
    expect(
      container.querySelectorAll(".whiteboard-home-table tbody tr"),
    ).toHaveLength(3);
  ...
  it("opens the row menu without opening the review and requires confirmation to delete", async () => {
    const whiteboard = summary({ title: "Menu review" });
    const onOpen = vi.fn<(whiteboard: SessionSummary) => void>();

    const onDelete = vi.fn<(whiteboard: SessionSummary) => Promise<void>>(
      async () => undefined,
    );
    ...
    await act(async () => remove.click());
    expect(onDelete).toHaveBeenCalledWith(whiteboard);
    expect(onOpen).not.toHaveBeenCalled();
  });
  ...
  it("deletes a review after an arming click without opening it", async () => {
    ...
    const dismissed = container.querySelector<HTMLButtonElement>(
      ".whiteboard-home-dismissed-toggle",
    );
    ...
    expect(onDelete).toHaveBeenCalledWith(whiteboards[0]);
    expect(onOpen).not.toHaveBeenCalled();
  });
  ...
  it("keeps a successful deletion hidden until the catalog catches up, and allows reimport", async () => {
    const whiteboard = summary({ title: "Pending review" });
    const deletion = Promise.withResolvers<void>();
    ...
    const render = async (whiteboards: SessionSummary[]) =>
      act(async () =>
        root.render(
          <WhiteboardHome
            whiteboards={whiteboards}
  ...
  it("keeps attention actions on native summaries", async () => {
    ...
    const onRestore = vi.fn<(whiteboard: SessionSummary) => Promise<void>>(
      async () => {},
    );
    ...
    expect(onDismiss).toHaveBeenCalledWith(whiteboard);
    expect(onOpen).not.toHaveBeenCalled();
    ...
    await act(async () =>
      container
        .querySelector<HTMLButtonElement>(".whiteboard-home-dismissed-toggle")!
        .click(),
    );
    await act(async () =>
      container
        .querySelector<HTMLButtonElement>(".whiteboard-home-restore")!
        .click(),
    );
    expect(onRestore).toHaveBeenCalledWith(dismissed);
    await render({ ...dismissed, dismissedAt: null });
    expect(
      container.querySelector(".whiteboard-home-table-open")?.textContent,
  ...
  it("hides the delete action when the host does not support deletion", async () => {
    await act(async () =>
      root.render(
        <WhiteboardHome whiteboards={[summary()]} onOpen={() => {}} />,
      ),
    );
    expect(container.querySelector(".whiteboard-home-delete")).toBeNull();
  });
  ...
  it("shows the native snapshot update time in the table", async () => {
    ...
    await act(async () =>
      root.render(
        <WhiteboardHome whiteboards={[whiteboard]} onOpen={() => {}} />,
      ),
    );
