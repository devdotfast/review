...
import { app, protocol, crashReporter, Menu, contentTracing, net } from 'electron';
import minimist from 'minimist';
import { product } from './bootstrap-meta.js';
import { validatedIpcMain } from './vs/base/parts/ipc/electron-main/ipcMain.js';
import { parse } from './vs/base/common/jsonc.js';
import { getUserDataPath } from './vs/platform/environment/node/userDataPath.js';
// Evaluated before `startup()` calls `bootstrapESM()`, so this module and
// everything it imports runs with no NLS message table installed. It must not
// reach `vs/nls.js`, the platform registry, or the configuration registry: in a
// packaged build those throw `!!! NLS MISSING: <n> !!!` into a modal, before any
// window or log exists, and the app never starts. That is why it reads setting
// keys from the import-free `vs/whiteboard/common/whiteboardConfigurationDefaults.ts`
// rather than from `whiteboardConfiguration.ts`, which owns the registry calls.
// Enforced by apps/whiteboard-desktop/scripts/main-bootstrap-imports.test.mjs.
import { migrateWhiteboardUserConfig } from './vs/whiteboard/node/whiteboardUserConfigMigration.js';
import { importWhiteboardUserConfig, type WhiteboardUserConfigImportMode } from './vs/whiteboard/node/whiteboardUserConfigImport.js';
import { registerWhiteboardOptionalExtensionInstaller } from './vs/whiteboard/node/whiteboardOptionalExtensionInstaller.js';
import { writeWhiteboardBootstrapBreadcrumb } from './vs/whiteboard/node/whiteboardBootstrapBreadcrumb.js';
...
registerWhiteboardOptionalExtensionInstaller(validatedIpcMain, {
	userDataPath,
...
try {
	...
	importWhiteboardUserConfig({ userDataPath, log: message => console.log(message) });
} catch (error) {
	console.error('Review could not import VS Code settings and keybindings:', error);
	writeWhiteboardBootstrapBreadcrumb(userDataPath, error);
}
validatedIpcMain.handle('vscode:whiteboardImportUserConfig', async (_event, request: unknown) => {
	const mode = request && typeof request === 'object' ? (request as { mode?: unknown }).mode : undefined;
	if (mode !== 'preview' && mode !== 'apply') {
		throw new Error(`Unsupported Review user-config import mode: ${String(mode)}`);
	}
	return importWhiteboardUserConfig({
		userDataPath,
		mode: mode satisfies WhiteboardUserConfigImportMode,
...
async function onReady() {
	...
	try {
		...
		console.error(error);
		// Nothing is running yet that could report this: no window, no log, no
		// Review server. Leave a note on disk instead; the next launch sends it
		// through the normal opted-in path and deletes it.
		writeWhiteboardBootstrapBreadcrumb(userDataPath, error);
	}
}
