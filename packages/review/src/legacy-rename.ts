...
export function legacyReviewApi() {
...
export async function serveLegacyReviewMcp(stdin: Readable, stdout: Writable) {
...
export async function runLegacyReviewCli(
  argv: string[],
  stdin: Readable,
  stdout: Writable,
  stderr: Writable,
  ...
  if (argv[0] === "mcp") {
    await serveLegacyReviewMcp(stdin, stdout);
