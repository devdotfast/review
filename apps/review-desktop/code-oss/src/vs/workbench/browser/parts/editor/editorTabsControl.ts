...
export abstract class EditorTabsControl extends Themable implements IEditorTabsControl {
	...
	private static readonly EDITOR_TAB_HEIGHT = {
		...
		// Review pill tabs (review/browser/media/review.css) add 6px top + 6px
		// bottom padding to the tabs-and-actions-container. The total must equal
		// REVIEW_CHROME_HEIGHT, because the Review titlebar overlays this row.
	...
	protected updateEditorActionsToolbar(): void {
		...
		const visibleSecondary = IsReviewWindowContext.getValue(this.contextKeyService) ? [] : secondary;
	...
	protected get tabHeight() {
		...
		if (this.parent.classList.contains('tabs') && this.parent.closest('.review-workbench')) {
