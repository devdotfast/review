...
import { mountWhiteboardCanvas } from "./desktop-entry";
import { testWhiteboardBridge } from "./whiteboard-session-test-utils";

let canvas: ReturnType<typeof mountWhiteboardCanvas> | undefined;
...
it("exposes JSON section and Markdown headings plus imported PR and stack navigation", async () => {
    ...
    c.json({
      layers: [
        ...
        },
        {
          ...
          pullRequestUrl: null,
          sessionId: sessionId,
          whiteboardTitle: "Navigation",
          relation: "current",
        },
      ],
    }),
  );
  app.route("/sessions-api", createSessionApi(store));
  ...
  await act(async () => {
    canvas = mountWhiteboardCanvas(container, {
      kind: "api",
      sessionId,
      bridge,
    });
  ...
  const links = [
    ...container.querySelectorAll<HTMLButtonElement>(".whiteboard-toc-link"),
  ];

  expect(
    links.map(
      (link) => link.querySelector(".whiteboard-toc-text")?.textContent,
    ),
