...
import { alignmentRows } from "../common/whiteboardLens.js";
import type { WhiteboardInlineEditorSpec, WhiteboardInlineEditorHandle, WhiteboardFindQuery } from "../common/whiteboardProtocol.js";
import { structuralChangeCounts } from "../common/whiteboardProtocol.js";
import type { WhiteboardDiffProgress, WhiteboardDiffViewport } from "../common/whiteboardProtocol.js";
import { lensRanges, withLens } from "./whiteboardLens.js";
/*---------------------------------------------------------------------------------------------
...
import type {
	WhiteboardCommitScope,
	WhiteboardDiffLens,
	WhiteboardDiffFileWire,
	WhiteboardDiffViewHandle,
	WhiteboardDiffViewSpec,
} from "../common/whiteboardProtocol.js";
import { WhiteboardDiffLayoutSetting } from "./whiteboardDiffLayout.js";
import { markWhiteboardEmbeddedEditor } from "./whiteboardEmbeddedNavigation.js";
import { WhiteboardFilesDiffView, WhiteboardFilesEditorInput, type WhiteboardFilesEditorEntry } from "./whiteboardFilesDiffView.js";
import { WhiteboardEmbeddedEditors } from "./whiteboardEmbeddedEditors.js";

import { createStructuralDiffEditors } from "./whiteboardStructuralDiff.js";
import { StructuralDiffSession } from "./whiteboardStructuralDiffSession.js";
import type { StructuralDiffStream } from "./whiteboardStructuralDiffClient.js";

export interface WhiteboardDiffViewSource {
	load(scope?: WhiteboardCommitScope, lens?: WhiteboardDiffLens): Promise<{
		sourceUri: URI; entries: readonly WhiteboardFilesEditorEntry[];
		session?: StructuralDiffSession;
	}>;
	files(scope?: WhiteboardCommitScope): Promise<readonly WhiteboardDiffFileWire[]>;
}
 ...
 */
export class WhiteboardDiffViewService extends Disposable {
	...
	readonly diffLayout: WhiteboardDiffLayoutSetting;
	/**
	...
	constructor(
		...
		super();
		this.diffLayout = this._register(instantiationService.createInstance(WhiteboardDiffLayoutSetting));
	}

	get structuralRenderingEnabled(): boolean {
		return this.instantiationService.invokeFunction(a => a.get(IConfigurationService).getValue<boolean>(WHITEBOARD_STRUCTURAL_DIFF_SETTING) === true);
	}
	...
	create(spec: WhiteboardDiffViewSpec, source: WhiteboardDiffViewSource): WhiteboardDiffViewHandle {
	...
	createDocument(spec: WhiteboardInlineEditorSpec, lens: WhiteboardDiffLens, source: WhiteboardDiffViewSource): WhiteboardInlineEditorHandle {
	...
	async findDocument(lens: WhiteboardDiffLens, source: WhiteboardDiffViewSource, query: WhiteboardFindQuery): Promise<DocumentMatch[]> {
...
class DiffViewController extends Disposable implements WhiteboardDiffViewHandle {
	...
	private view: WhiteboardFilesDiffView | undefined;
	private progress: WhiteboardDiffProgress | undefined;
	private readonly progressChanged = this._register(new Emitter<void>());
	private pendingSectionId: string | undefined;
	private pendingSource: WhiteboardDiffLens['ranges'][number] | undefined;
	setProgress(progress: WhiteboardDiffProgress): void { this.progress = progress; this.view?.setProgress(progress); this.progressChanged.fire(); }
	revealSource(source: WhiteboardDiffLens['ranges'][number], sectionId?: string): void { this.pendingSource = source; this.pendingSectionId = sectionId; this.view?.revealSource(source, sectionId); }
	private readonly _onDidScroll = this._register(new Emitter<WhiteboardDiffViewport>());
	readonly onDidScroll = this._onDidScroll.event;
	sourceOffset(source: WhiteboardDiffLens['ranges'][number]): number | undefined { return this.view?.sourceOffset(source); }
	...
	constructor(
		private readonly spec: WhiteboardDiffViewSpec,
		private readonly instantiationService: IInstantiationService,
		private readonly inlineEditors: WhiteboardEmbeddedEditors,
		private readonly overflowWidgetsDomNode: HTMLElement | undefined,
		private readonly diffLayout: WhiteboardDiffLayoutSetting,
		private readonly viewStates: Map<string, IMultiDiffEditorViewState>,
		private readonly onDispose: () => void,
		private readonly source: WhiteboardDiffViewSource,
	) {
		super();
	...
	private async initialize(): Promise<void> {
		try {
			...
			const structuralEnabled = session !== undefined && this.instantiationService.invokeFunction(a => a.get(IConfigurationService).getValue<boolean>(WHITEBOARD_STRUCTURAL_DIFF_SETTING) === true);
			...
			if (lens && !sections?.length) {
				...
				selected = orderWhiteboardDiffFiles([...byFile.keys()]).map(file => byFile.get(file)!);
			}
			...
			const input = store.add(instantiation.createInstance(WhiteboardFilesEditorInput, sourceUri, selected,
				structuralEnabled, !!lens || !!this.spec.onToggleViewed));
			const view = store.add(
				instantiation.createInstance(
					WhiteboardFilesDiffView,
					this.spec.container,
					this.overflowWidgetsDomNode,
					this.diffLayout,
	...
	private observeSession(session: StructuralDiffSession, entries: readonly WhiteboardFilesEditorEntry[], view: WhiteboardFilesDiffView, store: DisposableStore, structuralEnabled: boolean): void {
		const rendered = new Set<string>();
		const annotationWarnings = new Set<string>();
		const renderCurrentState = (change?: import("./whiteboardStructuralDiffSession.js").StructuralSessionChange) => {
			if (this.disposed) return;
	 ...
	 */
	private bindActiveControl(view: WhiteboardFilesDiffView): void {
		...
		for (const editor of editors) {
			this.activeControlStore.add(markWhiteboardEmbeddedEditor(editor));
			if (this.spec.document) WhiteboardEmbeddedEditors.markDocumentEditor(editor);
