...
import {
	...
	whiteboardConfigurationDefaults,
} from '../common/whiteboardConfigurationDefaults.js';
import { importWhiteboardUserConfig } from './whiteboardUserConfigImport.js';

describe('WhiteboardUserConfigImport', () => {
	...
	test('imports JSONC keybindings verbatim and filters hardened settings', () => {
		...
		const result = importWhiteboardUserConfig({
	...
	test('does not import twice after writing the stamp', () => {
		...
		assert.strictEqual(importWhiteboardUserConfig(options).status, 'imported');
		writeFileSync(path.join(fixture.sourceUser, 'keybindings.json'), '[{ "key": "x", "command": "x" }]\n');

		const second = importWhiteboardUserConfig(options);
	...
	test('previews manual overwrites without changing them', () => {
		...
		const applied = importWhiteboardUserConfig({
			userDataPath: fixture.target,
			mode: 'apply',
			env: { DEV_WHITEBOARD_IMPORT_FROM: fixture.sourceRoot },
			homeDir: fixture.root,
