...
Write small and often while a reader may be watching: one paragraph, one sequence step, one flow node or one flow edge per edit, so the document draws itself as you go. Wire as you go: insert each flow node with `link:{from:"<node already drawn>"}` (or `to`) so it arrives attached, and use a separate `flow_edge` only between nodes that already exist. Never draw a pile of unconnected nodes and add the edges afterwards; that is not how anyone draws on a whiteboard.
