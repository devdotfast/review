...
export const WHITEBOARD_OPENED_VIA = ["home", "cli", "other"] as const;
...
export const WHITEBOARD_TELEMETRY_TABS = [
  "review",
  "commits",
  "map",
  "files",
 ...
 */
export const WHITEBOARD_APP_SESSION_ID_HEADER = "x-review-app-session-id";
...
export const UI_TELEMETRY_EVENTS = {
  ...
  diff_layout_changed: {
    ...
    properties: { layout: WHITEBOARD_DIFF_LAYOUTS },
  },
  ...
  review_opened: {
    ...
    properties: { via: WHITEBOARD_OPENED_VIA },
  },
  review_presented: {
...
export function sanitizeUiTelemetryEvent(input: {
  ...
  for (const [key, propSpec] of Object.entries(propertySpecs)) {
    ...
    if (propSpec === "opaque_id") {
      if (isValidWhiteboardAppSessionId(text)) properties[key] = text;
      continue;
    }
...
export function isValidWhiteboardAppSessionId(
