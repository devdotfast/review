...
import {
  type WhiteboardPublishEvidenceTargets,
  evaluateWhiteboardDocumentBundleForPublish,
} from "./whiteboard-publish-evaluate";
...
describe("publish range evaluation", () => {
  ...
  it("recovers the former fields accessor when evaluating a sealed database diagram", async () => {
    const result = await evaluateWhiteboardDocumentBundleForPublish({
      whiteboardDir: fixtureDir("legacy-fields"),
      ranges: "skip",
      bundleCode: `import { createBrowserWhiteboardDefinitionSession, createActiveWhiteboardDocument, jsx } from "review-doc-runtime";
        const session = createBrowserWhiteboardDefinitionSession({});
        ...
        export default createActiveWhiteboardDocument({ title: "Old schema access", routePath: "/", filePath: "review.mdx", modelNames: [], models: {},
  ...
  it("reads head and base peeks from their exact pinned worktrees", async () => {
    const whiteboardDir = fixtureDir("review");
    const head = sourceFixture("head line");
    const base = sourceFixture("base line");

    const prepareEvidence = vi.fn<
      () => Promise<WhiteboardPublishEvidenceTargets>
    >(async () => ({
  ...
  it("does not prepare a worktree when the document has no peeks", async () => {
    const prepareEvidence =
      vi.fn<() => Promise<WhiteboardPublishEvidenceTargets>>();

    const result = await evaluateWhiteboardDocumentBundleForPublish({
      whiteboardDir: fixtureDir("review"),
  ...
  it("recovers imported anchors omitted by old sealed document exports, including unused anchors", async () => {
    const result = await evaluateWhiteboardDocumentBundleForPublish({
      ...
      bundleCode: `import { createBrowserWhiteboardDefinitionSession, createActiveWhiteboardDocument, jsx } from "review-doc-runtime";
        ...
        export default createActiveWhiteboardDocument({ title: "Legacy", routePath: "/", filePath: "review.mdx", modelNames: [], models: {}, Component: ({ components }) => jsx(components.AnchorLink, { anchor: anchors.shown, children: "Shown" }), isDefault: true });`,
    });
  ...
  it("captures the explicit legacy repository map pair separately from inline models", async () => {
    const result = await evaluateWhiteboardDocumentBundleForPublish({
      whiteboardDir: fixtureDir("embedded-map"),
      bundleCode: `import { createActiveWhiteboardDocument, defineSoftwareModel, jsx } from "review-doc-runtime";
...
export default createActiveWhiteboardDocument({ title: "Legacy", routePath: "/", filePath: "review.mdx", modelNames: [], models: {}, repoSoftwareMap: head, baseSoftwareMap: base, Component: () => jsx("p", { children: "Legacy" }) });`,
    });
  ...
  it.each([
    ...
    async ({ head, base }) => {
      const result = await evaluateWhiteboardDocumentBundleForPublish({
        whiteboardDir: fixtureDir("invalid-embedded-map"),
        bundleCode: `import { createActiveWhiteboardDocument, defineSoftwareModel, jsx } from "review-doc-runtime";
const head = defineSoftwareModel({ systems: { service: { label: "Head" } } });
export default createActiveWhiteboardDocument({ title: "Legacy", routePath: "/", filePath: "review.mdx", modelNames: [], models: {}, repoSoftwareMap: ${head}, baseSoftwareMap: ${base}, Component: () => jsx("p", { children: "Legacy" }) });`,
      });
      ...
      bundleCode: `
        ...
        createActiveWhiteboardDocument({
          ...
          Component: ({ components }) => React.createElement(
            React.Fragment,
            null,
            React.createElement("h1", null, "Materialized"),
  ...
  it("returns no document after audit or document-schema failures", async () => {
    const auditFailure = await evaluateWhiteboardDocumentBundleForPublish({
      whiteboardDir: fixtureDir("review"),
    ...
    const documentLocalComponent =
      await evaluateWhiteboardDocumentBundleForPublish({
        whiteboardDir: fixtureDir("review"),
        bundleCode: bundleWithDocumentBody(
    ...
    const schemaFailure = await evaluateWhiteboardDocumentBundleForPublish({
      whiteboardDir: fixtureDir("review"),
  ...
  describe("TraceQuote validation", () => {
    ...
    beforeEach(() => {
      ...
      process.env.WHITEBOARD_TEST_TRACE_SEARCH_DIR = fixtureDir("trace-search");
      clearTraceEnvCache();
  ...
  it("serializes concurrent evaluations that share the process-global runtime", async () => {
    const whiteboardDir = fixtureDir("review");
    const events: string[] = [];
    vi.stubGlobal("__whiteboardEvaluationEvents", events);

    const slow = `
      globalThis.__whiteboardEvaluationEvents.push("first:enter");
      ${bundleWithAnchors("")
      ...
      globalThis.__whiteboardEvaluationEvents.push("first:exit");
    `;

    const fast = `
      globalThis.__whiteboardEvaluationEvents.push("second:enter");
      ${bundleWithAnchors("").replace('title: "Fixture"', 'title: "Second"')}
      globalThis.__whiteboardEvaluationEvents.push("second:exit");
    `;
  ...
  it("continues queued evaluations after an evaluation rejects", async () => {
    const whiteboardDir = fixtureDir("review");
    const invalidWhiteboardDir = path.join(whiteboardDir, "not-a-directory");
    fs.writeFileSync(invalidWhiteboardDir, "occupied");

    const failed = evaluateWhiteboardDocumentBundleForPublish({
      whiteboardDir: invalidWhiteboardDir,
      bundleCode: bundleWithAnchors(""),
      ranges: "skip",
    });

    const next = evaluateWhiteboardDocumentBundleForPublish({
      whiteboardDir,
      bundleCode: bundleWithAnchors(""),
      ranges: "skip",
    });
  ...
  it("restores the runtime and cleans temporary files after a document throws", async () => {
    ...
    vi.stubGlobal("__devFastWhiteboardPublishRuntime", previous);
    ...
    try {
      ...
      const holder = globalThis as typeof globalThis & {
        __devFastWhiteboardPublishRuntime?: typeof previous;
      };

      expect(holder.__devFastWhiteboardPublishRuntime).toBe(previous);
      expect(fs.readdirSync(path.join(whiteboardDir, ".build"))).toEqual([]);
    } finally {
      vi.unstubAllGlobals();
    }
  });
