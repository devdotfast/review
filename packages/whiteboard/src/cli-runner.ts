...
import { whiteboardAgentCliHelp } from "./session-api/agent-cli";
...
import {
  type WhiteboardAppEvent,
  runWhiteboardAppPick,
} from "./whiteboard-app";
import {
  type WhiteboardAppLaunchEvent,
...
import {
  emitWhiteboardEvent,
  serializeWhiteboardError,
} from "./whiteboard-logger";
import {
  type WhiteboardCliCommand,
  type WhiteboardCliCommandPath,
  type WhiteboardCommandTelemetry,
  WhiteboardTelemetry,
  type WhiteboardTelemetryErrorCategory,
  type WhiteboardTelemetryErrorName,
} from "./whiteboard-telemetry";

interface WhiteboardCliRuntime {
...
export interface WhiteboardCliInput {
  ...
  stdout: Writable;
  stderr: Writable;
  telemetry?: WhiteboardCommandTelemetry;
  runtime?: Partial<WhiteboardCliRuntime>;
}

interface WhiteboardInfoOptions {
  all?: boolean;
  review?: string;
  session?: string;
}

type OutputSurface = WhiteboardCliCommand | "plain";
...
export async function runWhiteboardCli(
  ...
  const cliVersion =
    input.cliVersion ?? readWhiteboardPackageVersion(import.meta.url);

  const runtime = whiteboardCliRuntime(input.runtime);
  const telemetry = input.telemetry ?? WhiteboardTelemetry.fromEnv(env);
  ...
  configureJsonOutput(
    ...
    const stateDir = whiteboardServerStateDir(authoringEnv(options.stateDir));
  ...
  configureJsonOutput(
    ...
    const stateDir = whiteboardServerStateDir(authoringEnv(options.stateDir));
    const discovery = await readWhiteboardServerDiscovery(stateDir);

    if (!discovery || !(await whiteboardServerIsHealthy(discovery)))
  ...
  const writeAppEvent = (
    event: WhiteboardAppLaunchEvent | WhiteboardAppEvent,
  ...
  const pickWhiteboard = async (options: {
    review?: string;
    session?: string;
    focus?: boolean;
    json?: boolean;
    ...
    const event = await runtime.runWhiteboardAppPick({
      cwd,
  ...
  const launchApp = async (options: { focus?: boolean; json?: boolean }) => {
    const event = await runtime.runWhiteboardAppLaunch({
      focus: options.focus,
    });
  ...
  configureJsonOutput(
    ...
    .action(async (options: WhiteboardInfoOptions) => {
      const event = await runtime.runWhiteboardInfo({
        cwd,
        all: options.all,
        sessionId: options.session,
      });
  ...
  configureJsonOutput(
    ...
    state.exitCode = await runtime.runWhiteboardMigration({
      env,
      force: options.force,
      json: options.json,
      stdout: input.stdout,
  ...
  // The JSON authoring surface is owned by session-api/agent-cli.ts. Commander
  ...
  for (const [name, description] of [
    ...
    configureOutput(
      ...
      const { runWhiteboardAgentCli } =
        await import("./session-api/agent-cli.js");
  ...
  try {
    ...
    if (error instanceof CommanderError) {
      ...
      if (state.json || surface === "review") {
        emitWhiteboardEvent(input.stdout, {
          event: "error",
          error: {
            name: "WhiteboardCliUsageError",
    ...
    if (state.json) {
      const serialized: ReturnType<typeof serializeWhiteboardError> & {
        code?: string;
        remedy?: string;
      } = serializeWhiteboardError(error);
      ...
      emitWhiteboardEvent(input.stdout, { event: "error", error: serialized });
    } else {
...
function whiteboardCliRuntime(
  overrides: Partial<WhiteboardCliRuntime> | undefined,
): WhiteboardCliRuntime {
  return {
    runWhiteboardAppLaunch,
    runWhiteboardAppPick,
    runWhiteboardInfo,
...
): WhiteboardCliCommandPath | undefined {
