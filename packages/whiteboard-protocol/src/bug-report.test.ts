...
import {
  WhiteboardBugReportMetaV2Schema,
  parseWhiteboardBugReportRequest,
} from "./bug-report.js";
...
describe("bug report protocol", () => {
  it("round-trips trace consent and defaults it off for older clients", () => {
    ...
    expect(parseWhiteboardBugReportRequest(request)).toEqual(request);

    const { include_trace: _includeTrace, ...withoutTraceConsent } = request;
    expect(parseWhiteboardBugReportRequest(withoutTraceConsent)).toEqual(
      request,
    );
  });
  ...
  it("accepts ordered schema 2 trace parts", () => {
    expect(WhiteboardBugReportMetaV2Schema.parse(baseMeta)).toEqual(baseMeta);
  });
  ...
  it("accepts a verbatim Unicode and multiline description", () => {
    ...
    expect(WhiteboardBugReportMetaV2Schema.parse(meta)).toEqual(meta);
  });
  ...
  it("rejects an oversized description or a byte-length mismatch", () => {
    expect(() =>
      WhiteboardBugReportMetaV2Schema.parse({
        ...baseMeta,
