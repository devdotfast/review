...
export function ReviewBranchRange({
  ...
  useEffect(() => {
    ...
    setLinks({});
    session
      .fetch("/branch-links", { signal: controller.signal })
      .then(async (response) => {
        ...
        setLinks({
          baseRef: jsonString(json.baseRef),
          headRef: jsonString(json.headRef),
  ...
  }, [fallbackBaseRef, fallbackHeadRef, session]);

  const baseRef = links.baseRef ?? fallbackBaseRef;
  const headRef = links.headRef ?? fallbackHeadRef;
