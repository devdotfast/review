...
# in apps/whiteboard-desktop/README.md.

MONOREPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
APP_DIR="$MONOREPO_ROOT/apps/whiteboard-desktop"
CHECKOUT="$APP_DIR/code-oss"
PROTOCOL_BUNDLER="$MONOREPO_ROOT/packages/whiteboard-protocol/scripts/bundle-native-runtime.mjs"
PROTOCOL_OUTPUT="$CHECKOUT/out/vs/whiteboard/common/whiteboardProtocol.js"
EVENT_STREAM_OUTPUT="$CHECKOUT/out/vs/whiteboard/common/whiteboardEventStream.js"
...
DEV_FAST_ACTIVE=0
if [[ "${WHITEBOARD_DESKTOP_CI_FAST:-0}" != "1" && "${WHITEBOARD_DESKTOP_DEV_FAST:-0}" == "1" ]]; then
  DEV_FAST_ACTIVE=1
fi
...
if [[ -n "${WHITEBOARD_DESKTOP_CURATED_EXTENSION_TARGET:-}" ]]; then
  node "$APP_DIR/scripts/curated-extensions.mjs" \
    "--target=$WHITEBOARD_DESKTOP_CURATED_EXTENSION_TARGET"
...
if [[ "${WHITEBOARD_DESKTOP_COMPILE_ONLY:-0}" != "1" ]]; then
...
TYPECHECK_PID=""
if [[ "${WHITEBOARD_DESKTOP_CI_FAST:-0}" == "1" ]]; then
  ...
  if [[ ! -f "$CHECKOUT/out/main.js" || ! -f "$PROTOCOL_OUTPUT" || ! -f "$EVENT_STREAM_OUTPUT" ]] || \
    needs_rebuild "$CLIENT_STAMP" \
      ...
      "$DEPENDENCY_STAMP" \
      "$MONOREPO_ROOT/packages/whiteboard-protocol/src" \
...
if [[ "$DEV_FAST_ACTIVE" != "1" ]]; then
  if [[ -n "${WHITEBOARD_POSTHOG_KEY:-}" ]]; then
    node "$MONOREPO_ROOT/packages/whiteboard/scripts/embed-posthog-key.mjs"
  fi
  rebuild_review_desktop_outputs "$MONOREPO_ROOT" "$MONOREPO_ROOT/packages/whiteboard"
