...
import { isJsonObject, parseJsonText } from "@dev.fast/whiteboard-protocol";
...
interface WhiteboardToolIdentity {
  tool: string;
  platform: string;
  sha256: string;
}

export function whiteboardToolsRoot(
  env: NodeJS.ProcessEnv = process.env,
): string {
  return path.join(devWhiteboardHome(env), "review-tools");
}
...
async function ensureBundledTool(
  ...
  const platform = whiteboardToolPlatform();
  ...
  const identity: WhiteboardToolIdentity = {
    tool: input.tool,
    platform,
    sha256,
  };
...
export function stagedToolPath(
  env: NodeJS.ProcessEnv,
  tool: string,
  platform = whiteboardToolPlatform(),
): string {
  return path.join(
    whiteboardToolsRoot(env),
    tool,
    "bundled",
    platform,
...
function installedToolMatches(
  destination: string,
  executable: string,
  identity: WhiteboardToolIdentity,
): boolean {
...
function whiteboardToolPlatform(): string {
  const platform =
