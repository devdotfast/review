...
import { createGlobalWhiteboardServer } from "../server/desktop-server.js";
import { GlobalWhiteboardDesktopVerbRelay } from "../server/global-verb-relay.js";
...
describe("snapshot authoring", () => {
  ...
  it("deletes one review and its history, keeps other reviews, and cannot replay deleted content", async () => {
    ...
    await store.close();
    store = new SessionStore(database, providers);
    expect(store.list().map((whiteboard) => whiteboard.sessionId)).toEqual([
      other.sessionId,
    ]);
  ...
  it("persists attention without creating a document version or notifying its readers", async () => {
    ...
    store = new SessionStore(database, providers);
    expect(
      store.list().find((whiteboard) => whiteboard.sessionId === sessionId),
    ).toMatchObject({
    ...
    expect(
      store
        .list()
        .find((whiteboard) => whiteboard.sessionId === other.sessionId),
    ).toMatchObject({
      viewedAt: null,
      dismissedAt: null,
    });
...
describe("create for a pull request", () => {
  ...
  it("returns the PR's existing review, unchanged, instead of making another", async () => {
    ...
    expect(again.otherWhiteboardIds).toBeUndefined();
  ...
  it("creates another review on request and then returns the newest, naming the rest", async () => {
    ...
    expect(found).toMatchObject({
      ...
      otherWhiteboardIds: [oldest.sessionId],
    });
    ...
    expect(await createFor(url)).toMatchObject({
      ...
      otherWhiteboardIds: [newer.sessionId],
    });
    expect(store.list()).toHaveLength(2);
  });
...
it("serves the experiment through the real desktop HTTP server and existing authentication", async () => {
  ...
  const server = createGlobalWhiteboardServer({
    ...
    whiteboardData: new LocalSessionData(store),
    relay,
  });
  ...
  try {
    ...
    relay.attach({
      ...
      write(frame) {
        ...
        relay.acceptResult({
          id,
          response:
            request.name === "openApiWhiteboard" &&
    ...
    const whiteboardsOnly = (entries: { kind?: string }[]) =>
      entries.filter((entry) => entry.kind !== "scratchpad");

    expect(whiteboardsOnly(listed)).toMatchObject([{ sessionId }]);
    ...
    expect(whiteboardsOnly((await catalog.next()).value)).toMatchObject([
...
it("keeps reference coverage apart by pins: one path, changed under one comparison and not another", async () => {
  ...
  const progress = await whiteboardProgress(store, data, store.read(sessionId));
...
it("preserves unchanged partial file coverage across pins and rejects stale writes after either file side changes", async () => {
  ...
  const initial = await whiteboardProgress(store, data, store.read(sessionId));
  ...
  expect(
    (await whiteboardProgress(store, data, store.read(sessionId))).files[0]
      .viewed.head,
  ).toEqual([[1, 2]]);
  ...
  const next = await whiteboardProgress(store, data, store.read(sessionId));
  expect(
...
it("resolves file lenses to whole changed files, preserves empty groups, and shares viewed coverage", async () => {
  ...
  const viewed = await whiteboardProgress(store, data, store.read(sessionId));
...
it("validates range lens evidence and scopes progress and Uncategorized to distinct changed lines", async () => {
  ...
  const result = await whiteboardProgress(store, data, store.read(sessionId));
  ...
  const stale = await whiteboardProgress(store, data, store.read(sessionId));
