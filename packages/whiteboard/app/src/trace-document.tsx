import type { WhiteboardAgentTraceEvent } from "@dev.fast/whiteboard-protocol";
import {
  type ReactNode,
  useEffect,
  useLayoutEffect,
...
export function buildIndexedTraceTurns(
  events: WhiteboardAgentTraceEvent[],
...
export type IndexedTraceToolItem = {
  event: Extract<WhiteboardAgentTraceEvent, { kind: "tool" }>;
  index: number;
};
...
export function toolGroupLabel(
  events: Array<Extract<WhiteboardAgentTraceEvent, { kind: "tool" }>>,
): string {
...
export function TraceToolGroup({
  ...
  return (
    <details
      className="whiteboard-trace-toolgroup"
      open={hasTarget}
      data-trace-event={items[0]?.index}
    >
      <summary>
        <span className="whiteboard-trace-tool-icon">
          {iconSvg(
            <>
              <rect
                x="2"
        ...
        <span className="whiteboard-trace-toolgroup-label">
          {toolGroupLabel(events)}
        </span>
        <span className="whiteboard-trace-toolgroup-count">
          {events.length} steps
        </span>
        <span className="whiteboard-trace-toolgroup-chevron" aria-hidden="true">
          <ChevronIcon />
        </span>
      </summary>
      <div className="whiteboard-trace-toolgroup-body">
        {items.map((item) => (
          <div
            key={item.index}
            id={
...
export function TraceEvent({
  ...
  if (event.kind === "assistant") {
    if (event.thinking) {
      return (
        <details
          className="whiteboard-trace-tool whiteboard-trace-tool--expandable"
          open={Boolean(highlightQuote)}
        >
          <summary>
            <span className="whiteboard-trace-tool-row">
              <span className="whiteboard-trace-tool-icon">
                {toolIcon("Thinking")}
              </span>
              <span className="whiteboard-trace-tool-label">
                <span className="whiteboard-trace-tool-verb">Thinking</span>
              </span>
              <span
                className="whiteboard-trace-tool-chevron"
                aria-hidden="true"
              >
                <ChevronIcon />
              </span>
          ...
          <figure className="whiteboard-trace-figure">
            <figcaption className="whiteboard-trace-figure-head">
              <span>Thinking</span>
            </figcaption>
            <div className="whiteboard-trace-figure-body whiteboard-trace-figure-body--thinking">
              <AgentMarkdown
    ...
    return (
      <div className="whiteboard-trace-prose">
        <AgentMarkdown
          source={event.markdown}
          highlightQuote={highlightQuote}
        />
...
export function TraceToolRow({
  ...
  const row = (
    <span className="whiteboard-trace-tool-row">
      <span className="whiteboard-trace-tool-icon">{toolIcon(event.verb)}</span>
      <span className="whiteboard-trace-tool-label">
        <span className="whiteboard-trace-tool-verb">{event.verb}</span>
        <span
          className={
            event.filePath
              ? "whiteboard-trace-tool-title whiteboard-trace-tool-title--file"
              : "whiteboard-trace-tool-title"
          }
        >
          {event.filePath ? <bdi>{event.title}</bdi> : event.title}
        </span>
        {event.additions !== undefined && event.additions > 0 && (
          <span className="whiteboard-trace-added">+{event.additions}</span>
        )}
        {event.deletions !== undefined && event.deletions > 0 && (
          <span className="whiteboard-trace-removed">−{event.deletions}</span>
        )}
        {event.error && (
          <span className="whiteboard-trace-error-flag">failed</span>
        )}
      </span>
      {expandable && (
        <span className="whiteboard-trace-tool-chevron" aria-hidden="true">
          <ChevronIcon />
        </span>
      )}
    </span>
  ...
  if (!expandable) {
    return <div className="whiteboard-trace-tool">{row}</div>;
  }

  return (
    <details className="whiteboard-trace-tool whiteboard-trace-tool--expandable">
      <summary>{row}</summary>
      <figure className="whiteboard-trace-figure">
        <figcaption className="whiteboard-trace-figure-head">
          <span>{event.command ? "Shell" : event.tool}</span>
        </figcaption>
        <pre className="whiteboard-trace-figure-body">
          {event.command && (
            <code className="whiteboard-trace-figure-command">
              {event.command}
            </code>
          )}
          ...
          {event.output && (
            <code className="whiteboard-trace-figure-output">
              {event.output}
            </code>
          )}
        </pre>
...
function iconSvg(children: ReactNode): ReactNode {
  return (
    <svg
      viewBox="0 0 16 16"
      className="whiteboard-trace-icon"
      aria-hidden="true"
    >
      {children}
    </svg>
...
export function extractEventText(
  event: WhiteboardAgentTraceEvent | undefined,
): string {
  if (!event) return "";
...
export function ElidedMessage({
  ...
  event: Extract<WhiteboardAgentTraceEvent, { kind: "user" | "assistant" }>;
  keep: string[];
  quote?: string;
  onExpand: () => void;
}) {
  ...
  const body = (
    <span className="whiteboard-trace-lens-elided">
      {segments.map((segment, index) =>
        segment.kind === "kept" ? (
          <span key={index} className="whiteboard-trace-lens-kept">
            <mark className="whiteboard-trace-quote-mark">{segment.text}</mark>
          </span>
        ) : (
          <button
            key={index}
            type="button"
            className="whiteboard-trace-lens-chip"
    ...
    return (
      ...
      >
        ...
        <figure className="whiteboard-trace-figure">
          ...
          <div className="whiteboard-trace-figure-body whiteboard-trace-figure-body--thinking">
            <div className="whiteboard-trace-prose whiteboard-trace-prose--elided">
              {body}
            </div>
          </div>
        </figure>
...
export function TraceGapChip({
  ...
  return (
    <button
      key={`gap-${from}`}
      type="button"
      className="whiteboard-trace-lens-gap"
      data-trace-gap={from}
      onClick={onExpand}
    >
      <span className="whiteboard-trace-lens-gap-line" />
      <span className="whiteboard-trace-lens-gap-chip">
...
export function TraceCollapseRow({
  ...
  return (
    <button
      type="button"
      className="whiteboard-trace-lens-collapse"
      onClick={onCollapse}
    >
      <span className="whiteboard-trace-lens-collapse-line" />
      <span className="whiteboard-trace-lens-collapse-chip">
        <svg
          viewBox="0 0 16 16"
          className="whiteboard-trace-lens-collapse-chevron"
          aria-hidden="true"
        >
          {edge === "top" ? (
...
export function TraceTurn({
  ...
  const renderTurnEvent = (item: IndexedTraceTurnEvent) => {
    ...
    return (
      <div
        key={item.index}
        id={isTarget ? "whiteboard-trace-target-event" : undefined}
        className={isTarget ? "whiteboard-trace-target-turn" : undefined}
      ...
      userElement =
        topRow || bottomRow ? (
          <div
            ...
            className="whiteboard-trace-user-slot"
          >
            {topRow}
  ...
  if (turn.work.length > 0) {
    ...
    if (includedWorkCount === 0 && effectiveIncluded !== null) {
      ...
      workElement = (
        <details className="whiteboard-trace-worked" open={openWorked}>
          <summary>
            <span className="whiteboard-trace-worked-label">{workedLabel}</span>
            <span
              className="whiteboard-trace-worked-chevron"
              aria-hidden="true"
            >
              <ChevronIcon />
            </span>
            <span className="whiteboard-trace-worked-line" />
          </summary>
          <div className="whiteboard-trace-worked-body">
            {renderWorkItems()}
          </div>
        </details>
      );
  ...
  return (
    <div className="whiteboard-trace-turn">
      {userElement}
      {workElement}
      {renderFinalItems()}
    </div>
...
export function TraceDocument({
  ...
  useEffect(() => {
    ...
    const targetTurn = document.getElementById("whiteboard-trace-target-event");
    const quoteMark = targetTurn?.querySelector(".whiteboard-trace-quote-mark");
  ...
  return (
    <div
      ...
      className={
        className
          ? `whiteboard-trace-events ${className}`
          : "whiteboard-trace-events"
      }
    >
      {turnElements}
    </div>
