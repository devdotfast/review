...
export async function serveReviewMcp(
  ...
  const server = new Server(
    { name: "review", version: "1.0.0" },
    {
      ...
      instructions:
        "Author through the running Review server. Read review_capabilities before authoring; call review_open only when desktopAvailable is true, dispatch software-map workers only when softwareMapEnabled is true, and name the scratchpad only when scratchpadEnabled is true. Accepted edits are validated and saved immediately. Never read or write Review files or SQL. Reuse commandId and identical input after a lost response. Use returned target IDs to edit components; there is no expectedVersion or publish step.",
    },
  );
