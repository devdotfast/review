...
export class HistoryService extends Disposable implements IHistoryService {
	...
	private explicitNavigationSource: { groupId: GroupIdentifier; editor: EditorInput; selection: IEditorPaneSelection | undefined } | undefined;
	private explicitNavigationDestination: { groupId: GroupIdentifier; editor: EditorInput; selection: IEditorPaneSelection | undefined } | undefined;
	private explicitNavigationTarget: { groupId: GroupIdentifier; editor: EditorInput | IResourceEditorInput; selection: IEditorPaneSelection | undefined } | undefined;
	...
	updateContextKeys(): void {
		this.contextKeyService.bufferChangeEvents(() => {
			...
			this.canNavigateForwardContextKey.set(!!this.explicitNavigationTarget || activeStack.canGoForward(GoFilter.NONE));
	...
	addActiveEditorNavigation(): void {
		this.explicitNavigationSource = undefined;
		this.explicitNavigationDestination = undefined;
		this.explicitNavigationTarget = undefined;
		const activeEditorGroup = this.editorGroupService.activeGroup;
		const activeEditorPane = activeEditorGroup.activeEditorPane;
		if (isEditorPaneWithSelection(activeEditorPane)) {
			const selection = activeEditorPane.getSelection();
			this.getStack(activeEditorGroup, activeEditorPane.input).addActiveEditorNavigation(activeEditorPane);
			if (activeEditorPane.input) {
				this.explicitNavigationSource = {
	...
	completeActiveEditorNavigation(groupId: GroupIdentifier, editor: EditorInput, selection: IEditorPaneSelection | undefined): void {
		if (this.explicitNavigationSource && !this.explicitNavigationDestination) {
			this.explicitNavigationDestination = {
				groupId,
				editor,
				selection,
			};
	...
	async goBackToActiveEditorNavigation(): Promise<'none' | 'source' | 'destination'> {
		...
		if (!activeEditorPane.input.matches(destination.editor)) {
			const targetEditor = this.editorHelper.preferResourceEditorInput(activeEditorPane.input);
			if (!targetEditor) {
				this.explicitNavigationSource = undefined;
				this.explicitNavigationDestination = undefined;
				return 'none';
			...
			this.explicitNavigationSource = undefined;
			this.explicitNavigationDestination = undefined;
			this.explicitNavigationTarget = {
				groupId: activeEditorPane.group.id,
				editor: targetEditor,
				selection: activeSelection,
			};
			await this.editorService.openEditor(destination.editor, destination.selection?.restore({}), destination.groupId);
			this.updateContextKeys();
			return 'destination';
		}
		this.explicitNavigationSource = undefined;
		...
		if (activeEditorPane?.input) {
			...
			if (targetEditor) {
				this.explicitNavigationTarget = {
					...
					editor: targetEditor,
					selection: isEditorPaneWithSelection(activeEditorPane) ? activeEditorPane.getSelection() : undefined,
				};
			}
		}
		await this.goBack(GoFilter.NONE);
		await this.editorService.openEditor(source.editor, source.selection?.restore({}), source.groupId);
		return 'source';
	}

	async goForwardToActiveEditorNavigation(): Promise<boolean> {
		const target = this.explicitNavigationTarget;
		this.explicitNavigationTarget = undefined;
		if (!target) {
			return false;
		}
		const options = target.selection?.restore({});
		if (isEditorInput(target.editor)) {
			await this.editorService.openEditor(target.editor, options, target.groupId);
		} else {
			await this.editorService.openEditor({
				...target.editor,
				options: { ...target.editor.options, ...options },
			}, target.groupId);
		}
		this.updateContextKeys();
		return true;
...
interface IEditorNavigationStacks extends IDisposable {
	...
	addActiveEditorNavigation(editorPane: IEditorPaneWithSelection): void;
...
class EditorNavigationStacks extends Disposable implements IEditorNavigationStacks {
	...
	addActiveEditorNavigation(editorPane: IEditorPaneWithSelection): void {
		...
		const selection = editorPane.getSelection();
		this.selectionsStack.addOrReplace(editorPane.group.id, editorPane.input, selection, false, true);
		this.navigationsStack.addOrReplace(editorPane.group.id, editorPane.input, selection, false, true);
	}
...
${entryLabels.join('\n')}
	...
	addOrReplace(groupId: GroupIdentifier, editorCandidate: EditorInput | IResourceEditorInput, selection?: IEditorPaneSelection, forceReplace?: boolean, forceAdd?: boolean): void {
		...
		if (this.current) {
			...
			} else if (!forceAdd && this.shouldReplaceStackEntry(this.current, { groupId, editor: editorCandidate, selection })) {
