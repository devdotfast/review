  ...
  if (pointer === "s3") {
    if (!hasS3) {
      return {
        ...
        s3,
        hosted,
        error: `S3 trace storage is selected but no bucket credentials are configured. Run \`review trace storage use s3 --endpoint <url> --bucket <name> --key <id> --secret <secret>\` or Review Agent Setup.`,
      };
    }
