...
export async function runCliBootstrap(
  ...
  async function runCli(effectivePath: string): Promise<number> {
    if (!supportedNodeRuntime()) {
      process.stderr.write(
        `${product === "whiteboard" ? "Whiteboard" : "Review"} needs Node.js 24 or newer; found ${process.versions.node}. ` +
          `Update Node, or use the ${product} command installed by Desktop.\n`,
      );
  ...
  async function maybeDelegateToDesktopCli(
    ...
    try {
      ...
      cliPath =
        product === "whiteboard"
          ? path.join(path.dirname(discoveredCliPath), "whiteboard-cli.js")
          : discoveredCliPath;
