...
const WHITEBOARD_BRANCH = "refs/heads/main";

const WHITEBOARD_AUTHOR = {
...
export interface WhiteboardVcsLogEntry {
  oid: string;
  message: string;
...
export interface WhiteboardVcs {
...
export const whiteboardVcs: WhiteboardVcs = {
  async init(dir) {
  ...
  async seal(dir, message) {
    ...
    return git.commit({
      fs,
      dir,
      ref: WHITEBOARD_BRANCH,
      parent: parent ? [parent] : [],
      message,
      author: { ...WHITEBOARD_AUTHOR, timestamp },
      committer: { ...WHITEBOARD_AUTHOR, timestamp },
    });
  },
  ...
  async log(dir) {
    ...
    try {
      commits = await git.log({ fs, dir, ref: WHITEBOARD_BRANCH });
    } catch {
      return [];
    }
