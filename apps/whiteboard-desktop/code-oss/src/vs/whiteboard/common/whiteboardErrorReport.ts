...
export interface WhiteboardErrorReport {
 ...
 * Review.
 */
export function packWhiteboardError(error: unknown): WhiteboardErrorReport | undefined {
 ...
 */
export class WhiteboardErrorReportLimiter {
	 ...
	 */
	report(error: unknown, send: (report: WhiteboardErrorReport) => void): void {
		if (this.reporting) {
			return;
		}
		this.reporting = true;
		try {
			...
			const packed = packWhiteboardError(error);
	...
	private isRepeat(report: WhiteboardErrorReport): boolean {
		const key = `${report.name}\n${report.stack.split('\n', 2).join('\n')}`;
		const time = this.now();
		const repeat = key === this.previousKey && time - this.previousTime <= WhiteboardErrorReportLimiter.REPEAT_WINDOW_MS;
