...
it("repairs missing registrations and launcher automatically for a previously enabled install", async () => {
  ...
  await rm(whiteboardMcpLauncher(env));
  expect(
...
it.each(["cursor", "opencode"] as const)(
  ...
  "${key}": { "other": { "command": "other" }, },
    ...
    const server = settings[key].whiteboard;
    ...
    const { stdout } = await promisify(execFile)(command, args, {
      env: {
        ...env,
        DEV_WHITEBOARD_SERVER_DIR: "other-server",
        ...environment,
      },
    });
    ...
    expect(
      parseJsonc(await readFile(registration.configPath, "utf8"))[key]
        .whiteboard,
    ).toEqual(server);
...
it.each(["cursor", "opencode"] as const)(
  ...
  async (target) => {
    ...
    settings[key].whiteboard.enabled = false;
...
it("keeps OpenCode ownership when a JSONC config is added later", async () => {
  ...
  expect(
    parseJsonc(await readFile(registration.configPath, "utf8")).mcp.whiteboard,
  ).toBeUndefined();
});
...
it("uses OpenCode's existing JSONC config under XDG_CONFIG_HOME", async () => {
  ...
  expect(parseJsonc(source).mcp.whiteboard.command).toEqual([
...
for (const target of targets) {
  it(`migrates an unchanged legacy ${target} registration without touching other servers`, async () => {
    await install();
    const stamp = (await readCliInstallStamp(cliInstallStampPath(env)))!;
    const desired = await whiteboardMcpRegistration(target, homeDir, env);
    await writeWhiteboardMcpRegistration(desired, desired, true);
    ...
    await writeWhiteboardMcpRegistration(legacy);
  ...
  it(`preserves a conflicting ${target} Whiteboard server during legacy migration`, async () => {
    ...
    await writeWhiteboardMcpRegistration(foreign);
    const before = await readFile(desired.configPath, "utf8");
    expect(await writeWhiteboardMcpRegistration(desired, legacy)).toBe(false);
