    ...
    async (focus, requests) => {
      ...
      await expect(
        runWhiteboardAppLaunch(
          { focus },
          {
            readWhiteboardDesktopDiscovery: async () => discovery,
            fetch,
            launchDesktop,
          },
        ),
  ...
  it("reports the launch method and recovery after an early exit", async () => {
    await expect(
      runWhiteboardAppLaunch(
        { timeoutMs: 1_000 },
        {
          readWhiteboardDesktopDiscovery: async () => null,
      ...
      'Could not launch Whiteboard with the macOS bundle identifier "dev.fast.review": the launch process exited with code 1. Open Whiteboard once, then run `whiteboard app launch` again.',
    );
  });
