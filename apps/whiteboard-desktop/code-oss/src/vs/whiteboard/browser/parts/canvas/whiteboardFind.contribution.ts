...
const WHITEBOARD_FIND_COMMAND_ID = "whiteboard.action.find";

class WhiteboardFindAction extends Action2 {
  constructor() {
    super({
      id: WHITEBOARD_FIND_COMMAND_ID,
      title: localize2("whiteboard.action.find", "Find in session"),
      keybinding: {
        weight: KeybindingWeight.WorkbenchContrib,
        primary: KeyMod.CtrlCmd | KeyCode.KeyF,
        when: ActiveEditorContext.isEqualTo(WhiteboardCanvasEditorInput.EDITOR_ID),
      },
    });
  }
  ...
  override async run(accessor: ServicesAccessor): Promise<void> {
    ...
    if (pane instanceof WhiteboardCanvasEditorPane) {
...
registerAction2(WhiteboardFindAction);
