...
import { relativeImportPath, whiteboardModelModulePath } from "./package-paths";
import {
...
export function localizeModelImport(input: {
  ...
  const modelPath = whiteboardModelModulePath(
 ...
 * the CLI (`whiteboard map check`). Stored notes must be location-independent, so
...
export async function flushScratch(input: {
  ...
  if (mapSource === null) {
    throw new Error(
      `No scratch exists for ${input.commit}. Run whiteboard map open first.`,
    );
  }
...
/**
 * The write gate (`whiteboard map check`) is strict, so a note that fails strict
...
function vendoredModelTypes(): string {
  return `// Generated by \`whiteboard map open\`. Ambient declarations so the scratch
