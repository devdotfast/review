  ...
  return [
    ...
    ...commandSchema.shape.operation.options.map((operation) => {
      ...
      return tool(
        ...
        `${descriptions[type]} Reuse commandId with identical input after a lost response. For content changes to an owned review, include the leaseId from review_activity.`,
        z.strictObject({
          ...fields,
