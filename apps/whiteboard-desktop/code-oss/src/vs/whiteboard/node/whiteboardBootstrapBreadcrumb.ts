...
export interface WhiteboardBootstrapBreadcrumb {
	readonly t: number;
...
export function whiteboardBootstrapBreadcrumbPath(userDataPath: string): string {
	return path.join(userDataPath, BREADCRUMB_FILE);
}

/** Append one crash note. Silent on every failure. */
export function writeWhiteboardBootstrapBreadcrumb(userDataPath: string, error: unknown, now: number = Date.now()): void {
	try {
		...
		const breadcrumb: WhiteboardBootstrapBreadcrumb = {
			t: now,
 ...
 */
export function drainWhiteboardBootstrapBreadcrumbs(userDataPath: string): WhiteboardBootstrapBreadcrumb[] {
	const file = whiteboardBootstrapBreadcrumbPath(userDataPath);
	let contents: string;
	try {
	...
	const breadcrumbs: WhiteboardBootstrapBreadcrumb[] = [];
	for (const line of contents.split('\n')) {
		if (!line) {
			continue;
		}
		try {
			const parsed = JSON.parse(line) as WhiteboardBootstrapBreadcrumb;
