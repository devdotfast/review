import type { WhiteboardDiffLens } from "@dev.fast/whiteboard-protocol";
import {
  type ReactNode,
  createContext,
  useContext,
...
import type { SessionApiClient } from "../../src/session-api/client";
import {
  type Lens,
  UNCATEGORIZED_LENS_ID,
} from "../../src/session-api/diff-lenses";
...
interface Lenses {
  progress: WhiteboardProgress | null;
  lenses: WhiteboardProgress["lenses"];
  ...
  active: WhiteboardDiffLens | undefined;
  select(id: string, sources?: WhiteboardDiffLens["ranges"]): void;
  clear(): void;
...
export type WhiteboardLensView = Pick<Lenses, "progress" | "resolve" | "error">;

const Context = createContext<Lenses | null>(null);

export const useWhiteboardLenses = () => useContext(Context);

export function WhiteboardLensesProvider({
  client,
  snapshot,
  ...
  const [progress, setProgress] = useState<WhiteboardProgress | null>(null);
  ...
  const lenses: WhiteboardProgress["lenses"] = progress?.lenses ?? [
  ...
  const mark: Lenses["mark"] = async (
    ...
    try {
      const next = await client.post<WhiteboardProgress>(route, {
        version: snapshot.version,
        mode,
        files,
        viewed,
