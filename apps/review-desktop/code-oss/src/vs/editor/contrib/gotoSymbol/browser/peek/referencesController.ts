...
export abstract class ReferencesController implements IEditorContribution {
	...
	toggleWidget(range: Range, modelPromise: CancelablePromise<ReferencesModel>, peekMode: boolean): void {
		...
		// inline review snippets are too small for a peek widget. Open the resource in the
		// workbench editor and let its controller show the peek at full size.
		if (this._editor.getDomNode()?.closest('.review-inline-code-editor')) {
			this._redirectToWorkbenchEditor(range, modelPromise, peekMode);
			return;
		}
	...
	private _redirectToWorkbenchEditor(range: Range, modelPromise: CancelablePromise<ReferencesModel>, peekMode: boolean): void {
		...
		this._editorService.openCodeEditor({
			...
			if (!openedEditor || openedEditor === this._editor) {
				// no different editor to hand off to
				modelPromise.cancel();
				return;
			}
			...
			// the open handler can remap the lines, so the opened editor knows the true anchor
			const anchorRange = openedEditor.getSelection() ?? range;
			other.toggleWidget(anchorRange, modelPromise, peekMode);
