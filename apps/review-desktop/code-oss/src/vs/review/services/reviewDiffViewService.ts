...
class DiffViewHandle extends Disposable implements ReviewDiffViewHandle {
	...
	private async initialize(): Promise<void> {
		try {
			const { sourceUri, entries } = await this.source.load(this.spec.scope);
			...
			const input = store.add(this.instantiationService.createInstance(ReviewFilesEditorInput, sourceUri, entries));
	...
	private bindActiveControl(view: ReviewFilesDiffView): void {
		...
		const editors: readonly ICodeEditor[] = unified
			? [unified]
