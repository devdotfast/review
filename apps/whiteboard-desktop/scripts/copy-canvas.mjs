...
export function canvasTargets(args, appRoot = appDirectory) {
  const targets = [path.join(appRoot, "code-oss/out/vs/whiteboard/canvas")];
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    targets.push(path.join(packagedAppRoot, "out/vs/whiteboard/canvas"));
  }
...
export function canvasLoaderSource({ canvasFile, wasmFile, stylesheets }) {
  return [
    `export { clearWhiteboardViewState, mountWhiteboardCanvas } from ${JSON.stringify(`./${canvasFile}`)};`,
    `export const whiteboardWasmUrl = new URL(${JSON.stringify(`./${wasmFile}`)}, import.meta.url).href;`,
    `export const whiteboardStylesheetUrls = ${JSON.stringify(stylesheets.map((file) => `./${file}`))}.map(file => new URL(file, import.meta.url).href);`,
    "",
  ].join("\n");
}
