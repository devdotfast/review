...
| --- | --- | --- |
| Claude Code | plugin with the `review` MCP server | `/plugin marketplace add devdotfast/whiteboard`, then `/plugin install review@devfast` |
| Codex | plugin with the `review` MCP server | `codex plugin marketplace add devdotfast/whiteboard`, then `codex plugin add review@devfast` |
