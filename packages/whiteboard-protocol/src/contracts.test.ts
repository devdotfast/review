...
describe("WhiteboardCliInstallStampSchema", () => {
  it("parses a legacy stamp and drops its agent records", () => {
    const stamp = WhiteboardCliInstallStampSchema.parse({
      consent: "granted",
      fingerprint: "abc",
      targets: ["claude", "codex"],
      shimPath: "/home/u/.local/bin/whiteboard",
      mcpRegistrations: [
        {
          target: "claude",
          configPath: "/x",
...
describe("WhiteboardCliInstallStatusSchema", () => {
  it("requires connect prompts and the legacy skill list", () => {
    const result = WhiteboardCliInstallStatusSchema.safeParse({
      fingerprint: "f",
      stamp: null,
      stale: false,
      updateNeeded: false,
