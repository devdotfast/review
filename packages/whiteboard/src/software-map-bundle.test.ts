...
describe("software map bundle", () => {
  ...
  it("writes head and base maps as JSON and reads them back", async () => {
    ...
    const bundle = bundleWhiteboardSoftwareMap({
      head,
      base,
      headCommit: "a".repeat(40),
      baseCommit: "b".repeat(40),
    ...
    await writeWhiteboardSoftwareMapBundle(directory, bundle);

    const read = await readWhiteboardSoftwareMapBundle(directory);
  ...
  it("returns null for a version-1 (JavaScript) bundle", async () => {
    directory = await mkdtemp(path.join(tmpdir(), "review-map-bundle-"));
    const bundleDir = path.join(directory, WHITEBOARD_SOFTWARE_MAP_BUNDLE_DIR);
    ...
    await expect(
      readWhiteboardSoftwareMapBundle(directory),
    ).resolves.toBeNull();
  });
});
