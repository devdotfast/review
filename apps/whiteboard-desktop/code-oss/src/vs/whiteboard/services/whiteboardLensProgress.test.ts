...
import type { WhiteboardDiffProgress } from '../common/whiteboardProtocol.js';
import type { WhiteboardFilesEditorEntry } from './whiteboardFilesDiffView.js';
import { withLens } from './whiteboardLens.js';
...
test('progress invalidates only relevant file ranges and coalesces a burst', async () => {
	...
	let progress: WhiteboardDiffProgress = { files: [file('a.ts'), file('b.ts')] };
	const original = URI.parse('test:/base/a.ts'), modified = URI.parse('test:/head/a.ts');
	const entry = { original, modified, file: { path: 'a.ts' } } as WhiteboardFilesEditorEntry;
