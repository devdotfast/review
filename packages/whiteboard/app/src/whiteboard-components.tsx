...
import {
  useOptionalWhiteboardPanelStore,
  useWhiteboardPanel,
...
import type { WhiteboardSectionSummary } from "./whiteboard-section-summary";
import { useWhiteboardUiState } from "./whiteboard-ui-state";
...
function WhiteboardPanelFrame({
  ...
  const appRef = useWhiteboardRoots()?.appRef;
  ...
  return (
    ...
    >
      ...
      <div
        ref={bodyRef}
        className="whiteboard-panel-body"
...
export function WhiteboardSection({
  ...
  children,
  summary,
}: WhiteboardComponentProps<"WhiteboardSection"> & {
  stateKey?: string;
  id?: string;
  summary?: WhiteboardSectionSummary;
  children?: ReactNode;
}) {
  const [collapsed, setCollapsed] = useWhiteboardUiState(
    stateKey ?? title,
    defaultCollapsed,
    {
      scope: "session",
  ...
  useEffect(() => {
    ...
    sectionElement.addEventListener("whiteboard-section-expand", expand);

    return () => {
      sectionElement.removeEventListener("whiteboard-section-expand", expand);
    };
  }, []);
  ...
  return (
    <section
      className={
        collapsed
          ? "whiteboard-section whiteboard-section--collapsed"
          : "whiteboard-section"
      }
    ...
    >
      <div className="whiteboard-section-header">
        <button
          type="button"
          className="whiteboard-section-toggle"
        ...
        <div className="whiteboard-section-heading">
          <h2 id={id} data-whiteboard-copy-prose>
            {title}
          </h2>
        </div>
        {collapsed && summary && (
          <span className="whiteboard-section-meta">
            {whiteboardSectionSummaryLabel(summary)}
          </span>
        )}
      </div>
      <div
        ref={bodyRef}
        className="whiteboard-section-body"
...
function whiteboardSectionSummaryLabel(
  summary: WhiteboardSectionSummary,
): string {
  const parts: string[] = [];
...
export function ProsePeekAnchor({
  ...
  const panelStore = useOptionalWhiteboardPanelStore();
  ...
  return (
    <a
      href={href}
      className={className}
      data-whiteboard-anchor-id={anchorId}
      data-whiteboard-anchor-open={isOpen ? "true" : undefined}
      onClick={(event) => {
        event.preventDefault();
...
export function AnchorLink({
  anchor,
  children,
}: WhiteboardComponentProps<"AnchorLink"> & { children?: ReactNode }) {
  const openPeek = useWhiteboardPanel((state) => state.openPeek);

  const peekOpen = useWhiteboardPanel(
    (state) =>
  ...
  return (
    <ProsePeekAnchor
      href={`#whiteboard-anchor-${anchor.id}`}
      anchorId={anchor.id}
      isOpen={peekOpen}
      onOpen={(text) => {
        openPeek({
...
export function WhiteboardPanelHost() {
  const activePanel = useWhiteboardPanel((state) => state.active);
  const close = useWhiteboardPanel((state) => state.close);

  const activateTourAnchor = useWhiteboardPanel(
...
function CommitFileDiffPanel({
  ...
  commit: import("@dev.fast/whiteboard-protocol").WhiteboardCommitSummary;
  file: import("@dev.fast/whiteboard-protocol").WhiteboardDiffFileWire;
  onClose: () => void;
}) {
  return (
    <WhiteboardPanelFrame
...
function TraceQuotePeekPanel({
  ...
  const { openTraceSession } = useWhiteboardActions();
    ...
    return (
      ...
      >
        <div className="side-peek-body">
          <p className="whiteboard-trace-note">Loading trace…</p>
        </div>
      </WhiteboardPanelFrame>
    );
  }
  ...
  if (data.status === "error") {
    return (
      ...
      >
        <div className="side-peek-body">
          <p className="whiteboard-trace-note whiteboard-trace-note--error">
            {data.error}
          </p>
        </div>
      </WhiteboardPanelFrame>
  ...
  const headerAccessory = (
    <button
      type="button"
      className="whiteboard-trace-peek-open-full"
      onClick={() => {
        openTraceSession?.({
          sessionId,
          trace,
      ...
      <div className="side-peek-body">
        {targetEventIndex === -1 ? (
          ...
          <TraceDocument
            ...
            picks={picks}
            className="whiteboard-trace-events--scoped"
          />
        )}
      </div>
    </WhiteboardPanelFrame>
...
function CodeWhiteboardPeekPanel({
  anchor,
  content,
  onClose,
}: {
  ...
  const { softwareMapEnabled, openSoftwareMapElement } = useWhiteboardActions();
    ...
    >
      <div className="side-peek-body">
        ...
        <div className="peek-content">
          <WhiteboardPeekContentView anchor={anchor} content={content} />
        </div>
      </div>
    </WhiteboardPanelFrame>
  );
          ...
          {tour.stops.map((stop, index) => {
            ...
            return (
              <section
                ...
                data-whiteboard-anchor-id={stop.anchor.id}
              >
