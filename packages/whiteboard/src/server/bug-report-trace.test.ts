...
import type { WhiteboardAgentHarness } from "../agent-session-ref";
import {
...
describe("readAuthoringTraceAttachment", () => {
  ...
  beforeEach(() => {
    ...
    whiteboardRootPath = path.join(tempDir, "review");
  ...
  it("returns null without an attributable source session or local trace", async () => {
    writeWhiteboard("disabled:review");
    await expect(
      readAuthoringTraceAttachment({
        sourceSession: JSON.parse(
          readFileSync(path.join(whiteboardRootPath, "review.json"), "utf8"),
        ).sourceSession,
      }),
    ).resolves.toBeNull();

    writeWhiteboard("codex:missing-session");
    await expect(
      readAuthoringTraceAttachment({
        sourceSession: JSON.parse(
          readFileSync(path.join(whiteboardRootPath, "review.json"), "utf8"),
  ...
  it("drops malformed and blank lines and marks the trace truncated", async () => {
    ...
    writeWhiteboard("claude-code:" + id);
  ...
  it("accepts 32 Codex parent levels and omits the 33rd", async () => {
    ...
    writeWhiteboard("codex:" + parentId);
    ...
    writeWhiteboard("codex:" + sourceId);
  ...
  function writeWhiteboard(sourceSession: string): void {
    writeFileSync(
      path.join(whiteboardRootPath, "review.json"),
      JSON.stringify({
