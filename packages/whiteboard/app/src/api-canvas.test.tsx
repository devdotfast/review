...
import { createSessionApi } from "../../src/session-api/http";
import { SessionInputError } from "../../src/session-api/input-error";
import { LocalSessionData } from "../../src/session-api/local-data";
import { SessionStore } from "../../src/session-api/store";
...
beforeEach(() => {
  ...
  directory = mkdtempSync(path.join(tmpdir(), "whiteboard-api-canvas-"));
...
it("mounts the existing canvas and preserves a section's DOM and collapsed state through live edits", async () => {
  ...
  const node = container.querySelector(
    `[data-whiteboard-node-id="${inserted.targetId}"]`,
  )!;
  ...
  expect(
    container.querySelector(`[data-whiteboard-node-id="${inserted.targetId}"]`),
  ).toBe(node);
  expect(toggle.getAttribute("aria-expanded")).toBe("false");
  await act(async () => {
...
it.each([false, true])(
  ...
  async (inline) => {
    ...
    expect(container.querySelector(".whiteboard-trace-quote")).toBeTruthy();
...
it("renders a code peek block on its pinned side without fetching source text", async () => {
  ...
  const created: WhiteboardInlineEditorSpec[] = [];
...
it("copies prose and code from the displayed historical JSON review", async () => {
  ...
  const listeners = new Set<
    Parameters<WhiteboardCanvasBridge["subscribe"]>[0]
  >();
  ...
  try {
    ...
    const selected: WhiteboardSurfaceEvent = {
...
it("degrades to the retained document and an unavailable Commits tab when the checkout is gone", async () => {
  ...
  try {
    ...
    expect(
      container.querySelector(".whiteboard-source-context")?.textContent,
    ).toBe("Local checkout unavailable. Showing retained source.");
