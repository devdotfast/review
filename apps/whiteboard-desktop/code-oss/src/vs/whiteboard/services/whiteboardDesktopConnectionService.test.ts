...
import { WhiteboardDesktopConnectionService } from "./whiteboardDesktopConnectionService.js";
...
function serviceWith(storage = new TestStorage()): WhiteboardDesktopConnectionService {
	const service = new WhiteboardDesktopConnectionService({} as never, storage as never);
...
test("install status shares concurrent scans and refreshes on subsequent checks", async (t) => {
	...
	mockFetch(t, async () => {
		...
		return Response.json({
			...
			shim: { path: "/tmp/whiteboard", installed: false, profileConfigured: false, onPath: false },
			trace: { enabled: false, configured: false, autoActivateRepositories: false, envPath: "/tmp/env", settingsPath: "/tmp/settings" },
			cli: null,
			connect: { command: "whiteboard", args: ["mcp"], prompts: { claude: "c", codex: "c", cursor: "c", opencode: "c", pi: "c" }, plugins: { claude: { label: "c" }, codex: { label: "c" }, cursor: { label: "c" }, opencode: { label: "c" }, pi: { label: "c" } } },
			legacySkills: [],
		});
	});
