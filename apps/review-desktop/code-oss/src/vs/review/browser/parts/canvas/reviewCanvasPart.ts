				...
				await this.render({
					...
					bridge: {
						...
						post: async request => {
							...
							if (request.name === "openFile" || request.name === "reveal") {
								const range = request.name === "reveal"
									? { startLine: request.args.startLine, endLine: request.args.endLine }
									: request.args.line ? { startLine: request.args.line, endLine: request.args.endLine ?? request.args.line } : undefined;
								await this.apiSource.open({ reviewId, version, file: request.args.path, side: request.name === "reveal" ? request.args.side ?? "head" : "head" }, range);
								return { ok: true };
							}
