...
test("only the preview workflow opts into the preview icon", () => {
  assert.match(previewWorkflow, /WHITEBOARD_APP_ICON_CHANNEL:\s*preview/);
  assert.doesNotMatch(stableWorkflow, /WHITEBOARD_APP_ICON_CHANNEL/);
  assert.match(
    applyAppIcon,
    /getIconVariant\(process\.env\.WHITEBOARD_APP_ICON_CHANNEL\)/,
  );
});
