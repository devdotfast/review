 ...
 */
// Hand-maintained twin of WHITEBOARD_DESKTOP_DISCOVERY_VERSION in
// @dev.fast/whiteboard-protocol. Version 3: the desktop serves prebuilt
// revisions instead of building them.
export const WHITEBOARD_DESKTOP_CONNECTION_VERSION = 3;

/** Main-process IPC channel the renderer asks for that endpoint on. */
export const WHITEBOARD_DESKTOP_CHANNEL = "review";

export interface WhiteboardDesktopConnection {
...
export interface WhiteboardDesktopCredentials {
  readonly token: string;
  readonly instanceId: string;
}
 ...
 */
const WHITEBOARD_RUNTIME_SERVER_ENTRY =
 ...
 */
export function resolveWhiteboardServerEntry(input: {
  ...
  return join(input.appRoot, WHITEBOARD_RUNTIME_SERVER_ENTRY);
}

export class WhiteboardReadyEventReader {
  private buffer = "";
  private done = false;

  constructor(private readonly credentials: WhiteboardDesktopCredentials) {}
   ...
   */
  push(chunk: string): WhiteboardDesktopConnection | undefined {
  ...
  private readLine(line: string): WhiteboardDesktopConnection | undefined {
  ...
  private validate(event: Record<string, unknown>): WhiteboardDesktopConnection {
