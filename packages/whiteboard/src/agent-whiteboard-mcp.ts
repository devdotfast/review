...
import type { WhiteboardMcpRegistration } from "@dev.fast/whiteboard-protocol";
import {
  type ParseError,
  applyEdits,
  modify,
...
import {
  devWhiteboardHome,
  whiteboardDesktopStateDir,
} from "./whiteboard-home-paths.js";

export const WHITEBOARD_MCP_TARGETS = [
  "codex",
  "claude",
  "cursor",
  "opencode",
...
export function whiteboardMcpLauncher(env: NodeJS.ProcessEnv) {
  return path.join(whiteboardDesktopStateDir(env), "whiteboard-mcp");
}

export async function whiteboardMcpRegistration(
  target: WhiteboardMcpRegistration["target"],
  homeDir: string,
  env: NodeJS.ProcessEnv,
): Promise<WhiteboardMcpRegistration> {
  return {
    name: "whiteboard",
    target,
    configPath: await configPath(target, homeDir, env),
    command: whiteboardMcpLauncher(env),
    args: ["mcp"],
    env: {
      DEV_WHITEBOARD_HOME: devWhiteboardHome(env, homeDir),
...
function configuration(registration: WhiteboardMcpRegistration) {
...
function serverName(registration: WhiteboardMcpRegistration) {
  return registration.name ?? "review";
}

function tomlBlock(registration: WhiteboardMcpRegistration) {
  const owner =
...
async function readConfig(registration: WhiteboardMcpRegistration) {
...
export async function whiteboardMcpStatus(
  desired: WhiteboardMcpRegistration,
  previous?: WhiteboardMcpRegistration,
) {
  try {
...
export async function writeWhiteboardMcpRegistration(
  desired: WhiteboardMcpRegistration,
  previous?: WhiteboardMcpRegistration,
  remove = false,
): Promise<boolean> {
