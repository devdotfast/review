...
export class MultiDiffEditorWidget extends Disposable {
	...
	public setViewModel(viewModel: MultiDiffEditorViewModel | undefined, options?: { readonly preserveFocus?: boolean; readonly initialScrollPosition?: 'firstChange' | 'top'; readonly viewState?: IMultiDiffEditorViewState }): void {
		...
		this._widgetImpl.get().setInitialScrollPositionOnLoad(options?.initialScrollPosition ?? 'firstChange');
	...
	private readonly _activeItem = derived(this, reader => {
		const active = this._viewModel.read(reader)?.activeDiffItem.read(reader);
		return active ? { original: active.originalUri, modified: active.modifiedUri } : undefined;
	});
	...
	public readonly onDidScroll = Event.fromObservableLight(this._scrollTop);
	public readonly onDidChangeContentHeight = Event.fromObservableLight(this._contentHeight);
	public readonly onDidChangeActiveItem = Event.fromObservableLight(this._activeItem);

	public setScrollTop(scrollTop: number): void {
		this._widgetImpl.get().setScrollTop(scrollTop);
	}
	...
	public getContentHeight(): number {
		return this._contentHeight.get();
	}

	public getActiveItem(): IMultiDiffResourceId | undefined {
		return this._activeItem.get();
	}
