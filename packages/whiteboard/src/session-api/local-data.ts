...
import { structuralChangeCounts } from "@dev.fast/whiteboard-protocol";
import type {
  SessionSourceEntry,
  StructuralDiffEvent,
  WhiteboardLanguageEnvironment,
...
import { WhiteboardWorkspaces } from "./workspaces.js";
import {
  EMPTY_SOURCE,
  inspectWorktree,
  localSourcePath,
...
export class LocalSessionData {
  private readonly workspaceManager?: WhiteboardWorkspaces;
  ...
  async languageEnvironment(
    ...
    retryFailed = false,
    anchor?: SourcePins,
  ): Promise<WhiteboardLanguageEnvironment> {
    ...
    const unavailable: WhiteboardLanguageEnvironment = {
      rootPath: null,
  ...
  constructor(
    ...
    if (options.manageWorkspaces !== false)
      this.workspaceManager = new WhiteboardWorkspaces(
  ...
  async *structuralChanges({
    ...
    const rootPath = await ensureWhiteboardPinnedCheckout({
