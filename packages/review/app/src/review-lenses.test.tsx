...
it.each(["sequence", "file_lens"])(
  "clears a %s lens without destroying the full comparison's native state",
  async (kind) => {
    ...
    try {
      ...
      await act(async () => {
        const button = [...container.querySelectorAll("button")].find(
          (button) => button.textContent?.includes("Path"),
        )!;
      ...
      expect(selectedToggle?.getAttribute("aria-pressed")).toBe(
        kind === "file_lens" ? "true" : null,
      );
      expect(selectedToggle?.getAttribute("aria-expanded")).toBe(
        kind === "file_lens" ? null : "true",
      );
