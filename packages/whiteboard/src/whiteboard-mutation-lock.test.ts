...
import {
  GUARDED_WHITEBOARD_FIELDS,
  whiteboardMutationFingerprint,
...
it("changes the fingerprint for every guarded field", () => {
  ...
  const unchanged = whiteboardMutationFingerprint(record);
  ...
  for (const field of GUARDED_WHITEBOARD_FIELDS) {
    ...
    expect(whiteboardMutationFingerprint(changed)).not.toBe(unchanged);
  }
});
...
it("ignores unguarded metadata and source identity key order", () => {
  ...
  ).toBe(whiteboardMutationFingerprint(record));
});
...
it("allows nested operations in the same transaction without deadlocking", async () => {
  ...
  roots.push(root);
  expect(
    await withWhiteboardMutationLock(root, () =>
      withWhiteboardMutationLock(root, async () => "nested"),
    ),
  ).toBe("nested");
});
...
it("reports retryable contention and succeeds after the holder releases", async () => {
  ...
  const holding = withWhiteboardMutationLock(root, async () => {
    entered.resolve();
  ...
  try {
    await expect(
      withWhiteboardMutationLock(root, async () => "overlap", {
        timeoutMs: 20,
      }),
    ).rejects.toMatchObject({
      name: "WhiteboardBusyError",
      code: "WHITEBOARD_BUSY",
      retryable: true,
  ...
  await expect(
    withWhiteboardMutationLock(root, async () => "retried"),
...
`,
  ...
  try {
    ...
    ).rejects.toMatchObject({ code: "WHITEBOARD_BUSY", retryable: true });
    expect(
