...
import { openWhiteboardProfile } from "./profile.js";
...
async function fixture() {
  const initial = await openWhiteboardProfile(home, {
...
it("merges preview headless history and resources without changing review IDs or resurrecting deleted reviews", async () => {
  ...
  const profile = await openWhiteboardProfile(home, {
    manageWorkspaces: false,
  });
  ...
  const reopened = await openWhiteboardProfile(home, {
    manageWorkspaces: false,
  });
...
it("refuses migration while the preview server owns its source, then succeeds after shutdown", async () => {
  ...
  try {
    await expect(
      openWhiteboardProfile(home, { manageWorkspaces: false }),
