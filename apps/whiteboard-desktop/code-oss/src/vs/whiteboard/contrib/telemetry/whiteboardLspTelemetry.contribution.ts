...
class WhiteboardLspTelemetryContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		@IWhiteboardTelemetryService private readonly telemetryService: IWhiteboardTelemetryService,
	) {
		super();
	...
	private editorKind(editor: ICodeEditor): string {
		if (WhiteboardEmbeddedEditors.owns(editor)) {
			return 'inline_peek';
		}
...
).registerWorkbenchContribution(WhiteboardLspTelemetryContribution, LifecyclePhase.Eventually);
