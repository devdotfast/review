...
export function scrollToWhiteboardHeading(
  id: string,
  ...
  const collapsedSection = heading.closest(".whiteboard-section--collapsed");
  collapsedSection?.dispatchEvent(new CustomEvent("whiteboard-section-expand"));

  const performScroll = () => {
    const scrollRoot = getWhiteboardScrollRoot(article, scrollRegion);
...
export function getWhiteboardScrollRoot(
  article: HTMLElement,
  scrollRegion: HTMLElement | null,
): HTMLElement | null {
