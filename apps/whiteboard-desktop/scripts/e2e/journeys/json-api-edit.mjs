...
export async function run(ctx) {
  ...
  // createWhiteboard has not run yet, so an imported fixture is the only open canvas available.
  ...
  await openLegacyWhiteboard(ctx, fixture);
  ...
  const canvas = page.locator(".whiteboard-canvas-root");
  ...
  const created = await createWhiteboard(ctx, {
  ...
  ctx.check("createWhiteboard helper opens an API review");
}
