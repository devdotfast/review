...
const MIXED_STACK = [
  ...
  "    at handleUnexpectedError (vscode-file://vscode-app/Applications/Review.app/Contents/Resources/app/out/vs/whiteboard/browser/workbench.js:456:12)",
...
describe("packBundleFrames", () => {
  it("keeps shipped frames and drops everything else", () => {
    expect(packBundleFrames(MIXED_STACK)).toBe(
      [
        "vs/whiteboard/browser/workbench.js:456:12",
  ...
  it("strips a cache-busting query", () => {
    expect(
      packBundleFrames(
        "    at r (https://localhost/out/vs/whiteboard/browser/workbench.js?v=9:1:2)",
      ),
    ).toBe("vs/whiteboard/browser/workbench.js:1:2");
  });

  it("reads the Firefox and bare stack forms", () => {
    expect(
      packBundleFrames(
        "handleUnexpectedError@file:///app/out/vs/whiteboard/browser/workbench.js:5:6",
      ),
    ).toBe("vs/whiteboard/browser/workbench.js:5:6");
    ...
    const merged = mergeErrorTelemetryProperties(
      {
        ...
        frames: "vs/whiteboard/browser/workbench.js:1:1",
      },
      undefined,
    );
