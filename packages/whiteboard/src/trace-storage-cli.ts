...
export interface RunWhiteboardTraceStorageUseInput
...
export async function runTraceStorageUse(
  input: RunWhiteboardTraceStorageUseInput,
): Promise<number> {
...
async function useHosted(
  ...
  try {
    ...
    const devHome = devWhiteboardHome(scope.env, scope.homeDir);
...
export interface RunWhiteboardTraceConfigMigrateInput
  extends CliJsonOutput, TraceStorageCommandScope {
  dryRun?: boolean;
...
export async function runTraceConfigMigrate(
  input: RunWhiteboardTraceConfigMigrateInput,
): Promise<number> {
