...
const whiteboardWorkbench = read("../code-oss/src/vs/whiteboard/browser/workbench.ts");
...
test("Review marks its window so shared chrome can opt out", () => {
  assert.match(
    contextKeys,
    /export const IsWhiteboardWindowContext = new RawContextKey<boolean>\('isWhiteboardWindow', false/,
  );
  assert.match(
    whiteboardWorkbench,
...
test("the split editor toolbar icon is gated out of Review", () => {
  ...
  for (const item of splitToolItems) {
    assert.match(item, /IsWhiteboardWindowContext\.toNegated\(\)/);
  }
});
...
test("Review renders no editor actions overflow menu", () => {
  ...
  assert.match(
    update,
    /IsWhiteboardWindowContext\.getValue\(this\.contextKeyService\) \? \[\] : secondary/,
