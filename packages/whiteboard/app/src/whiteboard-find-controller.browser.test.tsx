...
it("orders duplicate editors with MDX and wraps navigation", async () => {
  ...
  focusTarget.focus();
  const host = createWhiteboardFindHost();
  ...
  await vi.waitFor(() => {
    expect(container.querySelector(".whiteboard-find-count")?.textContent).toBe(
      "1 of 4",
    );
  });
  // The shell is the widget's containing block.
  expect(
    container.querySelector(".whiteboard-find-widget")?.parentElement,
  ).toHaveClass("whiteboard-document-shell");
  ...
  await vi.waitFor(() => {
    expect(
      container.querySelector(
        '.whiteboard-find-widget input[aria-label="Find"]',
      ),
    ).not.toBeNull();
  });
...
function FindHarness({
  ...
  handles: WhiteboardInlineEditorHandle[];
}) {
      ...
      >
        <main ref={shellRef} className="whiteboard-document-shell">
          <section ref={scrollRef}>
            <article ref={articleRef} className="whiteboard-document">
              <p>Alpha first</p>
...
function InlineRegistration({
  handle,
}: {
  handle: WhiteboardInlineEditorHandle;
}) {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const find = useWhiteboardFindRegistration();
  useLayoutEffect(() => {
  ...
  return (
    <div ref={containerRef} data-whiteboard-inline-editor="duplicate.ts" />
  );
}
