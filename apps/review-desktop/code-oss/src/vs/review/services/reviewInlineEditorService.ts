import { displayedSources, evidenceCoordinates, type EvidenceRow } from "../common/reviewSearchEvidence.js";
/*---------------------------------------------------------------------------------------------
...
import type { ICompositeCodeEditor, IEditorDecorationsCollection } from "../../editor/common/editorCommon.js";
...
import { provideReviewUnifiedDefinition, provideReviewUnifiedHover } from "./reviewUnifiedDefinition.js";
...
export class ReviewInlineEditorService extends Disposable implements ICompositeCodeEditor {
	private readonly _onDidChangeActiveEditor = this._register(new Emitter<ICompositeCodeEditor>());
	...
	constructor(
		...
		super();
		this._register(
			this.codeEditorService.registerCodeEditorOpenHandler((input, source, sideBySide) =>
	...
	private async openUnifiedNavigation(
		...
		const sourceHandle = source ? this.handlesByEditor.get(source) : undefined;
		...
		const mapped = unified.targetForRange(startLine, endLine) ?? unified.targetForRange(startLine, startLine);
		if (!mapped) return null;

		const resource = mapped.side === "base" ? unified.original : unified.modified;
	...
	async find(
		...
		{
			const unified = await acquireUnifiedFromSource(this.resources, spec, source);
			if (unified) {
				try {
					return {
						matchCount: findModelRanges(unified.model, unified.windows, query).length,
					};
				} finally {
					unified.dispose();
				}
	...
	private updateMetrics(document: Document): void {
		...
		document.body.dataset["reviewInlineEditorModelCount"] = String(handles.filter((handle) => handle.hasModel).length);
	}
}
...
class InlineEditorHandle extends Disposable implements ReviewInlineEditorHandle {
	...
	private readonly findDecorations = new Map<ICodeEditor, IEditorDecorationsCollection>();
  ...
  private get coordinates() { return evidenceCoordinates(this.spec.content); }
	...
	get hasModel(): boolean {
		return this.modelReference !== undefined || this.unifiedModelReference !== undefined;
	}
	...
	async setFindQuery(query: ReviewFindQuery): Promise<ReviewInlineFindResult> {
		...
		if (codeEditor && modelReference) {
			matches.push(...this.findModelMatches(codeEditor, modelReference.model, modelReference.windows, query));
		}
	...
	override dispose(): void {
		...
		this.spec.container.classList.remove("review-inline-code-editor", "review-inline-unified-diff");
		super.dispose();
		this.onDispose();
	}
	...
	private async initialize(): Promise<void> {
		try {
			{
				const unifiedReference = await acquireUnifiedFromSource(this.resources, this.spec, this.source);
				if (unifiedReference) {
					if (this.disposed) {
			...
			const modelReference = await this.source.snippet();
			if (this.disposed) {
	...
	private initializeUnifiedEditor(reference: ReviewUnifiedCodeModelReference): void {
		...
		this.setHeader(
            this.spec.content.kind === "diffr" && !displayedSources(this.spec.content.result).lhs ? undefined : reference.target.original,
            this.spec.content.kind === "diffr" && !displayedSources(this.spec.content.result).rhs ? undefined : reference.target.modified,
			labelUris.original,
			labelUris.modified,
	...
	private layoutCodeEditorToContent(contentHeight?: number): void {
		...
		const estimated =
			(this.spec.heightMode === "content" ? lineCount : Math.min(REVIEW_PEEK_MAX_VISIBLE_LINES, lineCount)) *
			REVIEW_PEEK_LINE_HEIGHT;
		const renderedHeight =
			rendered !== undefined && rendered > CONTENT_HEIGHT_EPSILON ? Math.ceil(rendered) : estimated;
		...
		const measured = Math.max(
			renderedHeight,
			contentHeight !== undefined && contentHeight > CONTENT_HEIGHT_EPSILON ? Math.ceil(contentHeight) : 0,
		);
	...
	private trackScroll(getScrollTop: () => number, onDidScroll: (listener: () => void) => IDisposable): void {
		const recordScrollTop = () => {
			this.spec.container.dataset["reviewInlineEditorScrollTop"] = String(getScrollTop());
		};
		recordScrollTop();
	...
	private markCreated(): void {
		this.spec.container.dataset["reviewInlineEditorCreationDuration"] = (performance.now() - this.startedAt).toFixed(1);
		this.onStateChange();
	}

	private bindFocus(editor: ICodeEditor, store: DisposableStore = this.editorStore): void {
	...
	private findModelMatches(
		...
		return findModelRanges(model, windows, query).map((match) => ({ editor, range: match.range }));
	}
	...
	private rangeClassName(): string {
		return this.active ? "review-inline-code-range review-inline-code-range-active" : "review-inline-code-range";
	}

	private applyWindows(editor: ICodeEditor, windows: readonly ReviewPeekWindow[]): void {
		const model = editor.getModel();
		if (!model) return;
		editor.setHiddenAreas(
			reviewPeekHiddenAreas(model.getLineCount(), windows).map(
				(area) => new Range(area.startLineNumber, 1, area.endLineNumber, 1),
			),
			this,
		);
	}
...
function estimatedHeight(
	...
	const visibleLineCount = heightMode === "content" ? lineCount : Math.min(REVIEW_PEEK_MAX_VISIBLE_LINES, lineCount);
	return visibleLineCount * REVIEW_PEEK_LINE_HEIGHT + INLINE_HEADER_HEIGHT;
}

function findModelRanges(model: ITextModel, windows: readonly ReviewPeekWindow[], query: ReviewFindQuery) {
	const searchRanges = windows.map(
		(window) => new Range(window.startLine, 1, window.endLine, model.getLineMaxColumn(window.endLine)),
	);
	return model
		.findMatches(
			query.text,
...
function inlineEditorOptions(overflowWidgetsDomNode?: HTMLElement): IEditorConstructionOptions {
	return {
