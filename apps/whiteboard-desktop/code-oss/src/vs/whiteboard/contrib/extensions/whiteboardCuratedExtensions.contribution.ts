...
import { WHITEBOARD_KEYMAP_SETTING, type WhiteboardKeymap } from '../../common/whiteboardConfigurationDefaults.js';
import { WHITEBOARD_DESKTOP_CHANNEL } from '../../common/whiteboardDesktopBootstrap.js';
import { setFirstRunReloadPending } from '../../common/whiteboardFirstRunReload.js';
import type { WhiteboardUserConfigImportResult } from '../../node/whiteboardUserConfigImport.js';
import { IWhiteboardTelemetryService } from '../../services/whiteboardTelemetryService.js';
import { whiteboardOptionalExtensionCatalog } from '../../node/whiteboardOptionalExtensionCatalog.js';
import {
	installMissingOptionalExtensions,
	OptionalExtensionInstallError,
	optionalGroupMemberIds,
	upgradeOptionalExtensionPins
} from './whiteboardOptionalExtensionManagement.js';

/**
 * The curated bundled extensions Review materializes, mirroring
 * apps/whiteboard-desktop/scripts/curated-extensions.manifest.mjs. A test in
...
const ALL_CURATED_IDS = [
	...
	...whiteboardOptionalExtensionCatalog.map(extension => extension.id)
];
...
const KEYMAP_EXTENSION_IDS: Readonly<Record<Exclude<WhiteboardKeymap, 'none'>, string>> = {
	vim: 'vscodevim.vim',
...
export async function setCuratedExtensionsEnabled(
	...
	whiteboardTelemetryService: IWhiteboardTelemetryService,
	...
	for (const extension of installedCurated) {
		...
		if (enabledIds.some(id => areSameExtensions(extension.identifier, { id }))) {
			...
			if (didChange) {
				whiteboardTelemetryService.capture('extension_enabled', { extension_id: extensionId, trigger });
			}
		} else {
			didChange = await enablementService.disableExtension(extension.identifier);
			if (didChange) {
				whiteboardTelemetryService.capture('extension_disabled', { extension_id: extensionId, trigger });
			}
		}
		changed ||= didChange;
	}
 ...
 */
async function applyWhiteboardKeymap(accessor: ServicesAccessor, keymap: WhiteboardKeymap): Promise<void> {
	const configurationService = accessor.get(IConfigurationService);
	const enablementService = accessor.get(IGlobalExtensionEnablementService);
	const commandService = accessor.get(ICommandService);
	const dialogService = accessor.get(IDialogService);
	const whiteboardTelemetryService = accessor.get(IWhiteboardTelemetryService);

	if (configurationService.getValue<WhiteboardKeymap>(WHITEBOARD_KEYMAP_SETTING) === keymap) {
		return;
	}
	await configurationService.updateValue(WHITEBOARD_KEYMAP_SETTING, keymap, ConfigurationTarget.USER);

	let changed = false;
	for (const [candidate, id] of Object.entries(KEYMAP_EXTENSION_IDS) as [Exclude<WhiteboardKeymap, 'none'>, string][]) {
		const didChange = candidate === keymap
			? await enablementService.enableExtension({ id })
			: await enablementService.disableExtension({ id });
		if (didChange) {
			whiteboardTelemetryService.capture(candidate === keymap ? 'extension_enabled' : 'extension_disabled', {
				extension_id: id,
				trigger: 'keymap'
			});
		}
	...
	if (reload.confirmed) {
		await whiteboardTelemetryService.flush();
		await commandService.executeCommand('workbench.action.reloadWindow');
	}
}

CommandsRegistry.registerCommand('whiteboard.setKeymap', (accessor, keymap: WhiteboardKeymap) =>
	applyWhiteboardKeymap(accessor, keymap)
);
...
function optionalDownloadSize(group: string, installed: readonly ILocalExtension[]): number {
	...
	return whiteboardOptionalExtensionCatalog
...
function optionalExtensionLabel(extensionId: string): string {
	const extension = whiteboardOptionalExtensionCatalog.find(candidate => candidate.id === extensionId);
...
async function stageRustAnalyzer(
	...
	try {
		await mainProcessService.getChannel(WHITEBOARD_DESKTOP_CHANNEL).call('stageRustAnalyzer');
	} catch (error) {
...
class ManageCuratedExtensionsAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		for (const optionalGroup of OPTIONAL_GROUPS) {
			...
			const primary = whiteboardOptionalExtensionCatalog.find(extension =>
		...
		const picked = await new Promise<readonly CuratedQuickPickItem[] | undefined>(resolve => {
			...
			const updateAcceptLabel = (selected: readonly CuratedQuickPickItem[]): void => {
				...
				const selectedGroups = new Set(selected
					...
					.map(item => item.group));
				const installCount = whiteboardOptionalExtensionCatalog.filter(extension =>
				...
				const hasOtherChanges = items.some(item => {
					...
					return item.kind !== 'optional' || !selectedSet.has(item) ||
						!whiteboardOptionalExtensionCatalog.some(extension =>
				...
				void (async () => {
					...
					try {
						...
						for (const extension of userMembers) {
							...
							const extensionId = ALL_CURATED_IDS.find(id => areSameExtensions(extension.identifier, { id }));
							if (extensionId) {
								whiteboardTelemetryService.capture('extension_uninstalled', {
			...
			newInstalls = await progressService.withProgress({
				...
				download: async extensionId => {
					...
					return ipcRenderer.invoke(
						'vscode:whiteboardDownloadOptionalExtension',
						extensionId
					) as Promise<string>;
				},
				...
				onInstalled: (extensionId, trigger, durationMs) => {
					whiteboardTelemetryService.capture('extension_installed', {
						extension_id: extensionId,
						trigger,
				...
				onInstallFailed: (extensionId, trigger, phase) => {
					whiteboardTelemetryService.capture('extension_install_failed', {
						extension_id: extensionId,
						trigger,
						phase
					});
		...
		const keymap = (Object.entries(KEYMAP_EXTENSION_IDS) as [Exclude<WhiteboardKeymap, 'none'>, string][])
		...
		const keymapChanged = configurationService.getValue<WhiteboardKeymap>(WHITEBOARD_KEYMAP_SETTING) !== keymap;
		if (keymapChanged) {
			await configurationService.updateValue(WHITEBOARD_KEYMAP_SETTING, keymap, ConfigurationTarget.USER);
		}
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		try {
			const preview = await ipcRenderer.invoke(
				'vscode:whiteboardImportUserConfig',
				{ mode: 'preview' },
...
class CuratedExtensionDefaults implements IWorkbenchContribution {
	constructor(
		...
		@IWhiteboardTelemetryService whiteboardTelemetryService: IWhiteboardTelemetryService,
	) {
		const alreadySeeded = storageService.getBoolean(SEEDED_STORAGE_KEY, StorageScope.APPLICATION, false);
		const keymap = configurationService.getValue<WhiteboardKeymap>(WHITEBOARD_KEYMAP_SETTING);
		const disabled = enablementService.getDisabledExtensions();
		const keymapStateMatches = (Object.entries(KEYMAP_EXTENSION_IDS) as [Exclude<WhiteboardKeymap, 'none'>, string][])
		...
		void this.applyKeymapDefault(keymap, enablementService, commandService, whiteboardTelemetryService);
	}

	private async applyKeymapDefault(
		keymap: WhiteboardKeymap,
		enablementService: IGlobalExtensionEnablementService,
...
class OptionalExtensionPinUpgrades implements IWorkbenchContribution {
	constructor(
		...
		@IWhiteboardTelemetryService private readonly whiteboardTelemetryService: IWhiteboardTelemetryService
	) {
	...
	private async run(): Promise<void> {
		...
		await upgradeOptionalExtensionPins({
			...
			onInstalled: (extensionId, trigger, durationMs) => {
				this.whiteboardTelemetryService.capture('extension_installed', {
					extension_id: extensionId,
					trigger,
					duration_ms: durationMs
				});
			...
			onInstallFailed: (extensionId, trigger, phase) => {
				this.whiteboardTelemetryService.capture('extension_install_failed', {
					extension_id: extensionId,
					trigger,
					phase
				});
