...
export class SessionStore {
  ...
  private summaries(
    ...
    return this.withDiffStats(whiteboards, mode);
  }
  ...
  withDiffStats<T extends SessionSummary>(
    ...
    return whiteboards.map((whiteboard) => ({
      ...whiteboard,
      diffStats: whiteboard.pins
        ? (stats.get(JSON.stringify([whiteboard.pins, mode])) ?? null)
        : null,
    }));
  }
  ...
  execute(
    ...
    const run = this.pending.then(async () => {
      ...
      if (
        ...
        const [found, ...others] = this.whiteboardsForPullRequest(
          op.pullRequestUrl,
        );

        if (found) {
          const result = this.existingWhiteboard(
            found,
            others,
  ...
  private startPullRequest(
    ...
    const [existing] =
      op.reuseExisting === false
        ? []
        : this.whiteboardsForPullRequest(op.pullRequestUrl);
  ...
  private whiteboardsForPullRequest(url: string): string[] {
    return this.db
      .prepare(
  ...
  private existingWhiteboard(
    ...
    const note = [
      ...
      others.length > 0 &&
        "Older sessions also name this PR; see otherWhiteboardIds.",
    ]
      .filter(Boolean)
      .join(" ");
    ...
    return {
      ...
      ...(others.length > 0 && { otherWhiteboardIds: others }),
    };
  }
