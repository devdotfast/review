...
test("canvas targets are derived from fixed output locations", () => {
  ...
  assert.deepEqual(canvasTargets([], fakeAppRoot), [
    path.join(fakeAppRoot, "code-oss/out/vs/whiteboard/canvas"),
  ]);
  assert.deepEqual(
    canvasTargets(["--packaged-root", packagedRoot], fakeAppRoot),
    [
      path.join(fakeAppRoot, "code-oss/out/vs/whiteboard/canvas"),
      path.join(packagedRoot, "resources/app/out/vs/whiteboard/canvas"),
    ],
  );
  ...
  assert.deepEqual(
    ...
    [
      path.join(fakeAppRoot, "code-oss/out/vs/whiteboard/canvas"),
      path.join(packagedMacRoot, "Contents/Resources/app/out/vs/whiteboard/canvas"),
    ],
  );
  assert.throws(
...
test("the canvas loader exposes transient view-state reset", () => {
  ...
  assert.match(
    source,
    /export \{ clearWhiteboardViewState, mountWhiteboardCanvas \} from "\.\/assets\/canvas\.js";/,
  );
  assert.doesNotMatch(source, /whiteboardDocRuntimeUrl|doc-runtime/);
});
