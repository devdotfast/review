...
import { useWhiteboardFindRegistration } from "./whiteboard-find";
import { emitWhiteboardInteraction } from "./whiteboard-interaction-event";
import {
...
export function DocumentCodeView({
  ...
  const progress: WhiteboardDiffProgress = {
  ...
  const handleNavigation = useCallback(
    () =>
      emitWhiteboardInteraction(container, {
  ...
  const handleHover = useCallback(
    () => emitWhiteboardInteraction(container, { kind: "inline-hover", path }),
    [container, path],
  );
  ...
  const handleRef = useRef<WhiteboardInlineEditorHandle | null>(null);
  const creationFailedRef = useRef(false);
  const latestFindQueryRef = useRef<WhiteboardFindQuery | null>(null);

  const handleWaitersRef = useRef<
    Array<(handle: WhiteboardInlineEditorHandle | null) => void>
  >([]);
  ...
  const ensureEditor = useCallback(async () => {
    ...
    return new Promise<WhiteboardInlineEditorHandle | null>((resolve) => {
      handleWaitersRef.current.push(resolve);
    });
  }, []);

  const setFindQuery = useCallback(
    async (query: WhiteboardFindQuery) => {
  ...
  useLayoutEffect(() => {
    if (!container || !whiteboardFind) return;

    return whiteboardFind.register({
      container,
      setFindQuery,
      revealFindMatch,
      clearFind,
  ...
  }, [clearFind, container, revealFindMatch, whiteboardFind, setFindQuery]);
  ...
  useLayoutEffect(() => {
    ...
    let handle: WhiteboardInlineEditorHandle;
  ...
  return (
    <>
      <div
        ref={setContainer}
        className="whiteboard-inline-editor"
        data-whiteboard-inline-editor={path}
        data-whiteboard-inline-editor-active={active ? "true" : "false"}
        style={{ height }}
      />
      {error ? (
        <div className="whiteboard-inline-editor-error" title={error}>
