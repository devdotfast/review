...
export class NativeManagedSettingsService extends Disposable implements INativeManagedSettingsService {
	...
	private async updateWatcher(): Promise<void> {
		...
		// Review does not ship @vscode/policy-watcher; without an explicit
		// factory the service degrades to reporting no managed settings.
		let createWatcher: NativePolicyWatcherFactory;
		if (this.watcherFactory) {
			...
			try {
				...
				this.logService.trace(`NativeManagedSettingsService - @vscode/policy-watcher is not installed; managed settings are disabled`);
				return;
			}
		}
