...
const MARK_PATHS: Record<WhiteboardDiffFileWire["status"], ReactElement> = {
...
export function FileMark({
  ...
  status: WhiteboardDiffFileWire["status"];
}): ReactElement {
  return (
    <svg
      className={`whiteboard-file-mark whiteboard-file-mark-${status}`}
      viewBox="0 0 16 16"
      aria-hidden="true"
    >
      {MARK_PATHS[status]}
