...
// change to it. See apps/whiteboard-desktop/UPSTREAM.
...
Registry.as<IConfigurationRegistry>(
  ...
  id: "whiteboardTrustFilterFixture",
  type: "object",
  properties: {
    "whiteboardTrustFixture.restricted": {
      type: "string",
      restricted: true,
      scope: ConfigurationScope.RESOURCE,
    },
    "whiteboardTrustFixture.safe": {
      type: "string",
      scope: ConfigurationScope.RESOURCE,
    },
    "whiteboardTrustFixture.object": {
      type: "object",
      scope: ConfigurationScope.RESOURCE,
    },
  },
...
test("filters a restricted setting written as a dotted key", () => {
  const parsed = parseUntrusted({
    "whiteboardTrustFixture.restricted": "attacker",
    "whiteboardTrustFixture.safe": "kept",
  });

  assert.equal(
    parsed.configurationModel.getValue("whiteboardTrustFixture.restricted"),
    undefined,
  );
  assert.equal(
    parsed.configurationModel.getValue("whiteboardTrustFixture.safe"),
    "kept",
  );
...
test("filters the same setting written as a nested object", () => {
  const parsed = parseUntrusted({
    whiteboardTrustFixture: { restricted: "attacker", safe: "kept" },
  });
...
test("leaves an object-valued setting intact", () => {
  const parsed = parseUntrusted({
    "whiteboardTrustFixture.object": { a: 1, b: { c: 2 } },
  });

  assert.deepEqual(
    parsed.configurationModel.getValue("whiteboardTrustFixture.object"),
    { a: 1, b: { c: 2 } },
  );
});
