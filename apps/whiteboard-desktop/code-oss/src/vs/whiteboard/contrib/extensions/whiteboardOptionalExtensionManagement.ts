...
export async function installMissingOptionalExtensions<T>(
	...
	const missing = whiteboardOptionalExtensionCatalog.filter(extension =>
...
export async function upgradeOptionalExtensionPins(
	...
	for (const group of new Set(whiteboardOptionalExtensionCatalog.map(extension => extension.group))) {
		const primary = whiteboardOptionalExtensionCatalog.find(extension => extension.group === group && extension.role === 'primary');
		if (!primary || !installed.has(primary.id)) {
			continue;
		}

		const members = whiteboardOptionalExtensionCatalog
			.filter(extension => extension.group === group)
