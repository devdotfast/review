...
export async function stageWhiteboardRuntime(packagedRoot) {
  const runtimeRoot = runtimeRootForPackagedRoot(packagedRoot);
  const packagedAppRoot = path.dirname(runtimeRoot);
...
export async function stageDiffrBinary(
  ...
  const require = createRequire(
    path.join(monorepoRoot, "packages/whiteboard/package.json"),
  );
...
export async function readTutorialRuntimeManifest(tutorialRoot) {
  ...
  if (
    value?.version !== 1 ||
    !validEntries(value.whiteboardFiles) ||
    !validEntries(value.requiredPaths) ||
    value.whiteboardFiles.some((entry) => !value.requiredPaths.includes(entry))
  ) {
  ...
  return {
    version: 1,
    whiteboardFiles: [...new Set(value.whiteboardFiles)],
...
if (
  ...
  if (
    ...
    throw new Error(
      "usage: stage-whiteboard-runtime.mjs [--verify] --packaged-root <packaged-app-root>",
    );
  }
  ...
  if (verifyOnly) {
    ...
    const runtimeRoot = await stageWhiteboardRuntime(packagedRoot);
