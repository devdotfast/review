...
class ReviewDefaultConfiguration extends DefaultConfiguration {
	constructor(
		cacheScope: string,
...
export class ReviewConfigurationService extends Disposable implements IWorkbenchConfigurationService {
	...
	constructor(
		...
		this.defaultConfiguration = this._register(new ReviewDefaultConfiguration(userDataProfileService.currentProfile.id, configurationCache, environmentService, logService, options.defaultOverrides ?? {}));
...
class ConfigurationEditing {
	...
	constructor(
		private readonly fileService: IFileService,
		private readonly configurationService: ReviewConfigurationService,
	) { }
