...
to `session_create`; the URL records identity and does not track new commits.
...
| `DEV_FAST_REVIEW_DESKTOP_STATE_ROOT`             | Gives a launched Whiteboard instance separate `user-data` and `extensions` directories beneath this root. Empty or unset uses the normal Desktop state.                                                                    |
| `DEV_FAST_REVIEW_DESKTOP_BACKGROUND` | Set to `1` by `whiteboard app` launches without `--focus`. Whiteboard then shows its first window without taking focus and ignores focus requests until you click it or run `whiteboard app launch --focus`. |
