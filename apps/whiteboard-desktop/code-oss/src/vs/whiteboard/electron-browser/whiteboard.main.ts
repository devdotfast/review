...
import { IAnyWorkspaceIdentifier, IWorkspaceContextService, isSingleFolderWorkspaceIdentifier, isWorkspaceIdentifier, reviveIdentifier } from '../../platform/workspace/common/workspace.js';
...
import { IFileManagedSettingsService, INativeManagedSettingsService } from '../../platform/policy/common/copilotManagedSettings.js';
...
import { WhiteboardWorkbench } from '../browser/workbench.js';
import { IWorkspaceEditingService } from '../../workbench/services/workspaces/common/workspaceEditing.js';
import { WhiteboardConfigurationService } from '../services/configuration/browser/configurationService.js';
import { ConfigurationCache } from '../../workbench/services/configuration/common/configurationCache.js';
import { WhiteboardWorkspaceContextService } from '../services/workspace/browser/workspaceContextService.js';
import { getWorkspaceIdentifier } from '../../platform/workspaces/common/workspaceIdentifier.js';
import { WhiteboardDefaultAccountService } from '../services/whiteboardDefaultAccountService.js';
import { whiteboardAgentsWindowDefaultOverrides } from '../common/whiteboardConfigurationDefaults.js';

export class WhiteboardMain extends Disposable {
	...
	async open(): Promise<void> {
		...
		const workbench = new WhiteboardWorkbench(mainWindow.document.body, {
	...
	private registerListeners(workbench: WhiteboardWorkbench, storageService: NativeWorkbenchStorageService): void {
	...
	private async initServices(): Promise<{ serviceCollection: ServiceCollection; logService: ILogService; storageService: NativeWorkbenchStorageService; configurationService: WhiteboardConfigurationService }> {
		...
		// Default Account
		const defaultAccountService = this._register(new WhiteboardDefaultAccountService());
		...
		const workspaceContextService = new WhiteboardWorkspaceContextService(workspaceIdentifier, uriIdentityService);
	...
	private async createConfigurationService(
		workspaceContextService: WhiteboardWorkspaceContextService,
	...
	): Promise<WhiteboardConfigurationService> {
		const configurationCache = new ConfigurationCache([Schemas.file, Schemas.vscodeUserData], environmentService, fileService);
		const configurationService = new WhiteboardConfigurationService(userDataProfileService, workspaceContextService, uriIdentityService, fileService, policyService, logService, configurationCache, environmentService, { defaultOverrides: whiteboardAgentsWindowDefaultOverrides });
		try {
...
export function main(configuration: INativeWindowConfiguration): Promise<void> {
	const workbench = new WhiteboardMain(configuration);
