...
import { ViewedButton, useWhiteboardLenses } from "./whiteboard-lenses";
...
export function WhiteboardDiffView({
  scope,
}: {
...
function NativeDiffView({
  ...
  const handle = useRef<WhiteboardDiffViewHandle | null>(null);
  ...
  return (
    <>
      <div
        ref={setContainer}
        className="whiteboard-diff-view-host"
        style={hidden ? { display: "none" } : undefined}
      />
      {!hidden && error && (
        <div role="alert" className="whiteboard-diff-view-error">
          {error}
        </div>
      )}
    </>
