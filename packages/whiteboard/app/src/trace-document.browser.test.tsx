...
describe("TraceDocument", () => {
  ...
  const sampleEvents: WhiteboardAgentTraceEvent[] = [
    // Turn 0: events 0..4
  ...
  it("coalesces tool runs in every turn; a group containing the target renders open", async () => {
    ...
    const turns = container.querySelectorAll(".whiteboard-trace-turn");
    expect(turns.length).toBe(2);

    // Turn 0 (non-target turn): 3 edit tools should coalesce into 1 TraceToolGroup
    const turn0 = turns[0];

    const turn0ToolGroups = turn0.querySelectorAll(
      ".whiteboard-trace-toolgroup",
    );
    ...
    const turn1ToolGroups = turn1.querySelectorAll(
      ".whiteboard-trace-toolgroup",
    );
    ...
    // Target event should have #whiteboard-trace-target-event
    const targetEl = container.querySelector("#whiteboard-trace-target-event");
  ...
  it("renders gap chips when picks are provided and expands them on click", async () => {
    ...
    const initialGaps = container.querySelectorAll(
      ".whiteboard-trace-lens-gap",
    );
  ...
  it("brackets an expanded gap with collapse rows that fold it back", async () => {
    ...
    const collapseRows = container.querySelectorAll(
      ".whiteboard-trace-lens-collapse",
    );
    ...
    expect(
      container.querySelectorAll(".whiteboard-trace-lens-collapse").length,
    ).toBe(0);
    expect(container.textContent).not.toContain("First turn question");
    const gapsAfter = container.querySelectorAll(".whiteboard-trace-lens-gap");
  ...
  it("expands elided message text when ellipsis chip is clicked", async () => {
    ...
    const chipButton = container.querySelector(
      ".whiteboard-trace-lens-chip",
    ) as HTMLButtonElement;

    expect(chipButton).not.toBeNull();
    expect(
      container.querySelector(".whiteboard-trace-quote-mark")?.textContent,
  ...
  it("renders the entire user message and highlights the quote without elision", async () => {
    ...
    expect(container.querySelector(".whiteboard-trace-lens-chip")).toBeNull();
    const userBubble = container.querySelector(".agent-chat-user-bubble");
    expect(userBubble?.textContent).toBe("Second turn question");
    expect(
      userBubble?.querySelector(".whiteboard-trace-quote-mark")?.textContent,
    ).toBe("Second turn");
  });
  ...
  it("scrolls directly to the highlighted quote mark when available", async () => {
    ...
    const quoteMark = container.querySelector(".whiteboard-trace-quote-mark");
