...
export class ReviewPlanTool extends Disposable implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
		...
		const whiteboardData = new ChatPlanReviewData(
		...
		this.chatService.appendProgress(request, whiteboardData);

		const result = await raceCancellation(whiteboardData.completion.p, token);
