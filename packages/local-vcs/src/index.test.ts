...
describe("local vcs", () => {
  ...
  it("reads patch-free Git file summaries with statuses, renames, binary files, and exact counts", async () => {
    ...
    // Paths shaped like the delimiters of git's own machine formats.
  ...
  it("summarizes an edited rename in one Git call", async () => {
    ...
    // A rename git scored below 100% still carries its counts.
    await expect(
    ...
    // Statuses and counts come out of a single diff.
    expect(spawns).toHaveLength(1);
  });

  it("reads a copied file without dropping the records after it", () => {
    /* `-C` is never passed, so git does not emit a copy today; a record that
       carried one must still not swallow the rest of the diff. */
    const raw = [
  ...
  it("refuses a diff whose counts arrive before its records", () => {
    ...
    // Silently reading as "no changes" would be worse than failing.
    expect(() =>
  ...
  it("reads no file where a jj workspace has a directory", async () => {
    ...
    // A tree is not a file, and jj answers a directory with empty content.
    await expect(
  ...
  it("falls back to colocated Git when a jj root cannot answer", async () => {
    ...
    // A jj-kind handle over a root with no jj workspace reads through Git.
    await expect(
      readFileAtCommit({
        rootPath,
        kind: "jj",
