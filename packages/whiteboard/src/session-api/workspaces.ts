...
import { whiteboardManagedCheckoutRoot } from "../whiteboard-checkout-paths.js";
import { ensureWhiteboardPinnedCheckout } from "../whiteboard-head-checkout.js";
import {
  markerMatches,
  prepareWhiteboardPinnedCheckout,
  removeWhiteboardPrepareArtifacts,
  whiteboardPrepareCommandsHash,
  whiteboardPrepareLogPath,
  whiteboardPrepareMarkerPath,
} from "../whiteboard-prepare.js";
 ...
 */
export class WhiteboardWorkspaces {
  ...
  attachExternalWhiteboards(source: {
    has(id: string): boolean;
    subscribe(listener: () => void): () => void;
  }) {
  ...
  private hasWhiteboard(id: string): boolean {
  ...
  private async acquire(
    ...
    try {
      ...
      const commandsHash = whiteboardPrepareCommandsHash(commands);
      ...
      if (!this.hasWhiteboard(sessionId)) {
      ...
      if (!commands.length) {
        ...
        await markerMatches(whiteboardPrepareMarkerPath(checkout), commandsHash)
      ) {
  ...
  private prepare(environment: Environment, commands: string[]) {
    ...
    const done = prepareWhiteboardPinnedCheckout({
      ...
      .then(async (result) => {
        ...
        if (!result.prepared)
          environment.log = await readFile(
            whiteboardPrepareLogPath(environment.rootPath!),
            "utf8",
  ...
  async retry(sessionId: string, id: string): Promise<WorkspaceStatus> {
    ...
    if (environment.rootPath)
      await rm(whiteboardPrepareMarkerPath(environment.rootPath), {
        force: true,
      });
  ...
  private collect(retryId?: string, removedWhiteboardId?: string) {
    // Capture ownership before awaiting, so shutdown never reads a closed store.
    const deleted = this.all().filter(
      (environment) =>
        (environment.sessionId === removedWhiteboardId ||
          !this.hasWhiteboard(environment.sessionId)) &&
      ...
      for (const environment of deleted) {
        ...
        try {
          if (environment.rootPath && existsSync(environment.rootPath)) {
            const managed = whiteboardManagedCheckoutRoot(
          ...
          if (environment.rootPath)
            await removeWhiteboardPrepareArtifacts(environment.rootPath);
