...
import { readWhiteboardDocumentBundle } from "./whiteboard-bundle";
import { isDerivedWhiteboardPath } from "./whiteboard-derived-paths";
import {
...
const corpus = process.env.WHITEBOARD_LEGACY_CORPUS;
  ...
  return !relative
    ...
    .some(
      (part) =>
        part === ".mutation-lock" ||
        part.endsWith("-shm") ||
        (isDerivedWhiteboardPath(part) && !/^review\.db(?:-|$)/.test(part)),
    );
}

async function sourceSnapshot(root: string): Promise<string> {
  const files = await snapshotWhiteboardTree(root, {
    include: included,
  ...
  it("migrates isolated corpus copies without loss", async () => {
    ...
    try {
      ...
      for (const uuid of uuids) {
        ...
        const validated = parseAnyStoredWhiteboardRecord(original);
        ...
        if (legacySession)
          await migrateStoredWhiteboard({ whiteboardDir: dir });
        ...
        if (loaded.review.presentedDocumentRevision) {
          ...
          const bundle = await readWhiteboardDocumentBundle(out, "/");
        ...
        if (loaded.review.presentedSoftwareMapRevision) {
          ...
          const bundle = await readWhiteboardSoftwareMapBundle(out);
        ...
        const repeated = await readStoredWhiteboard(dir);
        expect("error" in repeated).toBe(false);
        expect(digest(await snapshotWhiteboardTree(dir))).toBe(
          digest(snapshot),
        );
        rows.push({
          uuid,
