import { displayedSources, evidenceRows } from "../common/reviewSearchEvidence.js";
...
export class ReviewCodeResourceService extends Disposable implements IReviewCodeResourceService {
  ...
  acquireEvidence(result: SearchResultData, target: ReviewCodeDiffTarget): ReviewUnifiedCodeModelReference {
    ...
    const entry: ReviewUnifiedResourceEntry = {
      model, rows, originalLineCount: displayedSources(result).lhs?.text.split('\n').length ?? 0,
      modifiedLineCount: displayedSources(result).rhs?.text.split('\n').length ?? 0, references: 1,
