...
import {
  ...
  reviewAppSourcePath,
  reviewAuthoringTypesPath,
  reviewModelModulePath,
} from "./package-paths";
...
describe("compiler resource paths", () => {
  ...
  it("resolves default resources in Review when the root finder lives in trace-core", () => {
    expect(findReviewPackageRoot()).toBe(packageRoot);
    expect(reviewAppSourcePath()).toBe(path.join(packageRoot, "app", "src"));
    expect(reviewAuthoringTypesPath()).toBe(
      path.join(packageRoot, "src", "authoring.ts"),
    );
    const model = reviewModelModulePath("software-map-model.ts");
  ...
  it("resolves the review app source directory inside the package", () => {
    for (const moduleUrl of moduleUrls) {
      expect(reviewAppSourcePath(moduleUrl)).toBe(
  ...
  it("uses fresh source types in development and generated declarations after packaging", () => {
    expect(reviewAuthoringTypesPath(moduleUrls[0])).toBe(
      path.join(packageRoot, "src", "authoring.ts"),
    );

    for (const moduleUrl of moduleUrls.slice(1)) {
      expect(reviewAuthoringTypesPath(moduleUrl)).toBe(
