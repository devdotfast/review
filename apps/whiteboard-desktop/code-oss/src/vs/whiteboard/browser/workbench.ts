...
import { WhiteboardErrorReportLimiter } from '../common/whiteboardErrorReport.js';
...
import { MainWhiteboardTitlebarPart, WhiteboardTitleService } from './parts/whiteboardTitlebarPart.js';
import { IContextKeyService, RawContextKey } from '../../platform/contextkey/common/contextkey.js';
import { IsWhiteboardWindowContext } from '../../workbench/common/contextkeys.js';
import {
	NotificationsPosition,
	NotificationsSettings,
	getNotificationsPosition
} from '../../workbench/common/notifications.js';
import { IWhiteboardCanvasPartsService } from './parts/canvas/whiteboardCanvasPart.js';
import { IWhiteboardExplorerPartsService } from './parts/explorer/whiteboardExplorerPart.js';
import { initialWhiteboardPartVisibility } from '../common/whiteboardWorkbenchVisibility.js';
...
//#region WhiteboardWorkbench Options
...
enum LayoutClasses {
	...
	EXPLORER_VISIBLE = 'whiteboard-explorer-visible',
...
const WHITEBOARD_CHROME_INSET_PROPERTIES = ['--whiteboard-chrome-left-width', '--whiteboard-chrome-right-width', '--whiteboard-explorer-width'] as const;
export class WhiteboardWorkbench extends Disposable implements IAgentWorkbenchLayoutService {
	...
	private readonly whiteboardChromeTabStrips: HTMLElement[] = [];
	private readonly whiteboardChromeDragListeners = this._register(new DisposableStore());
	private readonly whiteboardChromeInsetScheduler = this._register(new RunOnceScheduler(() => this.updateWhiteboardChromeInset(), 0));
	...
	private readonly errorReportLimiter = new WhiteboardErrorReportLimiter();
	...
	private reportUnexpectedError(error: unknown): void {
		this.errorReportLimiter.report(error, report => {
			this.whiteboardTelemetryService ??= this.workbenchInstantiationService?.invokeFunction(accessor => accessor.get(IWhiteboardTelemetryService));
			this.whiteboardTelemetryService?.capture('client_error', {
	...
	startup(): IInstantiationService {
		try {
			...
			instantiationService.invokeFunction(accessor => {
				...
				IsWhiteboardWindowContext.bindTo(contextKeyService).set(true);

				// Register Listeners
				this.registerListeners(lifecycleService, storageService, configurationService, hostService, dialogService);

				// Render WhiteboardWorkbench
				this.renderWorkbench(instantiationService, notificationService, storageService, configurationService);

				// WhiteboardWorkbench Layout
				this.createWorkbenchLayout();

				// WhiteboardWorkbench Management
	...
	private initServices(serviceCollection: ServiceCollection): IInstantiationService {
		...
		serviceCollection.set(ITitleService, new SyncDescriptor(WhiteboardTitleService, []));
	...
	private _applyInitialPartVisibility(): void {
		this.storageService.remove(WhiteboardWorkbench._PART_VISIBILITY_KEY, StorageScope.WORKSPACE);
		Object.assign(this.partVisibility, initialWhiteboardPartVisibility);
	}

	private _loadPartSizes(storageService: IStorageService): IPartSizesState {
		const raw = storageService.get(WhiteboardWorkbench._PART_SIZES_KEY, StorageScope.WORKSPACE);
		if (raw) {
			try {
				return JSON.parse(raw);
			} catch {
				// Corrupted data — remove the bad key so we don't keep warning on every startup
				storageService.remove(WhiteboardWorkbench._PART_SIZES_KEY, StorageScope.WORKSPACE);
			}
		}
		return {};
	}
	...
	private _savePartSizes(): void {
		...
		this.storageService.store(WhiteboardWorkbench._PART_SIZES_KEY, JSON.stringify(sizes), StorageScope.WORKSPACE, StorageTarget.MACHINE);
	}
	...
	private renderWorkbench(instantiationService: IInstantiationService, notificationService: NotificationService, storageService: IStorageService, configurationService: IConfigurationService): void {
		...
		for (const { id, role, classes } of [
			...
			partContainer.classList.add('whiteboard-chrome-overlay');
		...
		// Add WhiteboardWorkbench to DOM
	...
	private createExplorerPart(): void {
		...
		explorerPartContainer.classList.add('part', 'whiteboard-explorer');
		explorerPartContainer.id = Parts.WHITEBOARD_EXPLORER_PART;
		explorerPartContainer.setAttribute('role', 'navigation');

		mark(`code/willCreatePart/${Parts.WHITEBOARD_EXPLORER_PART}`);
		this.getPart(Parts.WHITEBOARD_EXPLORER_PART).create(explorerPartContainer);
		mark(`code/didCreatePart/${Parts.WHITEBOARD_EXPLORER_PART}`);
	...
	initLayout(accessor: ServicesAccessor): void {
		...
		accessor.get(IWhiteboardCanvasPartsService);
		// Same for the file explorer part, which also owns its own show/hide policy.
		accessor.get(IWhiteboardExplorerPartsService);
		...
		this._register(editorMainPart.onDidLayout(() => this.whiteboardChromeInsetScheduler.schedule()));
		const titlebarPart = this.getPart(Parts.TITLEBAR_PART);
		if (titlebarPart instanceof MainWhiteboardTitlebarPart) {
			this._register(titlebarPart.onDidChangeChromeInsets(() => this.whiteboardChromeInsetScheduler.schedule()));
		}
		this._register(editorMainPart.onDidAddGroup(() => this.whiteboardChromeInsetScheduler.schedule()));
		this._register(editorMainPart.onDidRemoveGroup(() => this.whiteboardChromeInsetScheduler.schedule()));
		this._register(editorMainPart.onDidMoveGroup(() => this.whiteboardChromeInsetScheduler.schedule()));
	 ...
	 * later restores the same dimensions.
	 */
	 ...
	 */
	private publishWhiteboardChromeInset(tabStrip: HTMLElement): void {
		const titlebarPart = this.getPart(Parts.TITLEBAR_PART);
		const insets = titlebarPart instanceof MainWhiteboardTitlebarPart ? titlebarPart.chromeInsets : { left: 0, right: 0 };
		const explorerWidth = this.isVisible(Parts.WHITEBOARD_EXPLORER_PART) ? this.getSize(Parts.WHITEBOARD_EXPLORER_PART).width : 0;
		tabStrip.style.setProperty('--whiteboard-chrome-left-width', `${insets.left}px`);
		tabStrip.style.setProperty('--whiteboard-chrome-right-width', `${insets.right}px`);
		tabStrip.style.setProperty('--whiteboard-explorer-width', `${explorerWidth}px`);
	}

	private updateWhiteboardChromeInset(): void {
		this.whiteboardChromeDragListeners.clear();
		this.mainContainer.classList.remove('whiteboard-tab-dragging');
		for (const tabStrip of this.whiteboardChromeTabStrips) {
			tabStrip.classList.remove('whiteboard-chrome-tab-strip', 'whiteboard-chrome-inset');
			for (const property of WHITEBOARD_CHROME_INSET_PROPERTIES) {
				tabStrip.style.removeProperty(property);
			}
		}
		this.whiteboardChromeTabStrips.length = 0;
		...
		const finishTabDrag = () => this.mainContainer.classList.remove('whiteboard-tab-dragging');
		...
		for (const { group, rect } of visibleGroups) {
			...
			this.whiteboardChromeTabStrips.push(tabStrip);
			tabStrip.classList.add('whiteboard-chrome-tab-strip');
			if (group === topLeftGroup) {
				tabStrip.classList.add('whiteboard-chrome-inset');
				this.publishWhiteboardChromeInset(tabStrip);
			}
			this.whiteboardChromeDragListeners.add(addDisposableListener(tabStrip, 'dragstart', () => this.mainContainer.classList.add('whiteboard-tab-dragging')));
			this.whiteboardChromeDragListeners.add(addDisposableListener(tabStrip, 'dragend', finishTabDrag));
			this.whiteboardChromeDragListeners.add(addDisposableListener(tabStrip, 'drop', finishTabDrag));
		}
	}

	//#endregion

	//#region WhiteboardWorkbench Layout Creation

	createWorkbenchLayout(): void {
		const editorPart = this.getPart(Parts.EDITOR_PART);
		const sessionsPart = this.getPart(Parts.SESSIONS_PART);
		const explorerPart = this.getPart(Parts.WHITEBOARD_EXPLORER_PART);
		...
		const viewMap: { [key: string]: ISerializableView } = {
			...
			[Parts.WHITEBOARD_EXPLORER_PART]: this.explorerPartView
		};
	...
	private createDesktopGridDescriptor(width: number, height: number): ISerializedGrid {
		...
		const explorerNode: ISerializedLeafNode = {
			type: 'leaf',
			data: { type: Parts.WHITEBOARD_EXPLORER_PART },
	...
	layout(): void {
		...
		this.logService.trace(`WhiteboardWorkbench#layout, height: ${this._mainContainerDimension.height}, width: ${this._mainContainerDimension.width}`);
		...
		// ground rather than a margin around a card; whiteboard.css paints it with the editor
		...
		this.getPart(Parts.TITLEBAR_PART).layout(gridWidth, WHITEBOARD_CHROME_HEIGHT, 0, 0);
		this.updateWhiteboardChromeInset();
	...
	isVisible(part: Parts, targetWindow?: Window): boolean {
		switch (part) {
			...
			case Parts.WHITEBOARD_EXPLORER_PART:
	...
	private setSessionsHidden(hidden: boolean): void {
		...
		// collapse. See `initialWhiteboardPartVisibility`.
		if (!hidden) {
			return;
		}
	 ...
	 *
	 * `WhiteboardExplorerParts` is the only caller: it derives the state from the
	...
	getSize(part: Parts): IViewSize {
		if (part === Parts.TITLEBAR_PART) {
			return { width: this._mainContainerDimension.width, height: WHITEBOARD_CHROME_HEIGHT };
		}
	...
	getVisibleNeighborPart(part: Parts, direction: Direction): Parts | undefined {
		...
		if (neighborView === this.explorerPartView) {
			return Parts.WHITEBOARD_EXPLORER_PART;
		}
