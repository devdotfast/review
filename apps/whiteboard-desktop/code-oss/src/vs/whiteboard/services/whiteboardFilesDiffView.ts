import type { WhiteboardDiffProgress, WhiteboardDiffLens, WhiteboardDiffViewSpec } from "../common/whiteboardProtocol.js";
...
import "../browser/media/whiteboard.css";
...
import { WhiteboardChangedFilesTree } from "../browser/whiteboardChangedFilesTree.js";
import { type WhiteboardDiffFileWire, type StructuralLineCounts } from "../common/whiteboardProtocol.js";
import type { WhiteboardDiffLayoutSetting } from "./whiteboardDiffLayout.js";
import { whiteboardMultiDiffLabelUris, WhiteboardMultiDiffUIElementFactory } from "./whiteboardMultiDiff.js";
...
const WHITEBOARD_FILES_DIFF_EDITOR_OPTIONS = {
...
export interface WhiteboardFilesEditorEntry {
	readonly sectionId?: string;
	readonly sectionStart?: boolean;
	readonly file: WhiteboardDiffFileWire;
	readonly original: URI | undefined;
	readonly modified: URI | undefined;
	readonly goToFileResource: URI;
}

export class WhiteboardFilesEditorInput extends MultiDiffEditorInput {
	static override readonly ID = "workbench.input.devfast.whiteboardFiles";
	static readonly EDITOR_ID = "workbench.editor.devfast.whiteboardFiles";
	...
	constructor(
		source: URI,
		readonly entries: readonly WhiteboardFilesEditorEntry[],
		...
		const items = entries.map(
				(entry) =>
					new MultiDiffEditorItem(
						...
						undefined,
						undefined,
						whiteboardMultiDiffLabelUris(entry.file),
					...
					(structural || lens)
						? {
							...WHITEBOARD_FILES_DIFF_EDITOR_OPTIONS,
						...
						: { ...WHITEBOARD_FILES_DIFF_EDITOR_OPTIONS, hideOriginalLineNumbers: entry.file.status === "added" },
					),
		);
	...
	override get typeId(): string {
		return WhiteboardFilesEditorInput.ID;
	}

	override get editorId(): string {
		return WhiteboardFilesEditorInput.EDITOR_ID;
	}
}
 ...
 * layout call.
 */
export class WhiteboardFilesDiffView extends Disposable {
	...
	private readonly changedFilesTree: WhiteboardChangedFilesTree | undefined;
	private readonly widget: MultiDiffEditorWidget;
	private viewModel: MultiDiffEditorViewModel | undefined;
	private input: WhiteboardFilesEditorInput | undefined;
	...
	>();
	private readonly headerFactory: WhiteboardMultiDiffUIElementFactory;
	...
	constructor(
		...
		layout: WhiteboardDiffLayoutSetting,
		...
		private readonly document: WhiteboardDiffViewSpec["document"],
		@IInstantiationService
		private readonly whiteboardInstantiationService: IInstantiationService,
		@IEditorService private readonly editorService: IEditorService,
		@IEditorGroupsService
		private readonly editorGroupService: IEditorGroupsService,
	) {
		super();
		this.root = append(container, $(".whiteboard-files-editor"));
		const fileTree = document ? container.ownerDocument.createElement("div") : append(this.fileTreeContainer ?? this.root, $(".whiteboard-files-editor-tree"));
		const diffContainer = append(this.root, $(".whiteboard-files-editor-diffs"));
		this.diffContainer = diffContainer;
		if (document) fileTree.hidden = true;

		const factory = this.whiteboardInstantiationService.createInstance(
			WhiteboardMultiDiffUIElementFactory,
			() =>
				this.input
		...
		this.widget = this._register(
			this.whiteboardInstantiationService.createInstance(MultiDiffEditorWidget, diffContainer, factory, undefined),
		);
		...
		this.streamStatus = append(diffContainer,
			$(".whiteboard-structural-stream-status"));
		...
		this.summary = append(fileTree, $(".whiteboard-files-editor-summary"));
		this.summary.hidden = true;
		this.changedFilesTree = document ? undefined : this._register(
			this.whiteboardInstantiationService.createInstance(WhiteboardChangedFilesTree, fileTree),
		);
	...
	async setInput(input: WhiteboardFilesEditorInput, viewState: IMultiDiffEditorViewState | undefined): Promise<void> {
		this.input = input;
	...
	startLoading(entries: readonly WhiteboardFilesEditorEntry[]): void {
	...
	private renderSummary(): void {
		...
		if (complete) {
			...
			append(this.summary, $("span.whiteboard-files-editor-summary-added")).textContent = `+${format.format(total.added).toLowerCase()}`;
			append(this.summary, $("span.whiteboard-files-editor-summary-removed")).textContent = `−${format.format(total.removed).toLowerCase()}`;
	...
	private entryProgress(entry: WhiteboardFilesEditorEntry) {
	...
	private progressTitle(entry: WhiteboardFilesEditorEntry): string | undefined {
	...
	revealSource(source: WhiteboardDiffLens['ranges'][number], sectionId?: string): void {
	 ...
	 */
	sourceOffset(source: WhiteboardDiffLens['ranges'][number]): number | undefined {
