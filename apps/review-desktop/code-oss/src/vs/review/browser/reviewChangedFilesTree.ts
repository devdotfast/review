    ...
    >
{
  ...
  constructor(private readonly progress: Map<string, ReviewDiffProgressFile>, private readonly viewed: Set<string>, private readonly states: Map<string, { status: "loading" | "error"; message?: string }>) {}
  ...
  private renderElements(
    ...
    if (isFile) {
      ...
      const additions = progress?.remaining.additions ?? element.file.additions;
      const deletions = progress?.remaining.deletions ?? element.file.deletions;
      ...
      template.counts.title = element.file.status === "unchanged" ? "Referenced context; no changed lines" : `Remaining +${additions} −${deletions} · Total +${progress?.total.additions ?? element.file.additions} −${progress?.total.deletions ?? element.file.deletions}`;
    }
...
export class ReviewChangedFilesTree extends Disposable {
  ...
  constructor(
    ...
    const renderer = new ChangedFilesTreeRenderer(this.progress, this.viewed, this.states);
