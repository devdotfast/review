...
import {
  ...
  removeLegacyWhiteboardCheckouts,
} from "./whiteboard-head-checkout";
import {
...
export type WhiteboardPackageManager = "npm" | "pnpm" | "yarn" | "bun";
...
interface RunWhiteboardMigrationRuntime {
  migrateStoredWhiteboardData: typeof migrateStoredWhiteboardData;
...
export async function runWhiteboardMigration(input: {
  ...
  stdout: Writable;
  stderr: Writable;
  runtime?: Partial<RunWhiteboardMigrationRuntime>;
}): Promise<number> {
  const human = humanStream(input);
  const env = input.env ?? process.env;
  const homeDir = input.homeDir ?? os.homedir();
  const whiteboardHome = devWhiteboardHome(env, homeDir);
  const packageRoot = input.packageRoot ?? defaultPackageRoot();

  const runtime: RunWhiteboardMigrationRuntime = {
  ...
  const stored = await runMigrationPhase(
    ...
    () =>
      runtime.migrateStoredWhiteboardData({
        whiteboardHome,
  ...
  const jj = await runMigrationPhase(
    ...
    () =>
      runtime.migrateJjWhiteboardRepositories({
        whiteboardHome,
        skipWhiteboardUuids: stored.failedWhiteboardUuids,
        force: input.force,
        log: (message) => input.stderr.write(`${message}\n`),
      }),
    blockers,
  ...
  const managedCheckouts = await runMigrationPhase(
    ...
    () =>
      runtime.migrateWhiteboardManagedCheckouts({
        whiteboardHome,
        skipWhiteboardUuids: stored.failedWhiteboardUuids,
        log: (message) => input.stderr.write(`${message}\n`),
      }),
  ...
  const catalog = await runMigrationPhase(
    ...
    () => runtime.removeLegacyDesktopCatalog({ whiteboardHome }),
    blockers,
  );
  ...
  const globalCli = await runMigrationPhase(
    ...
    { checked: 0, removed: 0, blockers: [] },
    () =>
      runtime.removeLegacyGlobalWhiteboardInstalls({
        packageRoot,
        homeDir,
        env,
  ...
  emitJsonEvent(input, {
    ...
    droppedWhiteboards: stored.droppedWhiteboards,
    droppedLegacyPeekWhiteboards: stored.droppedLegacyPeekWhiteboards,
...
export async function migrateWhiteboardManagedCheckouts(input: {
  whiteboardHome: string;
  skipWhiteboardUuids?: readonly string[];
  ...
  for (const entry of await readDirectory(reviewsRoot)) {
    ...
    if (input.skipWhiteboardUuids?.includes(entry.name)) continue;
    const whiteboardDir = path.join(reviewsRoot, entry.name);
    result.checked += 1;

    try {
      const review = parseStoredWhiteboardRecord(
        JSON.parse(
      ...
      for (const pin of pins) {
        const checkout = await ensureWhiteboardPinnedCheckout({
      ...
      result.blockers.push(`${whiteboardDir}: ${errorMessage(error)}`);
    }
  }

  for (const sourceRoot of sourceRoots) {
    try {
      result.legacyRemoved += await removeLegacyWhiteboardCheckouts({
        rootPath: sourceRoot,
...
export async function migrateJjWhiteboardRepositories(input: {
  whiteboardHome: string;
  skipWhiteboardUuids?: readonly string[];
  force?: boolean;
  log?: (message: string) => void;
  ...
  for (const entry of await readDirectory(reviewsRoot)) {
    ...
    if (!(await pathExists(path.join(whiteboardDir, ".jj")))) continue;
    ...
    try {
      ...
      const parsed = parseAnyStoredWhiteboardRecord(JSON.parse(recordSource));
      ...
      if (
        ...
        (await whiteboardVcs.log(whiteboardDir)).length > 0
      ) {
        input.log?.(
      ...
      await resetJjWhiteboardRepository({
        whiteboardDir,
        review: parsed,
        recordSource,
        force: input.force,
...
async function resetJjWhiteboardRepository(input: {
  whiteboardDir: string;
  review: StoredWhiteboardRecord;
  recordSource: string;
  force?: boolean;
}): Promise<void> {
  const gitDir = path.join(input.whiteboardDir, ".git");
  const jjDir = path.join(input.whiteboardDir, ".jj");
  const backupDir = `${input.whiteboardDir}.review-migrate-git-backup`;
  ...
  try {
    ...
    await whiteboardVcs.init(input.whiteboardDir);
    ...
    const revision = await whiteboardVcs.seal(
      input.whiteboardDir,
      "Migrate Review history to plain Git",
    );

    await whiteboardVcs.resolve(input.whiteboardDir, revision);

    const stored = parseStoredWhiteboardRecord(
      JSON.parse(
...
function isLegacyDesktopCatalogRecord(
  ...
  return (
    record.whiteboardKey === whiteboardKey &&
    requiredStrings
      .map(jsonString)
...
export async function removeLegacyGlobalWhiteboardInstalls(input: {
  packageRoot: string;
  homeDir: string;
  env: NodeJS.ProcessEnv;
...
async function globalPackageRoots(input: {
  manager: WhiteboardPackageManager;
  homeDir: string;
  runCommand: RunCommand;
}): Promise<string[]> {
  try {
...
function packageManagerUninstall(manager: WhiteboardPackageManager): string[] {
