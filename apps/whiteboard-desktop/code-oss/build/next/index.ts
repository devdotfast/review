...
const desktopEntryPoints = [
	'vs/whiteboard/whiteboard.desktop.main',
	'vs/whiteboard/electron-utility/whiteboardDesktopHostMain',
 ...
 * Uses desktop resource patterns from the old build. Only called by bundle().
 */
async function copyResources(outDir: string): Promise<void> {
	console.log(`[resources] Copying to ${outDir}...`);
	let copied = 0;

	const ignorePatterns = ['**/test/**', '**/*-dev.html'];

	for (const pattern of desktopResourcePatterns) {
...
function inlineReviewBrowserDependenciesPlugin(): esbuild.Plugin {
	return {
		name: 'inline-review-browser-dependencies',
		setup(build) {
			build.onResolve({ filter: /^(?:eventsource-parser|zod(?:\/.*)?)$/ }, args => ({
 ...
 */
function fileContentMapperPlugin(outDir: string): esbuild.Plugin {
...
async function bundle(outDir: string, doMinify: boolean, doNls: boolean, doManglePrivates: boolean, sourceMapBaseUrl?: string): Promise<void> {
	...
	console.log(`[bundle] ${SRC_DIR} → ${outDir}${doMinify ? ' (minify)' : ''}${doNls ? ' (nls)' : ''}${doManglePrivates ? ' (mangle-privates)' : ''}`);
	const t1 = Date.now();
 ...
 *--------------------------------------------------------*/`,
	...
	await Promise.all(entryPoints.map(async (entryPoint) => {
		...
		if (entryPoint === 'vs/whiteboard/whiteboard.desktop.main') {
		...
		const needsCssBundling = cssBundleEntryPoints.has(entryPoint);
...
async function main(): Promise<void> {
	...
	try {
		switch (command) {
			...
			case 'bundle':
				await bundle(options.out ?? OUT_VSCODE_DIR, options.minify, options.nls, options.manglePrivates, options.sourceMapBaseUrl);
				break;
