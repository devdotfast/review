...
import type { WhiteboardDiffLens } from './whiteboardProtocol.js';

/** Project pinned ranges onto the current diff's correspondence, never onto another revision. */
export function lensContextGaps(diff: IDocumentDiff, originalCount: number, modifiedCount: number, ranges: WhiteboardDiffLens['ranges']): IDocumentContextGap[] {
...
export function viewedContextGaps(diff: IDocumentDiff, originalCount: number, modifiedCount: number, viewed: WhiteboardDiffLens['ranges'], changed: WhiteboardDiffLens['ranges']): IDocumentContextGap[] {
	const rows = alignmentRows(diff, originalCount, modifiedCount);
	const contains = (ranges: WhiteboardDiffLens['ranges'], side: 0 | 1, line: number) => ranges.some(range => range.side === (side === 0 ? 'base' : 'head') && line + 1 >= range.fromLine && line + 1 <= range.toLine);
