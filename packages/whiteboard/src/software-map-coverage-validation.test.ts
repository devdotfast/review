  ...
  it("errors when child coverage is not contained by parent coverage", () => {
    const model = defineSoftwareMap({
      systems: {
        product: {
          ...
          containers: {
            review: {
              coverage: { globs: ["packages/whiteboard/app/lib/**"] },
            },
          },
        },
      },
    ...
    expect(
      collectSoftwareMapCoverageErrors({
        ...
        model,
        listFiles: () => [
          "packages/whiteboard/src/runtime.ts",
          "packages/whiteboard/app/lib/review-api.ts",
        ],
      }),
    ).toEqual([
      'SoftwareMap coverage: "product" must cover child "product.review" file "packages/whiteboard/app/lib/review-api.ts" from child glob "packages/whiteboard/app/lib/**".',
    ]);
  });
