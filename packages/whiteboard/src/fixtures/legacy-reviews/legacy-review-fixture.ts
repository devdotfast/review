...
export const LEGACY_WHITEBOARD_FIXTURES_ROOT = path.resolve(
  import.meta.dirname,
);

export function listLegacyWhiteboardFixtures(): LegacyReviewFixtureMetadata[] {
  return readdirSync(LEGACY_WHITEBOARD_FIXTURES_ROOT)
...
function readMetadata(name: string): LegacyReviewFixtureMetadata {
  ...
  const metadata: LegacyReviewFixtureMetadata = JSON.parse(
    readFileSync(
      path.join(LEGACY_WHITEBOARD_FIXTURES_ROOT, `${name}.json`),
      "utf8",
    ),
  );
...
export async function extractLegacyWhiteboardFixture(
  name: string,
  ...
  try {
    ...
    await execFilePromise("tar", [
      "-xzf",
      path.join(LEGACY_WHITEBOARD_FIXTURES_ROOT, `${name}.tgz`),
      "-C",
      dir,
    ]);
...
function includeReviewTreeEntry(relative: string): boolean {
  ...
  return (
    name !== ".mutation-lock" &&
    (!isDerivedWhiteboardPath(name) || /^review\.db(?:-|$)/.test(name))
  );
}

/** Content digests for every included file under `dir`, keyed by relative path. */
export async function snapshotWhiteboardTree(
  dir: string,
  options: {
...
export async function readLegacyWhiteboardGolden(
  name: string,
