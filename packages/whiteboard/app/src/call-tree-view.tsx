...
import type { CallStackDiffBlock } from "../../src/session-api/blocks/call_stack_diff";
...
export function CallTree({
  ...
  return (
    <nav
      className="whiteboard-lens-sidebar whiteboard-lens-sidebar--callstack"
      aria-label="Call tree"
    >
      <div className="whiteboard-call-tree">
        {stops.map((stop, index) => {
          ...
          return (
            <div
              className={`whiteboard-call-entry ${stats?.state === "viewed" ? "is-viewed" : ""}`}
              key={stop.id}
            >
              <TreeConnectors
                stop={stop}
              ...
              <button
                ...
                className={`whiteboard-call-row whiteboard-call-row--${change}`}
                data-whiteboard-anchor-id={stop.anchorId}
              ...
              >
                <span className="whiteboard-call-name">{stop.label}</span>
                {unavailable && (
                  <span className="whiteboard-lens-stats">
                    {availability === "pending" ? "…" : "Unavailable"}
                  </span>
                )}
                {!unavailable && stats && change !== "unchanged" && (
                  <span
                    className="whiteboard-lens-stats"
...
function TreeConnectors({
  ...
  return (
    <svg
      className="whiteboard-call-connectors"
      width={width}
      height="26"
    ...
    >
      ...
      <g
        ...
        className={stop.callSite ? "whiteboard-call-edge" : undefined}
      ...
      >
        ...
        {stop.callSite ? (
          <>
            <path
              className="whiteboard-call-edge-highlight"
              d={`M ${branchX} ${-(parentDistance - 1) * 26} V 13 H ${width}`}
            />
            <path
              className="whiteboard-call-edge-hit"
...
export function DocumentCallTree({ block }: { block: CallStackDiffBlock }) {
  ...
  return (
    <figure
      className="whiteboard-document-call-tree"
      data-whiteboard-call-stack="ready"
    >
