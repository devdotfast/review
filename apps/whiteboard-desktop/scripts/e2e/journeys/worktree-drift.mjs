...
function canvasUi(ctx) {
  const canvas = ctx.page.locator(".whiteboard-canvas-root [data-whiteboard-api]");
  ...
  return {
    ...
    peek: canvas
      .locator('.whiteboard-inline-editor[data-whiteboard-inline-editor="order.ts"]')
      .first(),
  };
}
...
export async function run(ctx) {
  ...
  await pickWhiteboard(ctx, review.sessionId);
  ...
  await ctx.restartDesktop();
  await pickWhiteboard(ctx, review.sessionId, moved);
  ...
  const info = await ctx.cliRaw(
    ["info", "--session", review.sessionId, "--json"],
    moved,
  );
  ...
  await openHome(ctx);
  await ctx.page
    .locator("main.whiteboard-home .whiteboard-home-card")
  ...
  const commits = ctx.page.locator(".whiteboard-view-region--commits");
