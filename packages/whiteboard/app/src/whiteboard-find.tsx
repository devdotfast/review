...
import { compileWhiteboardFindQuery } from "./whiteboard-find-query";
import { whiteboardFindRanges } from "./whiteboard-find-text";
import { useWhiteboardRoots } from "./whiteboard-root-context";

const ALL_HIGHLIGHT = "whiteboard-find-match";

const ACTIVE_HIGHLIGHT = "whiteboard-find-match-active";
...
export interface WhiteboardFindHost extends FindController {
  attach(controller: FindController | null): void;
}

export function createWhiteboardFindHost(): WhiteboardFindHost {
...
export interface WhiteboardInlineFindRegistration {
  container: HTMLElement;
  setFindQuery(query: WhiteboardFindQuery): Promise<{ matchCount: number }>;
  revealFindMatch(index: number): Promise<void>;
  clearFind(): void;
  getHandle(): WhiteboardInlineEditorHandle | null;
  expand(): void;
}

interface FindContextValue {
  register(registration: WhiteboardInlineFindRegistration): () => void;
  setWhiteboardActive(active: boolean): void;
}

const WhiteboardFindContext = createContext<FindContextValue | null>(null);

export function useWhiteboardFindRegistration(): FindContextValue | null {
  return useContext(WhiteboardFindContext);
}
...
type UnifiedMatch =
  ...
  | {
      kind: "editor";
      registration: WhiteboardInlineFindRegistration;
      localIndex: number;
      node: Node;
    };

export function WhiteboardFindProvider({
  articleRef,
  scrollRegionRef,
  documentKey,
  host,
  ...
  documentKey: string;
  host?: WhiteboardFindHost;
  children: ReactNode;
}) {
  const registrations = useRef(new Set<WhiteboardInlineFindRegistration>());
  ...
  const shellRef = useWhiteboardRoots()?.shellRef;
  const inputRef = useRef<HTMLInputElement | null>(null);
  const whiteboardActive = useRef(true);
  ...
  const showFind = useCallback(
    (seed?: string) => {
      if (!whiteboardActive.current) return false;
  ...
  const reveal = useCallback(
    (index: number, currentMatches = matchesRef.current) => {
      ...
      if (match.kind === "mdx") {
        expandWhiteboardSection(match.node);
        requestAnimationFrame(() => {
  ...
  useEffect(() => {
    ...
    const query: WhiteboardFindQuery = {
      text: queryText,
      matchCase,
      wholeWord,
      isRegex,
    ...
    const compiled = compileWhiteboardFindQuery(query);
    ...
    const mdxMatches: UnifiedMatch[] = article
      ? whiteboardFindRanges(article, compiled.expression).map((range) => ({
          kind: "mdx" as const,
          range,
  ...
  const context = useMemo<FindContextValue>(
    () => ({
      ...
      setWhiteboardActive(active) {
        whiteboardActive.current = active;
  ...
  return (
    <WhiteboardFindContext.Provider value={context}>
      {children}
      {open && overlayHost
        ? createPortal(
            <div
              className="whiteboard-find-widget"
              role="search"
              aria-label="Find in session"
            >
              <div className="whiteboard-find-input-shell">
                <input
                  ref={inputRef}
                  aria-label="Find"
                ...
                />
                <div
                  className="whiteboard-find-options"
                ...
                >
                  ...
                  <FindToggle
                    ...
                    className="whiteboard-find-toggle--whole-word"
                    active={wholeWord}
                  ...
                  <FindToggle
                    ...
                    className="whiteboard-find-toggle--regex"
                    active={isRegex}
              ...
              <span className="whiteboard-find-count" aria-live="polite">
                {invalid
                  ? "Invalid expression"
                  : searching
                    ? "Searching…"
        ...
        : null}
    </WhiteboardFindContext.Provider>
  );
}
...
function FindActionButton({
  ...
  return (
    <button
      type="button"
      className="whiteboard-find-action"
      aria-label={label}
      title={description}
      disabled={disabled}
      onMouseDown={(event) => event.preventDefault()}
...
function expandWhiteboardSection(node: Node): void {
  const element = node instanceof Element ? node : node.parentElement;
  element
    ?.closest(".whiteboard-section--collapsed")
    ?.dispatchEvent(new CustomEvent("whiteboard-section-expand"));
}
