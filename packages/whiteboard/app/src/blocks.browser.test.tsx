...
import {
  ...
  elements,
  isUnit,
} from "../../src/session-api/document";
import { mapInputSchema } from "../../src/session-api/map-input";
import type { Snapshot } from "../../src/session-api/store";
import type { WhiteboardProgress } from "../../src/session-api/whiteboard-progress";
...
import { mountWhiteboardCanvas as mount } from "./desktop-entry";
import { fixtureWhiteboardBridge, settled } from "./fixture-whiteboard-bridge";
...
const fixtureProgress: WhiteboardProgress = {
  files: [],
  lenses: [],
...
> = {
  ...
  sequence: (c) =>
    ...
    has(c, "[data-whiteboard-anchor-id='step-1']") &&
    has(c, "[data-whiteboard-anchor-id='step-2']") &&
  ...
  image: (c) =>
    has(c, "figure.whiteboard-image img[src^='blob:']") &&
    text(c).includes("An image"),
  trace_quote: (c) =>
    has(c, ".whiteboard-trace-quote") && text(c).includes("queue the order"),
...
async function mountFixture(
  ...
  const bridge = fixtureWhiteboardBridge({
    snapshot,
...
describe("block components", () => {
  it("keeps shipped tutorial keybindings and view buttons interactive in the JSON canvas", async () => {
    const selectKeymap = vi.fn<WhiteboardCanvasTutorialBridge["selectKeymap"]>(
      async () => {},
    );
...
describe("tutorial guide placement", () => {
  it("keeps the guide inside the host when status rows sit above the document", async () => {
    ...
    const contentsElement = container.querySelector(".whiteboard-toc")!;
    const appElement = container.querySelector(".whiteboard-app")!;
    ...
    container.querySelector("[data-whiteboard-api]")!.prepend(statusRow);

    const host = container.getBoundingClientRect();

    const app = container
      .querySelector(".whiteboard-app")!
      .getBoundingClientRect();

    const status = statusRow.getBoundingClientRect();

    const contents = container
      .querySelector(".whiteboard-toc")!
      .getBoundingClientRect();
