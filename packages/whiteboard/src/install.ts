...
async function runInstallUnlocked(input: RunInstallInput): Promise<number> {
    ...
    for (const cwd of await listTraceRepositoryRoots(homeDir)) {
      ...
      if (status.enabled)
        await enableTraceRepository({
          cwd,
          scope,
          whiteboardCommand: input.whiteboardCommand,
        });
    }
  }
  ...
  for (const target of input.targets) {
    ...
    if (target === "claude") {
      await installClaudeTraceHook(homeDir, input.whiteboardCommand);
    } else if (target === "codex") {
      await installCodexTraceHook(homeDir, input.whiteboardCommand);
    } else if (target === "opencode") {
      await installOpenCodeTraceExtension(homeDir, input.whiteboardCommand);
    } else if (target === "pi") {
      await installPiTraceExtension(homeDir, input.whiteboardCommand);
    }
  }
