 ...
 */
export class WhiteboardUpdateDialog extends Disposable {
  ...
  private show(type: "info" | "error", message: string, detail?: string): void {
    this.logService.info(`whiteboardUpdateDialog#show - ${type}: ${message}`);
    this.dialogMainService
      .showMessageBox({ type, message, detail })
      .catch((error) => {
        this.logService.error("whiteboardUpdateDialog#show failed", error);
      });
  }
}
