...
describe("DiffLayoutControl", () => {
  ...
  let root: Root;
  let layout: WhiteboardDiffLayout;
  let listeners: Set<(layout: WhiteboardDiffLayout) => void>;

  let setDiffLayout: ReturnType<
    typeof vi.fn<(next: WhiteboardDiffLayout) => Promise<void>>
  >;
  ...
  beforeEach(async () => {
    ...
    setDiffLayout = vi.fn<(next: WhiteboardDiffLayout) => Promise<void>>(
      async (next) => {
        confirm(next);
      },
    );
  ...
  function confirm(next: WhiteboardDiffLayout) {
    layout = next;
