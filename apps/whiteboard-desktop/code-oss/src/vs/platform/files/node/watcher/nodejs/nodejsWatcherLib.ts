...
const APFS_FILE_SYSTEM_TYPE = 0x1A;

export function isApfsFileSystemType(type: number | bigint): boolean {
	return type === APFS_FILE_SYSTEM_TYPE || type === BigInt(APFS_FILE_SYSTEM_TYPE);
}
...
export class NodeJSFileWatcherLibrary extends Disposable {
	...
	private async doWatchWithNodeJS(isDirectory: boolean, disposables: DisposableStore): Promise<void> {
		...
		// macOS: watching samba shares can crash VSCode so we guard paths under
		// /Volumes, where network shares are conventionally mounted. Local APFS
		// volumes are also mounted there and are safe to watch, so positively
		// identify APFS before applying the conservative fallback.
		...
		let isLocalApfsVolume = false;
		if (isMacintosh && isEqualOrParent(realPath, '/Volumes/', true)) {
			try {
				isLocalApfsVolume = isApfsFileSystemType((await promises.statfs(realPath)).type);
			} catch {
				// Preserve the conservative refusal when the filesystem cannot
				// be identified.
			}
		}
		if (isMacintosh && isEqualOrParent(realPath, '/Volumes/', true) && !isLocalApfsVolume) {
