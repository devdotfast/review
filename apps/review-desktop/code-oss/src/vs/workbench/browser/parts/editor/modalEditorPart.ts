...
import { EditorResourceAccessor, IEditorCommandsContext, isEditorPaneWithSelection, SideBySideEditor, Verbosity } from '../../../common/editor.js';
...
export class ModalEditorPart {
	...
	async create(options?: IModalEditorPartOptions): Promise<ICreateModalEditorPartResult> {
		...
		disposables.add(Event.runAndSubscribe(modalEditorService.onDidActiveEditorChange, () => {
			const activeEditorPane = editorPart.activeGroup.activeEditorPane;
			if (activeEditorPane?.input) {
				this.historyService.completeActiveEditorNavigation(
					activeEditorPane.group.id,
					activeEditorPane.input,
					isEditorPaneWithSelection(activeEditorPane) ? activeEditorPane.getSelection() : undefined,
				);
			}
			const activeEditor = editorPart.activeGroup.activeEditor;
