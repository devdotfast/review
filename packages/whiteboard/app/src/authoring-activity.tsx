 ...
 */
export function WhiteboardSurfaceLabel({
  hasContent,
  active,
  label = "Whiteboard",
}: {
  ...
  const activity = useContext(AuthoringActivityContext);
  const version = useContext(DisplayedWhiteboardVersionContext) ?? null;
  ...
  return (
    <span className="whiteboard-segment-word" data-working={live || undefined}>
      {label}
      {unread && (
        <span className="whiteboard-segment-unread" aria-hidden="true" />
      )}
    </span>
  );
}
