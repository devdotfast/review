...
import { jsonString, parseJsonText } from "@dev.fast/whiteboard-protocol";

import { isMissingFileError } from "./fs-utils";

const WHITEBOARD_FILE_CONTENT_LIMIT_BYTES = 5 * 1024 * 1024;

export interface WhiteboardDiffFile {
  path: string;
  previousPath?: string;
...
export interface WhiteboardDiffFilesResult {
  baseRef?: string;
  headRef?: string;
  files: WhiteboardDiffFile[];
}

export type WhiteboardFileContentResult =
  | { content: string; truncated?: boolean }
  | { absent: true }
  | { binary: true };

export async function resolveWhiteboardDiffFiles(input: {
  rootPath: string;
  baseRef?: string;
  headRef?: string;
  contextLines?: number;
  ...
  paths?: string[];
}): Promise<WhiteboardDiffFilesResult> {
  ...
  return {
    ...
    files: splitGitDiffSections(stdout)
      .map(parseWhiteboardDiffFile)
      .filter((file): file is WhiteboardDiffFile => file !== null)
      .filter((file) => matchesDiffPath(file, paths)),
  };
}

export async function resolveWhiteboardFileContent(input: {
  rootPath: string;
  baseRef?: string;
  headRef?: string;
  path: string;
  side: "base" | "head";
  maxBytes?: number;
  comparison?: WhiteboardDiffFilesResult;
}): Promise<WhiteboardFileContentResult> {
  ...
  const comparison =
    input.comparison ??
    (await resolveWhiteboardDiffFiles({
      rootPath: input.rootPath,
  ...
  const limit = input.maxBytes ?? WHITEBOARD_FILE_CONTENT_LIMIT_BYTES;
...
function matchesDiffPath(file: WhiteboardDiffFile, paths: string[]): boolean {
...
function parseWhiteboardDiffFile(section: string): WhiteboardDiffFile | null {
