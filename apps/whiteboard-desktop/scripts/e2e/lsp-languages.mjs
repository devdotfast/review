...
async function reopenWhiteboard(ctx, review) {
  ...
  return page.locator(".whiteboard-canvas-root [data-whiteboard-api]");
}
...
export async function runLspJourney(ctx, id) {
  ...
  if (
    ...
    process.env.WHITEBOARD_E2E_NETWORK !== "1"
  )
    throw new Error(`skip: ${id} needs WHITEBOARD_E2E_NETWORK=1`);
  ...
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      canvas = await reopenWhiteboard(ctx, review);
    }
  }
}
...
async function hoverAndJump(ctx, id, language, canvas, lines, callLine) {
  ...
  const editor = canvas
    .locator(
      `.whiteboard-inline-editor[data-whiteboard-inline-editor="${language.peekFile}"]`,
    )
    .first();
