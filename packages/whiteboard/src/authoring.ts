...
import { isObjectValue, jsonValueSchema } from "@dev.fast/whiteboard-protocol";
...
export interface WhiteboardDefinitionEnvironment {
...
export interface WhiteboardDefinitionDiagnostic {
...
export const whiteboardCodePeekPropsSchema = z.strictObject({
  anchor: peekableAnchorRefSchema,
  children: noChildrenSchema,
});

export type WhiteboardCodePeekProps = z.infer<
  typeof whiteboardCodePeekPropsSchema
>;
...
export const whiteboardSectionPropsSchema = z.strictObject({
  title: nonEmptyStringSchema,
  defaultCollapsed: z.boolean().optional(),
  children: reactNodeSchema,
});

export type WhiteboardSectionProps = z.infer<
  typeof whiteboardSectionPropsSchema
>;
...
export interface WhiteboardAuthoringComponentRegistry {
  AnchorLink: ComponentType<AnchorLinkProps>;
  CallStackDiff: ComponentType<CallStackDiffProps>;
  CodePeek: ComponentType<WhiteboardCodePeekProps>;
  ...
  WhiteboardSection: ComponentType<WhiteboardSectionProps>;
...
} satisfies Record<keyof WhiteboardAuthoringComponentRegistry, z.ZodType>;

export type WhiteboardAuthoringComponentName =
  keyof typeof whiteboardAuthoringPropsSchemas;
...
// peek is already a plain source range. whiteboard-document-materialize.ts writes
// exactly this.
export const whiteboardComponentDataSchemas = {
  ...
  WhiteboardSection: z.strictObject({
    title: nonEmptyStringSchema,
    defaultCollapsed: z.boolean().optional(),
  }),
...
export type WhiteboardDocumentComponentName =
  keyof typeof whiteboardComponentDataSchemas;
...
export interface WhiteboardDefinitionSession {
  readonly diagnostics: readonly WhiteboardDefinitionDiagnostic[];
  begin(): void;
  ready(): Promise<void>;
...
export function createWhiteboardDefinitionSession(
  environment: WhiteboardDefinitionEnvironment,
): WhiteboardDefinitionSession {
  let pending: Promise<void>[] = [];
  const diagnostics: WhiteboardDefinitionDiagnostic[] = [];
  ...
  const reportMissingSoftwareMap = (
    ...
    } satisfies WhiteboardDefinitionDiagnostic);
