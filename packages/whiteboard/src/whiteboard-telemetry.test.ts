...
import {
  WHITEBOARD_APP_VERSION_ENV,
  WhiteboardTelemetry,
  type WhiteboardTelemetryCaptureClient,
  type WhiteboardTelemetryOptions,
} from "./whiteboard-telemetry";

describe("WhiteboardTelemetry", () => {
  ...
  it("adds package version and internal status to common properties", async () => {
    ...
    expect(events[0].properties).toMatchObject({
      ...
      version: await whiteboardPackageVersion(),
      internal: true,
    });
  });
  ...
  it("marks installation, command, server, and UI events from a stored marker", async () => {
    ...
    await telemetry.captureWhiteboardReaped({ retentionDays: 30 });
  ...
  it("adds a valid Desktop version without changing the package version", async () => {
    const { events, rootPath, telemetry } = createTelemetry({
      env: { [WHITEBOARD_APP_VERSION_ENV]: "0.0.16" },
    });
  ...
  it.each([undefined, "not-a-version"])(
    ...
    async (appVersion) => {
      const { events, rootPath, telemetry } = createTelemetry({
        env: appVersion ? { [WHITEBOARD_APP_VERSION_ENV]: appVersion } : {},
      });
  ...
  it("does not write config or send events when DO_NOT_TRACK is set", async () => {
    ...
    await telemetry.captureWhiteboardPresented({
  ...
  it("derives installation-scoped opaque review and presentation ids", async () => {
    ...
    const otherWhiteboardUuid = "9d64ac3b-4de8-432c-b715-e338492553b9";
...
function createTelemetry(input?: {
  ...
  const captureClient: WhiteboardTelemetryCaptureClient = {
    enabled: true,
    capture: async (event) => {
      events.push(event);
    },
  ...
  const options: WhiteboardTelemetryOptions = {
    captureClient,
    env: input?.env ?? {},
  ...
  const telemetry = new WhiteboardTelemetry(options);

  return { configPath, events, legacyConfigPath, rootPath, telemetry };
}

function storedConfig(
  input: Partial<WhiteboardTelemetryInstallConfig> = {},
...
async function writeStoredConfig(
  configPath: string,
  config: Partial<WhiteboardTelemetryInstallConfig> | { installId: string },
): Promise<void> {
...
async function whiteboardPackageVersion(): Promise<string> {
