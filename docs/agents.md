...
Whiteboard works with Claude Code, Codex, and other coding agents. The desktop app
installs a small set of skills that teaches the agent how to create, author,
validate, and update a session through the Whiteboard API and MCP tools.

## Built-in setup

Whiteboard provides setup shortcuts for these agent-specific skill
locations:
...
same Whiteboard skills from `~/.agents/skills`.

Whiteboard is the recommended installation path. On first launch it detects
installed agents, asks which integrations to enable, and keeps their skills in
sync with app updates. Generated skills carry the Whiteboard release version in
...
or missing supporting files; terminal-only installs are not automatically enrolled. You can manage the integrations later from Whiteboard
settings.

For Codex and Claude Code, Desktop setup also registers a user-level `whiteboard`
MCP connection. It launches a small adapter using Whiteboard's bundled runtime; no
separate Node installation, agent CLI, port, or token configuration is needed.
The desktop server remains the owner of every session. Other agents can use the
installed `whiteboard api` command.

App updates refresh the adapter along with enabled integrations and repair missing
MCP entries. Reinstall in settings runs the same setup again. Whiteboard leaves
...
terminal-only `whiteboard install` command still installs skills and the CLI only.

## Installed skills

- `whiteboard` authors change and architecture reviews, including software maps.

The authoring skill coordinates the whole workflow. In normal use, ask your
agent for a session instead of running the lower-level CLI commands yourself.
...
```text
Use $whiteboard to review my current branch against up to date main, then open
it in Whiteboard.
```
...
In Cursor, choose `whiteboard` from the `/` menu and give it the same request.
...
```text
Use the Whiteboard skill to explain the main data flows, storage boundaries, and
...
With no target, `whiteboard install` installs every supported integration. Run
`whiteboard install --help` for the current target list.
