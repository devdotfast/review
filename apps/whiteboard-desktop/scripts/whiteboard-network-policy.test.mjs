...
import {
  ...
  blockedWhiteboardRequestReason,
} from "./whiteboard-network-policy.mjs";
...
test("blocks disabled Microsoft service and asset hosts", () => {
  for (const url of [
    ...
    assert.ok(blockedWhiteboardRequestReason(url), url);
  }
});
...
test("blocks Copilot API paths without blocking ordinary GitHub API use", () => {
  ...
  assert.ok(
    blockedWhiteboardRequestReason("https://api.github.com/copilot/mcp_registry"),
  );
  assert.equal(
    blockedWhiteboardRequestReason(
...
test("allows local Review resources and non-service protocols", () => {
  for (const url of [
    ...
    assert.equal(blockedWhiteboardRequestReason(url), undefined, url);
  }
