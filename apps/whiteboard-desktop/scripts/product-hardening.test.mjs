...
test("updates only from the sanctioned dev.fast feed", () => {
  ...
  assert.equal(
    product.quality,
    process.env.WHITEBOARD_EXPECTED_QUALITY ?? "stable",
  );
});
...
// `whiteboardConfigurationDefaults.ts` is the single channel.
...
test("configures Zod's CSP-safe mode before the canvas module evaluates", () => {
  const candidateConfig = whiteboardCanvasPart.indexOf(
    "canvasGlobal.__zod_globalConfig ??= {};",
  );

  const candidateModule = whiteboardCanvasPart.indexOf(
    "vs/whiteboard/canvas/canvas-loader.js",
  );
