...
const { ChangedFilesTreeRenderer } = await import('./whiteboardChangedFilesTree.js');

const file = (path: string): WhiteboardDiffFileWire => ({ path, status: 'modified', additions: 3, deletions: 1 } as WhiteboardDiffFileWire);
const progress = (path: string, state: WhiteboardDiffProgressFile['state'], remaining: number): WhiteboardDiffProgressFile => ({
	path, state, remaining: { additions: remaining, deletions: 0 }, total: { additions: 3, deletions: 1 }, viewedRanges: [], changedRanges: [],
});

function row(state: WhiteboardDiffProgressFile['state'] | undefined, counts?: StructuralLineCounts) {
	const path = 'src/a.ts';
...
test('a folded file reads as done: greyed like a viewed one, "Folded" in place of its counts', () => {
	...
	assert.ok(folded.row.classList.contains('whiteboard-file-folded'));
	assert.ok(!folded.row.classList.contains('whiteboard-file-viewed'));
});

test('a viewed file says "Viewed" in place of its counts', () => {
	const viewed = row('viewed');
	assert.equal(viewed.counts, 'Viewed');
	assert.ok(viewed.row.classList.contains('whiteboard-file-viewed'));
});
...
test('a file with changes left to read shows its remaining counts, not greyed', () => {
	...
	assert.ok(!unread.row.classList.contains('whiteboard-file-folded'));
	assert.ok(!unread.row.classList.contains('whiteboard-file-viewed'));
});
...
test('without progress, the structural counts show and nothing reads as folded', () => {
	...
	assert.ok(!pending.row.classList.contains('whiteboard-file-folded'));
});
