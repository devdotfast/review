...
import { type Block, elements } from "../session-api/document";
import type {
 ...
 * upgraded (`upgradeWhiteboardDocumentJson`), so peeks carry a source range, call
...
export function legacyDocumentToBlocks(
  ...
  const traceQuote = (
    node: Extract<WhiteboardNode, { type: "component"; name: "TraceQuote" }>,
  ...
  const hoistable = (node: WhiteboardNode): Block | undefined => {
  ...
  const withoutHoisted = (
    node: WhiteboardNode,
    hoisted: Block[],
  ): WhiteboardNode => {
    if (node.type === "text") return node;

    const children = node.children.flatMap((child): WhiteboardNode[] => {
  ...
  const convert = (nodes: WhiteboardNode[]): Block[] => {
    const out: Block[] = [];
    let prose: WhiteboardNode[] = [];
...
function diagramBlock(node: WhiteboardComponentNode): Block | undefined {
  switch (node.name) {
    case "CallStackDiff":
      return {
...
function isStoredImage(node: WhiteboardElementNode): boolean {
  return (
    node.tag === "img" &&
...
function sectionBody(
  node: Extract<WhiteboardNode, { name: "WhiteboardSection" }>,
): WhiteboardNode[] {
...
export function repairImportedSectionHeadings(
  current: Block[],
  sealed: WhiteboardDocumentData,
): Block[] | undefined {
  const headings = new Map<string, Set<string>>();
  const footnotes = collectFootnoteDefinitions(sealed.body);

  const visit = (nodes: WhiteboardNode[]) => {
    for (const node of nodes) {
      if (node.type === "text") continue;

      if (node.type === "component" && node.name === "WhiteboardSection") {
        const first = node.children[0];
