import { sourceLocation, sourceTreeUri, sourceTreeRoot } from "../common/whiteboardSourceView.js";
import assert from "node:assert/strict";
import test from "node:test";

import { URI } from "../../base/common/uri.js";
import type { ITextModelContentProvider } from "../../editor/common/services/resolverService.js";
import { apiSourceUri, WhiteboardApiSourceService } from "./whiteboardApiSourceService.js";
import type { WhiteboardDiffViewSource } from "./whiteboardDiffViewService.js";
import { resolveWhiteboardSourceView, whiteboardSourceAnchor, whiteboardSourceComparison } from "../common/whiteboardProtocol.js";
import type { WhiteboardDiffLens } from "../common/whiteboardProtocol.js";

const view = (version: number) => resolveWhiteboardSourceView({ sessionId: "review-a", version, pins: {} });
...
function setup() {
	...
	const service = new WhiteboardApiSourceService(
		{
		...
		{
			registerWhiteboardEditor(sessionId: string) {
...
test("a document diff retains its pinned comparison even if the review advances before loading", async (t) => {
	...
	let version = 3;
	let source!: WhiteboardDiffViewSource;
	let lens!: WhiteboardDiffLens;
	const canvas = service.canvas(() => view(version), {} as never, {
		openComparison: () => undefined,
		createDocument: (_: unknown, scope: WhiteboardDiffLens, input: WhiteboardDiffViewSource) => { lens = scope; source = input; return {}; },
	} as never);
...
test("diff entries keep rename paths and missing sides, even when the review advances during the read", async (t) => {
	...
	const canvas = service.canvas(
		() => resolveWhiteboardSourceView({ sessionId: "review-a", version, pins: { worktreeRevision: generation } }),
		{} as never,
		{
			openComparison: () => undefined,
			create: (_: unknown, input: WhiteboardDiffViewSource) => {
				source = input;
				return {};
			},
		} as never,
...
test("tree entries retain version, side and selected commit when opening a child", async (t) => {
	...
	const root = apiSourceUri({ view: whiteboardSourceComparison(view(3), "chosen-commit"), side: "base", file: "src" });
...
test("a source at its own pins keeps them through its URI and reads them back from the server", async t => {
	...
	const anchored = whiteboardSourceAnchor(whiteboardSourceComparison(view(3), "c".repeat(40)), pins);
