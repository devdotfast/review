...
describe("submitWhiteboardBugReport", () => {
  let tempDir: string;
  ...
  it("sends ordered generic parts for a complete Codex lineage", async () => {
    ...
    await submitWhiteboardBugReport({
  ...
  it("does not replace a successful submit when trace cleanup fails", async () => {
    writeWhiteboard("claude-code:cleanup-test");
  ...
  function writeWhiteboard(value: string): void {
    sourceSession = value;
  }
...
function report(
  overrides: Partial<WhiteboardBugReportRequest> = {},
