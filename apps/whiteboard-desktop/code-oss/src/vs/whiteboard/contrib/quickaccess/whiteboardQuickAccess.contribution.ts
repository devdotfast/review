...
import { whiteboardCommandPaletteLabel } from '../../common/whiteboardCommandPalette.js';
...
export class WhiteboardCommandsQuickAccessProvider extends AbstractCommandsQuickAccessProvider {
	constructor(
	...
	protected override async getCommandPicks(token: CancellationToken): Promise<ICommandQuickPick[]> {
		...
		return [...CommandsRegistry.getCommands().keys()]
			...
			.map(commandId => ({
				commandId,
				label: whiteboardCommandPaletteLabel(commandId, MenuRegistry.getCommand(commandId)),
			}));
	}
...
Registry.as<IQuickAccessRegistry>(Extensions.Quickaccess).registerQuickAccessProvider({
	ctor: WhiteboardCommandsQuickAccessProvider,
	prefix: WhiteboardCommandsQuickAccessProvider.PREFIX,
	contextKey: 'inCommandsPicker',
	placeholder: localize('whiteboardCommandsQuickAccessPlaceholder', 'Type the name of an editor or Whiteboard command to run.'),
	helpEntries: [{
		description: localize('whiteboardCommandsQuickAccess', 'Show and run Whiteboard commands'),
