...
type ElementNode = Extract<WhiteboardNode, { type: "element" }>;

/** The link form `sourceReferences` in session-api/document.ts accepts. */
...
export function isProseNode(node: WhiteboardNode): boolean {
  return (
    node.type === "text" ||
    node.type === "element" ||
...
function alignRow(align: WhiteboardElementProps[string] | undefined): string {
  switch (align) {
    case "left":
      return ":--";
    case "center":
...
export function isFootnoteSection(node: WhiteboardNode): boolean {
...
function stripBackrefs(nodes: WhiteboardNode[]): WhiteboardNode[] {
  return nodes.flatMap((node): WhiteboardNode[] => {
...
function inlines(nodes: WhiteboardNode[], state?: FootnoteState): string {
  return nodes.map((node) => inline(node, state)).join("");
}

function inline(node: WhiteboardNode, state?: FootnoteState): string {
...
function inlineComponent(
  ...
  // SAFETY: whiteboardDocumentDataSchema validated the component props against
  // whiteboardComponentDataSchemas when the sealed document was parsed.
  const anchor = (
...
function plainText(nodes: WhiteboardNode[]): string {
  return nodes
    .map((node) =>
