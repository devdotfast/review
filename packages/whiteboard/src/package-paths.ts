...
export function findWhiteboardPackageRoot(
  moduleUrl: string = import.meta.url,
): string {
  return findPackageRoot(moduleUrl);
}

export function readWhiteboardPackageVersion(
  moduleUrl: string = import.meta.url,
): string {
  try {
    const packageJson = jsonObject(
      parseJsonText(
        readFileSync(
          path.join(findWhiteboardPackageRoot(moduleUrl), "package.json"),
          "utf8",
        ),
      ),
    );
...
export function whiteboardModelModulePath(
