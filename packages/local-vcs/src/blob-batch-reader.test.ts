...
/** A body that looks like a `cat-file --batch` header, plus a trailing newline. */
const headerLike = "deadbeef blob 3\nabc\n";

/** Bigger than one stdout chunk, so the parser has to join reads. */
...
/** Every git spawn recorded since `recordSpawns()`, as file plus args. */
const spawns: string[][] = [];
...
/** Readers opened by a test, closed after it whether it passed or not. */
...
describe("blob batch reader", () => {
  it("reads committed text, an empty file and a binary blob", async () => {
    ...
    // An empty blob is empty content, not an absent file.
  ...
  it("frames a body by its length, not by the next newline", async () => {
    ...
    // The header-shaped body must not have been read as a header.
    await expect(reader.read(commit, "src/app.ts")).resolves.toEqual(
      Buffer.from("export const app = 1;\n"),
    );
  });
  ...
  it("returns null for a path it cannot frame without writing it", async () => {
    ...
    // Nothing is written, so no process is even needed.
    ...
    await expect(reader.read(commit, "src/a\nb.ts")).resolves.toBeNull();
    // The stream is still in step after the rejected request.
    await expect(reader.read(commit, "src/app.ts")).resolves.toEqual(
      Buffer.from("export const app = 1;\n"),
    );
  });
  ...
  it("kills an idle process and respawns on the next read", async () => {
    ...
    await reader.read(commit, "src/app.ts");
    // Resolves when the idle timer has killed the process this read started.
  ...
  it("lets a host that forgot to close exit without waiting out the idle timer", async () => {
    ...
    // The read still answered, so a pending read does hold the loop open.
    await expect(
  ...
  it("keeps a host alive until an awaited close has finished", async () => {
    ...
    // The marker is written after `await close()`, so it proves the wait held.
    await expect(
  ...
  it("leaves no process behind when closed while idle", async () => {
    ...
    expect(reader.pid).toBeUndefined();
    // Closing twice is not an error, and does not start a process.
    await reader.close();
    expect(reader.pid).toBeUndefined();
  });
...
/**
 * Run a fixture that drives a reader in its own process, so what holds the
 * event loop open decides when it exits. Resolves on the child's `close`,
 * which is where its stdout is drained, and never waits out the fixture's
 * 30 s idle timeout.
 */
