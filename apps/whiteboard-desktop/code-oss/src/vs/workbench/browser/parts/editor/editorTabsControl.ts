...
export abstract class EditorTabsControl extends Themable implements IEditorTabsControl {
	...
	private static readonly EDITOR_TAB_HEIGHT = {
		...
		// Review pill tabs (review/browser/media/whiteboard.css) add 6px top + 6px
		// bottom padding to the tabs-and-actions-container. The total must equal
		// WHITEBOARD_CHROME_HEIGHT, because the Review titlebar overlays this row.
		review: 40 as const,        // 28px pill + 6px top + 6px bottom padding
		reviewCompact: 32 as const, // 20px pill + 6px top + 6px bottom padding
	};
	...
	protected updateEditorActionsToolbar(): void {
		...
		// Review has no editor management surface, so it drops the secondary
		// actions and with them the toolbar's "..." overflow button. Primary
		// actions (e.g. the diff layout toggle) still render.
		const visibleSecondary = IsWhiteboardWindowContext.getValue(this.contextKeyService) ? [] : secondary;

		editorActionsToolbar.setActions(prepareActions(primary), prepareActions(visibleSecondary));
	...
	protected get tabHeight() {
		...
		// The Review workbench draws its own pill tabs with the same shape.
		if (this.parent.classList.contains('tabs') && this.parent.closest('.whiteboard-workbench')) {
			return isCompact ? EditorTabsControl.EDITOR_TAB_HEIGHT.reviewCompact : EditorTabsControl.EDITOR_TAB_HEIGHT.review;
		}
