...
} from "../common/whiteboardErrorReport.js";
import { whiteboardTelemetryEventRequest } from "../common/whiteboardTelemetryRequest.js";
import { drainWhiteboardBootstrapBreadcrumbs } from "../node/whiteboardBootstrapBreadcrumb.js";

export interface WhiteboardMainErrorTelemetryOptions {
  /** Resolves with the loopback endpoint and token of the embedded server. */
  readonly whenConnected: () => Promise<WhiteboardDesktopConnection>;
...
interface PendingWhiteboardTelemetryEvent {
 ...
 */
export class WhiteboardMainErrorTelemetry {
  readonly appSessionId = generateUuid();

  private readonly limiter = new WhiteboardErrorReportLimiter();
  private readonly queued: PendingWhiteboardTelemetryEvent[] = [];
  private readonly unbind: () => void;
  private connection: WhiteboardDesktopConnection | undefined;
  private disposed = false;

  constructor(private readonly options: WhiteboardMainErrorTelemetryOptions) {
  ...
  send(errorSource: string, report: WhiteboardErrorReport): void {
    this.capture(
      "client_error",
      {
  ...
  private queueBootstrapBreadcrumbs(): void {
    ...
    try {
      for (const breadcrumb of drainWhiteboardBootstrapBreadcrumbs(
        userDataPath,
  ...
  private post(pending: PendingWhiteboardTelemetryEvent): void {
