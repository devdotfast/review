...
describe("runInlineC4Layout stability", () => {
  ...
  it("preserves visual rows after expanding and collapsing a middle node", async () => {
    const collapsedNodes: SoftwareMapNodeSnapshot[] = [
      ...
      { id: "reviewer", label: "Whiteboarder", type: "person" },
  ...
  it("does not flatten an expanded parent's sibling layout when expanding a child", async () => {
    ...
    const nodes: SoftwareMapNodeSnapshot[] = [
      ...
      ...childIds.map((id) => ({
        ...
        parentId: "system",
        expanded: id === "progressiveWhiteboard",
        expandable: id === "progressiveWhiteboard",
      })),
      ...Array.from({ length: 23 }, (_, index) => ({
        id: `progressiveWhiteboard.c${index}`,
        label: `Component ${index}`,
        type: "component" as const,
    ...
    const relationships: SoftwareMapRelationshipSnapshot[] = Array.from(
      ...
      (_, index) => ({
        ...
        from: `progressiveWhiteboard.c${index % 23}`,
        to: `progressiveWhiteboard.c${(index * 5 + 2) % 23}`,
        label: `edge ${index}`,
      }),
    );
    ...
    const collapsedNodes = nodes
      .values()
      .filter((node) => !node.id.startsWith("progressiveWhiteboard.c"))
      .map((node) =>
        node.id === "progressiveWhiteboard"
          ? { ...node, expanded: false }
          : node,
      )
      .toArray();
  ...
  it("deflates the outer layout after nested expansion is collapsed", async () => {
    ...
    const progressiveChildren: SoftwareMapNodeSnapshot[] = Array.from(
      { length: 23 },
      (_, index) => ({
        id: `devFast.progressiveWhiteboard.c${index}`,
        label: `Component ${index}`,
        type: "component",
        parentId: "devFast.progressiveWhiteboard",
      }),
    );
    ...
    const expandedProgressiveWhiteboard = await runInlineC4Layout(
      [
        ...topLevelNodes.map((node) =>
          node.id === "devFast" ? { ...node, expanded: true } : node,
        ),
        ...devFastChildren.map((node) =>
          node.id === "devFast.progressiveWhiteboard"
            ? { ...node, expanded: true }
            : node,
        ),
        ...progressiveChildren,
    ...
    const collapsedProgressiveWhiteboard = await runInlineC4Layout(
      [
        ...topLevelNodes.map((node) =>
          node.id === "devFast" ? { ...node, expanded: true } : node,
        ),
      ...
      [],
      undefined,
      expandedProgressiveWhiteboard.inlineLayout,
    );
    ...
    const collapsedDevFast = await runInlineC4Layout(
      ...
      [],
      undefined,
      collapsedProgressiveWhiteboard.inlineLayout,
    );
