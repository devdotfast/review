...
export const FFF_TARGETS: WhiteboardFffInstallTarget[] = [
  "claude",
  "codex",
  "pi",
];
...
export function fffCorpusRoot(
  ...
  return path.join(devWhiteboardHome(env, homeDir), "trace-search");
}

export function isFffTarget(
  target: string,
): target is WhiteboardFffInstallTarget {
  return target === "claude" || target === "codex" || target === "pi";
}

export async function installFffForTargets(input: {
  targets: WhiteboardFffInstallTarget[];
  homeDir: string;
  env: NodeJS.ProcessEnv;
  write: (text: string) => void;
}): Promise<{ ok: boolean; created: WhiteboardFffManagedRegistration[] }> {
  const binaryPath = fffBinaryPath(input.homeDir);
  const corpusRoot = fffCorpusRoot(input.homeDir, input.env);
  await mkdir(corpusRoot, { recursive: true });

  const missingTargets: WhiteboardFffInstallTarget[] = [];
  ...
  const created: WhiteboardFffManagedRegistration[] = [];
...
export async function readFffRegistration(
  target: WhiteboardFffInstallTarget,
  homeDir: string,
  env: NodeJS.ProcessEnv,
...
export function fffRegistration(
  ...
  binaryPath: string,
  corpusRoot: string,
): WhiteboardFffManagedRegistration {
  if (target === "pi") {
    return {
      target,
      command: "pi",
...
export function fffRegistrationMatches(
  output: string,
  registration: WhiteboardFffManagedRegistration,
): boolean {
...
function fffTargetLabel(target: WhiteboardFffInstallTarget): string {
  return target === "pi"
