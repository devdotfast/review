...
import { sourceReferences } from "../session-api/document";
import {
  type WhiteboardNode,
...
function peekableAnchorLinks(nodes: WhiteboardNode[]): number {
  let count = 0;
...
function proseRuns(nodes: WhiteboardNode[]): WhiteboardNode[][] {
  const runs: WhiteboardNode[][] = [];
  let current: WhiteboardNode[] = [];
