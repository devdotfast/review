...
import type { WhiteboardMainErrorTelemetry } from "./whiteboardMainErrorTelemetry.js";
...
export type WhiteboardUpdateFailurePhase = "check" | "download" | "install";
export type WhiteboardUpdateMessageSource =
  | "electron"
  | "request"
  | "shipit"
  | "fallback";

interface WhiteboardUpdateTelemetryOptions {
  readonly updateService: IUpdateService;
  readonly storageService: IApplicationStorageMainService;
  readonly telemetry: WhiteboardMainErrorTelemetry;
  readonly isTelemetryEnabled: () => boolean;
  readonly shipItLogPath?: string;
  readonly readInstallFailure?: (
    path: string,
    offset: number | undefined,
  ) => Promise<{ message: string; source: WhiteboardUpdateMessageSource }>;
 ...
 */
export class WhiteboardUpdateTelemetry extends Disposable {
  private lastState: State;

  constructor(private readonly options: WhiteboardUpdateTelemetryOptions) {
    super();
  ...
  private captureFailure(
    phase: WhiteboardUpdateFailurePhase,
    source: WhiteboardUpdateMessageSource,
    message: string,
...
function updateFailurePhase(
  previous: State,
): Exclude<WhiteboardUpdateFailurePhase, "install"> | undefined {
