...
PRECOMPILED="${WHITEBOARD_DESKTOP_PRECOMPILED:-0}"
if [[ "$PRECOMPILED" == "1" ]]; then
  ...
  for relative in "${DARWIN_PAYLOAD_REQUIRED_PATHS[@]}"; do
    ...
    if [[ ! -d "$required" ]]; then
      echo "WHITEBOARD_DESKTOP_PRECOMPILED=1 but $required does not exist" >&2
      exit 1
    fi
  done
...
node "$APP_DIR/scripts/stage-whiteboard-runtime.mjs" --verify --packaged-root "$PACKAGED_APP"
