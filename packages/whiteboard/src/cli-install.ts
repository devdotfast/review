...
export async function readCliInstallStamp(
  stampPath: string,
): Promise<WhiteboardCliInstallStamp | null> {
  let value: unknown;
  ...
  const parsed = WhiteboardCliInstallStampSchema.safeParse(value);
...
export async function installWhiteboardCommand(input: {
  cliPath: string;
  cliRuntimePath?: string;
  homeDir?: string;
  env?: NodeJS.ProcessEnv;
