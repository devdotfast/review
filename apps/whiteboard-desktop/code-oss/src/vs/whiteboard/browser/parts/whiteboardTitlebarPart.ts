...
import { WhiteboardLinuxTitlebar } from './whiteboardLinuxTitlebar.js';
 ...
 */
export class WhiteboardTitlebarPart extends Part implements ITitlebarPart {
	...
	get minimumHeight(): number {
		const wcoHeight = isWeb && isWCOEnabled()
			...
			: 0;
		return Math.max(WHITEBOARD_CHROME_HEIGHT, wcoHeight) / (this.preventZoom ? getZoomFactor(getWindow(this.element)) : 1);
	}
	...
	protected override createContentArea(parent: HTMLElement): HTMLElement {
		this.element = parent;
		this.rootContainer = append(parent, $('.titlebar-container.whiteboard-titlebar-container'));
		...
		if (!hasNativeTitlebar(this.configurationService, this.titleBarStyle)) {
			...
			if (isMacintosh && isNative && controlsLocation === 'left') {
				const spacer = append(this.leftContent, $('div.window-controls-container.whiteboard-traffic-light-inset'));
				const updateSpacer = () => {
		...
		if (isLinux && isNative) {
			this._register(this.instantiationService.createInstance(WhiteboardLinuxTitlebar, this.leftContent, this.windowControlsContainer, getWindow(parent)));
		}
		const navigationContainer = append(this.leftContent, $('div.whiteboard-titlebar-navigation'));
	 ...
	 * is what `--whiteboard-chrome-left-width` reports to the editor tab strip — so
	...
	private createFileTreeToggle(): void {
		const toggle = append(this.leftContent, $('div.whiteboard-titlebar-file-tree-toggle', {
			role: 'button',
			tabindex: '0',
		...
		append(toggle, renderIcon(Codicon.layoutSidebarLeft)).classList.add('whiteboard-file-tree-icon-open');
		append(toggle, renderIcon(Codicon.layoutSidebarLeftOff)).classList.add('whiteboard-file-tree-icon-closed');
...
export class MainWhiteboardTitlebarPart extends WhiteboardTitlebarPart {
	...
	override layout(width: number, height: number): void {
		...
		void this.nativeHostService.updateWindowControls({
			targetWindowId: getWindowId(mainWindow),
			height: Math.round(WHITEBOARD_CHROME_HEIGHT * getZoomFactor(mainWindow))
		});
	}
	...
	protected override createContentArea(parent: HTMLElement): HTMLElement {
		...
		// `publishWhiteboardChromeInset` for why.
		const updateChromeWidths = () => {
			...
			this.element.style.setProperty('--whiteboard-chrome-left-width', `${left}px`);
			this.element.style.setProperty('--whiteboard-chrome-right-width', `${right}px`);
...
export class AuxiliaryWhiteboardTitlebarPart extends WhiteboardTitlebarPart implements IAuxiliaryTitlebarPart {
	private static counter = 1;
	get height(): number { return this.minimumHeight; }

	constructor(
		readonly container: HTMLElement,
		private readonly mainTitlebar: WhiteboardTitlebarPart,
		@IConfigurationService configurationService: IConfigurationService,
		@IThemeService themeService: IThemeService,
		@IStorageService storageService: IStorageService,
		@IWorkbenchLayoutService layoutService: IWorkbenchLayoutService,
		...
		super(`workbench.parts.auxiliaryTitle.${AuxiliaryWhiteboardTitlebarPart.counter++}`, getWindow(container), configurationService, themeService, storageService, layoutService, hostService, instantiationService, commandService);
	}
...
export class WhiteboardTitleService extends MultiWindowParts<WhiteboardTitlebarPart> implements ITitleService {
	declare _serviceBrand: undefined;
	readonly mainPart: WhiteboardTitlebarPart;
	...
	constructor(
		...
		super('workbench.whiteboardTitleService', themeService, storageService);
		this.mainPart = this._register(this.instantiationService.createInstance(MainWhiteboardTitlebarPart));
	...
	createAuxiliaryTitlebarPart(container: HTMLElement, _editorGroupsContainer: IEditorGroupsContainer, instantiationService: IInstantiationService): IAuxiliaryTitlebarPart {
		...
		const part = instantiationService.createInstance(AuxiliaryWhiteboardTitlebarPart, partContainer, this.mainPart);
