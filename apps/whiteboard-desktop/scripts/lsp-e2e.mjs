...
const bundledDiffr = values.app
  ...
  : path.join(workspace, "packages/whiteboard/bin/diffr");

const structuralDiffAvailable =
  !!process.env.WHITEBOARD_DIFFR_BINARY ||
  ...
  const launchEnv = values.app
    ...
    : {
        ...
        DEV_FAST_WHITEBOARD_SERVER_ENTRY: path.join(
          workspace,
          "packages/whiteboard/dist/server/desktop-host.js",
        ),
...
async function createWhiteboard(fix, title, kind = "commits") {
  ...
  const whiteboard = await command({
    type: "create",
    title,
    ...(kind === "worktree"
  ...
  const sessionId = whiteboard.sessionId;
  await command({
    type: "edit",
    sessionId,
    edit: {
...
function uri(whiteboard, side = "head", file = "main.ts", commit) {
  const query = new URLSearchParams({ version: String(whiteboard.version), side });

  if (whiteboard.pins.worktreeRevision)
    query.set("generation", whiteboard.pins.worktreeRevision);
...
function environmentUri(whiteboard, file) {
  const resource = new URL(uri(whiteboard));
...
try {
  ...
  const headEnvironment = await readyEnvironment(whiteboard);
  const baseEnvironment = await readyEnvironment(whiteboard, "base");
  ...
  for (const side of ["base", "head"]) {
    await expectDefinition(
      uri(whiteboard, side),
      greetAt,
      uri(whiteboard, side, "library.ts"),
      2,
    );
    await expectHover(uri(whiteboard, side), greetAt, "string");
  }
  ...
  await expectDefinition(
    uri(whiteboard, "head", "main.py"),
    { line: 2, character: 9 },
    uri(whiteboard, "head", "library_py.py"),
    0,
  );
  await expectHover(
    uri(whiteboard, "head", "main.py"),
  ...
  for (const [feature, position, targetLine] of [
    ...
    await until(async () => {
      ...
      return locations(result.result).some(
        (item) =>
          locationUri(item.uri) === uri(whiteboard, "head", "library.ts") &&
  ...
  assert.ok(
    locations(refs.result).some(
      (item) => locationUri(item.uri) === uri(whiteboard),
    ),
  );
  await expectDefinition(
    uri(whiteboard),
  ...
  await expectDefinition(
    ...
    uri(whiteboard, "head", "library.ts"),
    2,
  );
  ...
  const concurrent = await createWhiteboard(
  ...
  await expectDefinition(
    uri(whiteboard, "base"),
    greetAt,
    uri(whiteboard, "base", "library.ts"),
    2,
  );
  await expectDefinition(
    uri(whiteboard),
    greetAt,
    environmentUri(whiteboard, path.join(headEnvironment.rootPath, "library.ts")),
    3,
  );
  ...
  for (const side of ["base", "head"]) {
    ...
    const result = await expectHover(uri(whiteboard, side), greetAt, "string");
  ...
  const renamed = await probe({
    uri: uri(whiteboard, "base", "old.ts"),
    line: 0,
    character: 25,
    feature: definition,
    open: true,
  ...
  assert.ok(
    locations(renamed.result).some(
      (item) => locationUri(item.uri) === uri(whiteboard, "base", "library.ts"),
    ),
  );
  await expectDefinition(
    uri(whiteboard, "head", "renamed.ts"),
    { line: 0, character: 25 },
    uri(whiteboard, "head", "library.ts"),
    2,
  );
  ...
  await expectDefinition(
    uri(whiteboard, "head", "main.ts", first.head),
    greetAt,
    uri(whiteboard, "head", "library.ts", first.head),
    2,
  );
  await record("selected-commit source retains its own pinned coordinates");

  const newerPins = await api("/pins", "POST", {
    repositoryId: whiteboard.pins.repositoryId,
    base: first.base,
    head: first.base,
  });
  ...
  const navigated = await until(async () => {
    ...
    return (
      locationUri(state.active?.uri) === uri(whiteboard, "head", "library.ts") &&
      state
    );
  ...
  if (structuralDiffAvailable) {
    ...
    await until(
      async () =>
        ...
        uri(restoredWhiteboard, "head", "library.ts"),
  ...
  await probe({ diff: { base: uri(whiteboard, "base"), head: uri(whiteboard) } });
  ...
  const exact = await createWhiteboard(precision, "Historical project semantics");
  ...
  const live = await createWhiteboard(
    liveFixture,
  ...
  const homeWhiteboard = await createWhiteboard(
    liveFixture,
    "Home live source",
    "worktree",
  );
  ...
  await page
    ...
    .click();
  await page
    .getByRole("tab", { name: new RegExp(`^Source — ${homeWhiteboard.title}$`) })
    .waitFor();
  await writeFile(
  ...
  await command({
    type: "rename",
    sessionId: homeWhiteboard.sessionId,
