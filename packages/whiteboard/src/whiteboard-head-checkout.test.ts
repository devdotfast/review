...
import { whiteboardManagedCheckoutDir } from "./whiteboard-checkout-paths";
import {
  ensureWhiteboardPinnedCheckout,
  removeWhiteboardPinnedCheckout,
} from "./whiteboard-head-checkout";

const TEST_WHITEBOARD_UUID = "00000000-0000-4000-8000-00000000dddd";

describe("ensureWhiteboardPinnedCheckout", () => {
  ...
  it("materializes a detached checkout of the pinned head", async () => {
    ...
    const checkoutPath = await ensureWhiteboardPinnedCheckout({
      rootPath: repo.rootPath,
      ref: repo.headCommit,
      sessionId: TEST_WHITEBOARD_UUID,
    });

    expect(checkoutPath).toBe(
      whiteboardManagedCheckoutDir(
        repo.commonDir,
        TEST_WHITEBOARD_UUID,
        "head",
        repo.headCommit,
      ),
    );
    expect(checkoutPath).toContain(
      path.join(repo.commonDir, "dev-fast", "reviews", TEST_WHITEBOARD_UUID),
    );
    expect(
  ...
  it("reuses the existing checkout on a second start", async () => {
    ...
    const first = await ensureWhiteboardPinnedCheckout({
      rootPath: repo.rootPath,
      ref: repo.headCommit,
      sessionId: TEST_WHITEBOARD_UUID,
    });
    ...
    const second = await ensureWhiteboardPinnedCheckout({
      rootPath: repo.rootPath,
      ref: repo.headCommit,
      sessionId: TEST_WHITEBOARD_UUID,
    });
...
describe("removeWhiteboardPinnedCheckout", () => {
  const cleanupPaths: string[] = [];
  ...
  it("removes only the review's own checkout, leaving concurrent ones", async () => {
    ...
    const other = await ensureWhiteboardPinnedCheckout({
      rootPath: repo.rootPath,
      ref: otherCommit,
      sessionId: TEST_WHITEBOARD_UUID,
    });
  ...
  it("sweeps prepare markers an older install left beside the checkout", async () => {
    ...
    const checkoutPath =
      (await ensureWhiteboardPinnedCheckout({
        rootPath: repo.rootPath,
        ref: repo.headCommit,
        sessionId: TEST_WHITEBOARD_UUID,
      })) ?? "";
