...
export class BulkEditService implements IBulkEditService {
	...
	async apply(editsIn: ResourceEdit[] | WorkspaceEdit, options?: IBulkEditOptions): Promise<IBulkEditResult> {
		...
		if (isWhiteboardReadonlySource(source) || edits.some(edit =>
			edit instanceof ResourceTextEdit && isWhiteboardReadonlySource(edit.resource) ||
			edit instanceof ResourceFileEdit && (isWhiteboardReadonlySource(edit.oldResource) || isWhiteboardReadonlySource(edit.newResource)))) {
