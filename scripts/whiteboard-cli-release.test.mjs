...
import {
  ...
  registryMetadata,
  resolveRelease,
} from "./whiteboard-cli-release.mjs";
...
function repository(t) {
  ...
  mkdirSync(path.join(directory, "packages/whiteboard"), { recursive: true });
  writeFileSync(
    path.join(directory, "packages/whiteboard/package.json"),
    '{"version":"0.0.1"}\n',
  );
  git("add", ".");
...
test("dispatch plans without mutations, reserves an annotated tag, and retries the same version", async (t) => {
  ...
  assert.equal(
    JSON.parse(readFileSync("packages/whiteboard/package.json", "utf8")).version,
    "0.0.1",
  );
  createTag(plan);
