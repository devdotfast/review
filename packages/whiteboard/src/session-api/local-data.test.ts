...
it("opens a created review in Desktop unless the author opts out", async () => {
  ...
  const shownWhiteboard = await shown.json();
  expect(shownWhiteboard).toMatchObject({
    opened: true,
    softwareMapEnabled: true,
  });
  expect(opened).toEqual([shownWhiteboard.sessionId]);
  ...
  const quietWhiteboard = await quiet.json();
  expect(quietWhiteboard).toMatchObject({ opened: false });
  expect(opened).toEqual([shownWhiteboard.sessionId]);
  ...
  expect(await retry.json()).toMatchObject({
    sessionId: quietWhiteboard.sessionId,
    opened: true,
  });
  expect(opened).toEqual([
    shownWhiteboard.sessionId,
    quietWhiteboard.sessionId,
  ]);
});
