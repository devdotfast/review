...
- Keep: `publish-stage.ts`, `whiteboard-vcs.ts`, `whiteboard-bundle.ts`, `whiteboard-sealed-document.ts`, `whiteboard-publish-evaluate.ts`, `whiteboard-head-checkout.ts` (`ensureWhiteboardPinnedCheckout`, `ensureReviewCheckouts` open legacy reviews), `whiteboard-mutation-lock.ts`, `registerSerialized`, `withWhiteboardLock`, `reapDismissedReviews`, `tutorial-service.ts`, `stored-review-migration.ts`, `isomorphic-git`, `createWhiteboardDir`, `sealWhiteboardCandidate`, `materializeWhiteboardRevision`.
...
Deleted (packages/whiteboard/src unless noted):
...
Trimmed: `cli-runner.ts`, `cli.ts`, `map-cli.ts`, `publish-preparation.ts` (→ only `resolvePublishWhiteboard`), `whiteboard-worktree-target.ts`, `whiteboard-prepare.ts`, `whiteboard-artifact-promotion.ts`, `whiteboard-home.ts` (`updateReviewPins`, `findWhiteboardForRepair`), `whiteboard-telemetry.ts`, `server/desktop-server.ts`, `server/review-session-mode.ts`, `app/src/desktop-entry.tsx`, `packages/whiteboard-protocol/src/{contracts,index}.ts`, `apps/whiteboard-desktop/code-oss/src/vs/whiteboard/browser/parts/canvas/whiteboardCanvasPart.ts`, `apps/whiteboard-desktop/scripts/native-authoring-e2e.mjs`, docs.
...
**Files:**
- Verify only, unless the grep finds hits: `packages/whiteboard/app/src/desktop-entry.tsx`, `apps/whiteboard-desktop/code-oss/src/vs/whiteboard/browser/parts/canvas/whiteboardCanvasPart.ts`, `packages/whiteboard-protocol/src/contracts.ts`
...
**Files:**
- Modify: `packages/whiteboard/src/server/desktop-server.ts`
- Delete: `packages/whiteboard/src/server/review-repair-promotion.ts`, `server/review-repair-promotion.test.ts`, `server/desktop-server-repair.test.ts`, `src/review-repair-state.ts`, `src/review-repair-state.test.ts`
- Modify: `packages/whiteboard/src/server/review-session-mode.ts:10-17,36-41`
- Modify: `packages/whiteboard/src/whiteboard-telemetry.ts:44,51,393-398`, `whiteboard-telemetry.test.ts`
- Modify: `packages/whiteboard-protocol/src/contracts.ts:9,1017-1031`, `src/index.ts:44-45,139-143`, `src/contracts.test.ts:166`
- Modify: `packages/whiteboard/src/server/desktop-server.test.ts:504-588`, `desktop-server-import.test.ts:251-287`, `whiteboard-busy.test.ts:104-130`

**Interfaces:**
- Produces: `desktop-server.ts` no longer exports or registers `/publish-ready`, `/repair-ready`, `/map-publish-ready`; `GlobalWhiteboardServerInput` loses nothing else.

- [ ] **Step 1: Delete the three test surfaces first**

Delete `server/desktop-server-repair.test.ts` and `server/review-repair-promotion.test.ts`. In `desktop-server.test.ts` delete the top-level `it("rejects a publication whose review moved its base ref…")` at `:504-588`. In `desktop-server-import.test.ts` delete the `migrated`-refusal table at `:251-287`. In `whiteboard-busy.test.ts` delete the `runReviewPublish(...)` call (`:104-109`) and the `["publish-ready","map-publish-ready","repair-ready"]` fan-out (`:110-130`) and the corresponding `expect`s on those results; the test keeps asserting that `open` during a locked review reports busy.

Run `pnpm vitest run --config vitest.config.ts src/server/desktop-server.test.ts src/server/desktop-server-import.test.ts src/whiteboard-busy.test.ts`. Expected: the remaining cases pass (the deleted code is still present, so nothing else moves yet).

- [ ] **Step 2: Delete the routes and their helpers from `desktop-server.ts`**

Delete, by symbol: `PublishMountTiming` (`:139-143`), `MountVerbResultSchema` (`:145-157`), `mountStepTimings` (`:1265-1273`), the `promoteReviewRepair` import (`:132`), the `ReviewRepairReadyRequestSchema` import (`:105`), `importAfterPromotion` (`:325-332`), `migratedError` (`:334-341`), `POST /publish-ready` (`:1003-1043`), `POST /repair-ready` (`:1044-1070`), `POST /map-publish-ready` (`:1071-1106`), `mountPublishedDocument` (`:1274-1460`, now register-and-promote only), `mountPublishedSoftwareMap` (`:1461-1603`), `promoteReview` (`:2650-2674`), `promoteSoftwareMap` (`:2675-2688`), `rejectTerminalPublication` (`:2706-2718`), `rejectConcurrentPublication` (`:2719-2735`). Delete the `repairValidation` arm: the field at `:202`, the mode branch at `:2035-2038`, and collapse the ternary at `:2049-2053` to `findWhiteboard`. Delete the two `findWhiteboardForRepair` call sites (`:1049`, `:2050`) and then `findWhiteboardForRepair` from `whiteboard-home.ts:402`.
...
`git rm packages/whiteboard/src/server/review-repair-promotion.ts packages/whiteboard/src/review-repair-state.ts packages/whiteboard/src/review-repair-state.test.ts`. In `review-session-mode.ts` remove the `repairValidation` variant (`:11-17`) so `ReviewSessionMode` is `live | historical`; `reviewSessionModeIsReadOnly` keeps returning `!mode.isPromoted()`.
...
`whiteboard-telemetry.ts`: delete `capturePublishGateRejected` (`:393-398`) and remove `"publish"` (`:44`) and `"map.publish"` (`:51`) from `WhiteboardCliCommandPath`. Delete its case from `whiteboard-telemetry.test.ts`. Keep `captureWhiteboardReaped`.

- [ ] **Step 5: Typecheck, run the server suites**

```bash
pnpm --filter @dev.fast/whiteboard-protocol build && pnpm --filter @dev.fast/whiteboard-desktop protocol:sync
pnpm --filter @dev.fast/review typecheck
cd packages/whiteboard && pnpm vitest run --config vitest.config.ts src/server src/whiteboard-busy.test.ts src/whiteboard-telemetry.test.ts
```
...
```bash
git add -A packages/whiteboard packages/whiteboard-protocol apps/whiteboard-desktop
...
- Modify: `src/cli-runner.ts`, `src/cli.ts:71-77`, `src/map-cli.ts`, `src/publish-preparation.ts`, `src/publish-preparation.test.ts`, `src/whiteboard-worktree-target.ts`, `src/whiteboard-prepare.ts`, `src/whiteboard-artifact-promotion.ts`, `src/whiteboard-artifact-promotion.test.ts`, `src/whiteboard-home.ts:304`, `src/cli.test.ts`, `src/whiteboard-info.test.ts`, `src/whiteboard-head-checkout.test.ts`

**Interfaces:**
- Consumes: `syntheticLegacyWhiteboard` from `src/review-import/import-test-utils.ts:44` as the fixture builder that replaces `runReviewScaffold` in tests.
...
Delete the dispatch (`:128-137`), the option wiring (`:354-356`), the help lines (`:200`, `:211`) and the import (`:31`). Keep `resolvePublishWhiteboard` usage at `:618` and `:685` (`review map check --review`, `review map open`).
...
`publish-preparation.ts`: keep only `resolvePublishWhiteboard` (`:98`) and its helpers; delete `prepareReviewPublish` (`:21`). Trim `publish-preparation.test.ts` to the `resolvePublishWhiteboard` cases.

`whiteboard-worktree-target.ts`: delete `prepareReviewSourceTargetForRef` (`:92`) and `ensurePinnedWhiteboardWorktreeAtCommit` (`:122-176`). Keep `resolveWhiteboardSessionBaseCommit`, `resolveWhiteboardRepoRootFromStore`, `readWhiteboardStoreRecord`.

`whiteboard-prepare.ts`: keep `removeWhiteboardPrepareArtifacts` (`:66`) which `whiteboard-head-checkout.ts:20` imports; delete `prepareWhiteboardPinnedCheckout` (`:78`), `markerMatches` (`:196`), `resolveReviewPrepareCliEntryPath` (`:215`), `spawnReviewPrepareBackground` (`:236`), `whiteboardPrepareCommandsHash` (`:55`) and now-unused imports. If the file is under 30 lines afterwards, move `removeWhiteboardPrepareArtifacts` into `whiteboard-head-checkout.ts` and delete the file.

`whiteboard-artifact-promotion.ts`: delete `commitWhiteboardArtifactPromotion` and `rollbackWhiteboardArtifactPromotion`; keep `promoteWhiteboardArtifactFiles` (used by `stored-review-migration.ts`). Trim its test to the kept function.

`whiteboard-home.ts`: delete `updateReviewPins` (`:304`); its only importer was `review-scaffold.ts`.

`whiteboard-head-checkout.test.ts`: delete the assertions that reference `review-prepare` markers.
...
`whiteboard-info.test.ts`: replace every `runReviewScaffold({ cwd, … })` fixture with `syntheticLegacyWhiteboard(...)` from `src/review-import/import-test-utils.ts:44` (it creates a `~/.dev/reviews/<uuid>` directory with a pinned worktree without touching MDX). Delete the cases whose subject is scaffold behaviour itself (`:48`, `:257-320`, `:354` "re-pins with scaffold --update"); keep the `review info` resolution cases (`:139`, `:191`, `:228`, `:339`). `prepareReviewPublish` import (`:9`) goes.

Run:

```bash
pnpm vitest run --config vitest.config.ts src/cli.test.ts src/whiteboard-info.test.ts src/publish-preparation.test.ts src/whiteboard-artifact-promotion.test.ts src/whiteboard-head-checkout.test.ts src/map-cli.test.ts
```
...
cd packages/whiteboard && for f in review-source-pins review-derived-paths software-map-health; do echo "== $f"; git grep -l "from \"\./$f\"\|from \"\.\./$f\"" -- src | grep -v test; done
```

Expected: `whiteboard-source-pins.ts` still has `publish-stage.ts` and `stored-review-migration.ts`; `whiteboard-derived-paths.ts` still has `stored-review-migration.ts` and the fixture loader; `software-map-health.ts` `loadPublishSoftwareMaps` is still used by `scripts/build-tutorial-assets.ts` (PR 2 removes that). Anything with zero non-test importers is deleted now.

- [ ] **Step 7: Commit**

```bash
git add -A packages/whiteboard
...
**Files:**
- Modify: `apps/whiteboard-desktop/scripts/native-authoring-e2e.mjs:440-680,750-770,884-960`
- Modify: `apps/whiteboard-desktop/scripts/legacy-import-live-home.sh` (comment only)
- Modify: `docs/superpowers/plans/2026-09-16-legacy-import-live-test-plan.md`

The harness seeds legacy reviews with `scaffold` + `publish` (`:440-445`, `:520-680`) and proves the `migrated` refusal (`:609-679`) and `repair` on an un-importable review (`:760`). Those flows no longer exist. The fixture tarballs in `packages/whiteboard/src/fixtures/legacy-reviews/*.tgz` are the seed instead, exactly as `legacy-import-live-home.sh:31-38` already does.
...
async function seedLegacyFixture(homeDir, name) {
  const fixtures = path.join(workspace, "packages/whiteboard/src/fixtures/legacy-reviews");
...
```bash
git add apps/whiteboard-desktop/scripts docs/superpowers/plans/2026-09-16-legacy-import-live-test-plan.md
...
**Files:**
- Modify: `packages/whiteboard/skills/dev-review/SKILL.md:43`
...
- Modify: `packages/whiteboard/README.md:3,46,55,58,67,79,85,88`
- Modify: `packages/whiteboard/onboarding.md` (whole file; shipped in the npm `files` list)
- Modify: `apps/whiteboard-desktop/README.md:12-13,75,81,94,173`
- Modify: `scripts/review-latency/README.md:38,51,52`

- [ ] **Step 1: Rewrite the authoring story once**

`onboarding.md` becomes a five-line pointer: install, open Desktop, run the dev-review skill, which authors through `review api` / MCP. `packages/whiteboard/README.md:3` becomes "dev.fast Review is a JSON review canvas hosted by Review Desktop." and the scaffold/publish sections (`:46-88`) are replaced by a link to `skills/dev-review/SKILL.md`. `SKILL.md:43` is replaced by: "Legacy `review scaffold/publish/repair` commands no longer exist. Whiteboards published from MDX before this release were imported into the JSON store and are edited through `review api` or these MCP tools."

- [ ] **Step 2: Remove the verbs from the reference docs**

`docs/cli-reference.md`: delete the `scaffold`, `publish`, `repair` rows and sections; keep `map` with `open|check|prune|push|fetch`; add rows for `api` and `mcp` pointing at `review api tools`. `docs/how-review-works.md`: replace the "Authoring remains review.mdx and data.ts" paragraphs (`:48`, `:134`) with the JSON route; delete `:43,63,98,103`. `docs/troubleshooting.md`: delete the six publish/scaffold entries. `docs/telemetry.md`: delete the `review_publish_gate_rejected` row (`:183`) and the note at `:259`. `apps/whiteboard-desktop/README.md`: delete `:12-13` (publish-ready), `:75`, `:81`; keep `:90-94` but reword to "were imported". `scripts/review-latency/README.md`: mark the publish-keyed phases as historical.

- [ ] **Step 3: Grep for leftovers**

```bash
git grep -n "review publish\|review scaffold\|review repair\|review rebind\|publish-ready\|map publish\|prepare-worktree" -- docs packages/whiteboard/README.md packages/whiteboard/onboarding.md packages/whiteboard/skills apps/whiteboard-desktop/README.md
```
...
```bash
git add docs packages/whiteboard/README.md packages/whiteboard/onboarding.md packages/whiteboard/skills apps/whiteboard-desktop/README.md scripts/review-latency/README.md
...
```bash
pnpm --filter @dev.fast/review typecheck && pnpm --filter @dev.fast/whiteboard-desktop typecheck
pnpm lint && pnpm format:check
cd packages/whiteboard && pnpm test:node && pnpm test:browser
```
...
- Not covered on purpose: `review info`, `review app pick`, `review migrate` (PR 5); `loadPublishSoftwareMaps` and `bundleWhiteboardSoftwareMap` (PR 2/3).
