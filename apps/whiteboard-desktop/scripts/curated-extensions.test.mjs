...
const curatedContribution = await readFile(
  new URL(
    "../code-oss/src/vs/whiteboard/contrib/extensions/whiteboardCuratedExtensions.contribution.ts",
    import.meta.url,
  ),
  "utf8",
);

const whiteboardConfiguration = await readFile(
  new URL(
...
const mainOptionalCatalog = (
  await loadImportFreeTypeScriptModule(
    new URL(
      "../code-oss/src/vs/whiteboard/node/whiteboardOptionalExtensionCatalog.ts",
      import.meta.url,
    ),
  )
).whiteboardOptionalExtensionCatalog;
...
test("carries Darwin curated extensions from Linux compile through release validation", async () => {
  ...
  assert.match(buildScript, /WHITEBOARD_DESKTOP_CURATED_EXTENSION_TARGET/);
  assert.match(
    compileScript,
    /WHITEBOARD_DESKTOP_CURATED_EXTENSION_TARGET=darwin-arm64/,
  );
  assert.match(
    compileScript,
...
test("keeps the in-app picker list in sync with the manifest", () => {
  ...
  for (const extension of bundledExtensions) {
    assert.ok(
      ...
      `${extension.id} must appear in whiteboardCuratedExtensions.contribution.ts`,
    );
  }
...
test("manages optional extensions as four user-facing groups", () => {
  ...
  assert.match(curatedContribution, /vscode:whiteboardDownloadOptionalExtension/);
...
test("keeps the keymaps mutually exclusive in the picker", () => {
  ...
  const enumDeclaration = whiteboardConfiguration.match(
    /WHITEBOARD_KEYMAPS\s*=\s*\[([^\]]+)\]/,
  );
